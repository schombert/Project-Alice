...
enum token_type { none, group_ident, group_item_ident, ident, lparen, rparen, newline };
struct token {
...
struct parser_state {
	...
	parser_state(std::string_view const _file_name) : file_name{_file_name} { char_facet = &std::use_facet<std::ctype<char>>(std::locale("C")); }
	...
	size_t get_column(std::string_view const s, std::string_view::iterator const it) { return size_t(std::distance(s.begin(), it)); }

	bool is_ident(char c) { return std::isalnum(c) || c == '#' || c == '@' || c == '_'; }
	...
	std::optional<token> get_token(auto& it) { return std::optional(*it++); }
		...
		while(it != tokens.end()) {
			...
			if(key.type == token_type::group_ident) {
				...
				if(key.data == "#free") {
					...
					auto const handler_opt = tmp.type == token_type::lparen ? get_specific_token(it, err_cond, token_type::ident) : token{};
					...
					static std::vector<std::string_view> const valid_group_handler_values = {"discard", "member", "member_fn", "function"};
...
std::string final_match_condition_internal(std::string_view const key, int32_t starting_position, int32_t ending_position) {
	...
	if(ending_position - starting_position >= 8) {
		return " && (*(uint64_t*)(&cur.content[" + std::to_string(starting_position) + "]) | uint64_t(0x2020202020202020) ) == uint64_t(" + string_to_hex(key, starting_position, 8) + ")" + final_match_condition_internal(key, starting_position + 8, ending_position);
	} else if(ending_position - starting_position >= 4) {
		return " && (*(uint32_t*)(&cur.content[" + std::to_string(starting_position) + "]) | uint32_t(0x20202020) ) == uint32_t(" + string_to_hex(key, starting_position, 4) + ")" + final_match_condition_internal(key, starting_position + 4, ending_position);
	} else if(ending_position - starting_position >= 2) {
		return " && (*(uint16_t*)(&cur.content[" + std::to_string(starting_position) + "]) | 0x2020 ) == " + string_to_hex(key, starting_position, 2) + final_match_condition_internal(key, starting_position + 2, ending_position);
	} else { /// single char
...
template<typename V, typename F> void enum_with_prefix(V const& vector, std::string_view const prefix, int32_t length, F const& fn) {
...
template<typename V> int32_t count_with_prefix(V const& vector, std::string_view const prefix, int32_t length) {
	int32_t total = 0;
	enum_with_prefix(vector, prefix, length, [&](auto const&) { ++total; });
	return total;
}

template<typename V> int32_t max_length(V const& vector) {
	int32_t mx = 0;
...
struct cxx_tree_builder {
	...
	void tabulate_increment() { tabs.push_back('\t'); }

	void tabulate_decrement() { tabs.pop_back(); }

	std::string tabulate(std::string_view const s) const { return tabs + s.data(); }

	template<typename V> std::string get_match_tree_running_prefix(V const& vector, std::string prefix, int32_t length) {
	...
	template<typename V, typename F> std::string construct_match_tree_internal(V const& vector, F const& generator_match, std::string_view const no_match, std::string_view const prefix, int32_t length) {
		int32_t top_count = count_with_prefix(vector, prefix, length);
		std::string output;
		...
		for(auto& g : groups) {
			...
			output += "\t\t if(cur.type == token_type::open_brace) { \n";
			{
				...
				if(g.set_handler.is_extern == false) {
					if(g.set_handler.handler.value == "discard") {
						...
						set_effect = "cobj." + (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + " = parse_" + g.set_handler.type_or_function + "(gen, err, context);";
					} else if(g.set_handler.handler.value == "member_fn") {
						set_effect = "cobj." + (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + "(parse_" + g.set_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else if(g.set_handler.handler.value == "function") {
						set_effect = (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + "(cobj, parse_" + g.set_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else {
					...
					if(g.set_handler.handler.value == "discard") {
						...
						set_effect = "cobj." + (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + " = " + g.set_handler.type_or_function + "(gen, err, context);";
					} else if(g.set_handler.handler.value == "member_fn") {
						set_effect = "cobj." + (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + "(" + g.set_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else if(g.set_handler.handler.value == "function") {
						set_effect = (g.set_handler.handler.opt.length() > 0 ? g.set_handler.handler.opt : std::string("free_group")) + "(cobj, " + g.set_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else {
						set_effect = "err.unhandled_free_group(cur); gen.discard_group();";
					}
				}
				...
				*/
				...
				if(g.any_group_handler.is_extern == false) {
					if(g.any_group_handler.handler.value == "discard") {
						...
						no_match_effect = "cobj." + (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + " = parse_" + g.any_group_handler.type_or_function + "(gen, err, context);";
					} else if(g.any_group_handler.handler.value == "member_fn") {
						no_match_effect = "cobj." + (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + "(cur.content, parse_" + g.any_group_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else if(g.any_group_handler.handler.value == "function") {
						no_match_effect = (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + "(cobj, cur.content, parse_" + g.any_group_handler.type_or_function + "(gen, err, context), err, cur.line, context);";
					} else {
					...
					if(g.any_group_handler.handler.value == "discard") {
						...
						no_match_effect = "cobj." + (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + " = " + g.any_group_handler.type_or_function + "(cur.content, gen, err, context);";
					} else if(g.any_group_handler.handler.value == "member_fn") {
						no_match_effect = "cobj." + (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + "(cur.content, " + g.any_group_handler.type_or_function + "(cur.content, gen, err, context), err, cur.line, context);";
					} else if(g.any_group_handler.handler.value == "function") {
						no_match_effect = (g.any_group_handler.handler.opt.length() > 0 ? g.any_group_handler.handler.opt : std::string("any_group")) + "(cobj, cur.content, " + g.any_group_handler.type_or_function + "(cur.content, gen, err, context), err, cur.line, context);";
					} else {
						no_match_effect = "err.unhandled_group_key(cur); gen.discard_group();";
					}
				}
				...
				auto match_handler = [](group_association const& v) {
					...
					if(v.is_extern) {
						if(v.handler.value == "discard") {
							...
							out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + " = " + v.type_or_function + "(gen, err, context);";
						} else if(v.handler.value == "member_fn") {
							out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(" + v.type_or_function + "(gen, err, context), err, cur.line, context);";
						} else if(v.handler.value == "function") {
							out = (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(cobj, " + v.type_or_function + "(gen, err, context), err, cur.line, context);";
						} else {
						...
						if(v.handler.value == "discard") {
							...
							out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + " = parse_" + v.type_or_function + "(gen, err, context);";
						} else if(v.handler.value == "member_fn") {
							out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(parse_" + v.type_or_function + "(gen, err, context), err, cur.line, context);";
						} else if(v.handler.value == "function") {
							out = (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(cobj, parse_" + v.type_or_function + "(gen, err, context), err, cur.line, context);";
						} else {
							out = "err.unhandled_group_key(cur);";
						}
					}
			...
			// match values
			{
				...
				if(g.any_value_handler.handler.value == "discard") {
					...
					no_match_effect = "cobj." + (g.any_value_handler.handler.opt.length() > 0 ? g.any_value_handler.handler.opt : std::string("any_value")) + " = parse_" + g.any_value_handler.type + "(rh_token.content, rh_token.line, err);";
				} else if(g.any_value_handler.handler.value == "member_fn") {
					no_match_effect = "cobj." + (g.any_value_handler.handler.opt.length() > 0 ? g.any_value_handler.handler.opt : std::string("any_value")) + "(cur.content, assoc_type, parse_" + g.any_value_handler.type + "(rh_token.content, rh_token.line, err), err, cur.line, context);";
				} else if(g.any_value_handler.handler.value == "function") {
					no_match_effect = (g.any_value_handler.handler.opt.length() > 0 ? g.any_value_handler.handler.opt : std::string("any_value")) + "(cobj, cur.content, assoc_type, parse_" + g.any_value_handler.type + "(rh_token.content, rh_token.line, err), err, cur.line, context);";
				} else {
				...
				*/
				auto match_handler = [](value_association const& v) {
					...
					if(v.handler.value == "discard") {
						...
						out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + " = parse_" + v.type + "(rh_token.content, rh_token.line, err);";
					} else if(v.handler.value == "member_fn") {
						out = "cobj." + (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(assoc_type, parse_" + v.type + "(rh_token.content, rh_token.line, err), err, cur.line, context);";
					} else if(v.handler.value == "function") {
						out = (v.handler.opt.length() > 0 ? v.handler.opt : v.key) + "(cobj, assoc_type, parse_" + v.type + "(rh_token.content, rh_token.line, err), err, cur.line, context);";
					} else {
			...
			if(g.single_value_handler_result.value.length() > 0) {
				if(g.single_value_handler_result.value == "discard") {
					...
					output += "\t\t\t cobj." + (g.single_value_handler_result.opt.length() > 0 ? g.single_value_handler_result.opt : std::string("free_value")) + "= parse_" + g.single_value_handler_type + "(cur.content, cur.line, err);\n";
				} else if(g.single_value_handler_result.value == "member_fn") {
					output += "\t\t\t cobj." + (g.single_value_handler_result.opt.length() > 0 ? g.single_value_handler_result.opt : std::string("free_value")) + "(parse_" + g.single_value_handler_type + "(cur.content, cur.line, err), err, cur.line, context);\n";
				} else if(g.single_value_handler_result.value == "function") {
					output += "\t\t\t " + (g.single_value_handler_result.opt.length() > 0 ? g.single_value_handler_result.opt : std::string("free_value")) + "(cobj, parse_" + g.single_value_handler_type + "(cur.content, cur.line, err), err, cur.line, context);\n";
				}
			} else {
