...
/*
 * NOTE :
 * 	The Structs described in this file have a potential to change as time progresses and more is figured out of the XAC File
 * Format, Generally speaking named variables can be *roughly* guaranteed to remain constant
 */

struct xac_header {
	char magic[4] = {'X', 'A', 'C', ' '}; // Offset 0x0h  --- Magic Value
	uint32_t version; // Offset 0x4h  --- changes depending on if the file was created by Maya 8.5 x64 or not, if it is then its
					  // 01 00 00 01 otherwise its 01 00 00 00
};					  // Size: 8 bytes

struct metadata_section {
	uint32_t section_id;  // Offset 0x8h  --- Based off leafs aggregation, this requires further research though to make sure its
						  // valid, it is always 07 00 00 00
	...
	char workinfo[workfilesize]; //              --- Contains the information regarding the originial file the XAC file was
								 //              generated from
...
struct {
	...
	// element is followed by a UID, however the first instance of a element does not need one, with the next instance of the
element starting out with UID 1 uint16_t elementname_size;  // Precedes the element name, its size includes the element name and
the UID char element[elementname_size] = "polySurface"                         // 0xA0 Bytes large
									...
									|| "Wheel_"                             // 0xA0 Bytes large, however numbers often appear at
the end of the next elements data causing it to appear larger
	...
	// the element is then ended with what appears to be a UID of the type, presumably unique within the same xac file aswell as
its accompying xsm files if any. } entry;
