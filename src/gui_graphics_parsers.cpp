...
namespace parsers {
	...
	obj_and_horizontal common_create_object(gfx_object const& obj_in, building_gfx_context const& context) {
		...
		if(obj_in.primary_texture.length() > 0) {
			if(auto it = context.ui_defs->map_of_textures.find(std::string(obj_in.primary_texture)); it != context.ui_defs->map_of_textures.end()) {
				new_obj.primary_texture_handle = it->second;
			} else {
				auto index = context.ui_defs->textures.size();
				context.ui_defs->textures.emplace_back();
				context.ui_defs->textures.back().file_name = obj_in.primary_texture;
				new_obj.primary_texture_handle = dcon::texture_id(uint16_t(index));
				context.ui_defs->map_of_textures.insert_or_assign(std::string(obj_in.primary_texture), dcon::texture_id(uint16_t(index)));
			}
		}
	...
	void gfx_object_outer::spritetype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::generic_sprite);

	}
	void gfx_object_outer::corneredtilespritetype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::bordered_rect);
	}
	void gfx_object_outer::maskedshieldtype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::flag_mask);
	}
	void gfx_object_outer::textspritetype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::text_sprite);
	}
	void gfx_object_outer::tilespritetype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::tile_sprite);
	}
	void gfx_object_outer::progressbartype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		if(res.horizontal)
	...
	void gfx_object_outer::barcharttype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::barchart);
	}
	void gfx_object_outer::piecharttype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
		res.obj->flags |= uint8_t(ui::object_type::piechart);
	}
	void gfx_object_outer::linecharttype(gfx_object const& obj, parsers::error_handler& err, int32_t line, building_gfx_context const& context) {
		auto res = common_create_object(obj, context);
