...
)
{
  ...
  if (positions_to_this && !position_from_this_list)
    {
      position_from_this_list = (FriBidiStrIndex*)fribidi_malloc (sizeof
...
)
{
  ...
  if (len < LOCAL_LIST_SIZE)
    ...
    bidi_types = (FriBidiCharType*)fribidi_malloc (len * sizeof bidi_types[0]);
  if (!bidi_types)
    goto out;
  ...
  if (len < LOCAL_LIST_SIZE)
    ...
    bracket_types = (FriBidiBracketType*)fribidi_malloc (len * sizeof bracket_types[0]);
  ...
  if (!embedding_levels)
    {
      if (len < LOCAL_LIST_SIZE)
        ...
        embedding_levels = (FriBidiLevel*)fribidi_malloc (len * sizeof embedding_levels[0]);
      if (!embedding_levels)
	goto out;
	...
	visual_str[i] = str[i];
      */
      ...
      if (len < LOCAL_LIST_SIZE)
        ...
        ar_props = (FriBidiArabicProp*)fribidi_malloc (len * sizeof ar_props[0]);
