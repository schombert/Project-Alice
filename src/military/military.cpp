		...
		for(uint32_t j = slots.size(); j-- > 0;) {
			...
			case ship_in_battle::mode_engaged:
			{
				...
				*/
				...
				state.world.ship_set_org(tship, std::max(0.0f, torg - org_damage));
				auto& tstr = state.world.ship_get_strength(tship);
				state.world.ship_set_strength(tship, std::max(0.0f, tstr - str_damage));
			...
			case ship_in_battle::mode_engaged:
				if(state.world.ship_get_strength(slots[j].ship) <= 0) {
					if((slots[j].flags & ship_in_battle::is_attacking) != 0) {
						...
						auto& cur_lost = state.world.naval_battle_get_attacker_loss_value(b);
						state.world.naval_battle_set_attacker_loss_value(b, cur_lost + state.military_definitions.unit_base_definitions[type].supply_consumption_score);
					} else {
						if((slots[j].flags & ship_in_battle::type_mask) == ship_in_battle::type_big) {
						...
						auto& cur_lost = state.world.naval_battle_get_defender_loss_value(b);
						state.world.naval_battle_set_defender_loss_value(b, cur_lost + state.military_definitions.unit_base_definitions[type].supply_consumption_score);
					}
					slots[j].flags &= ~ship_in_battle::mode_mask;
