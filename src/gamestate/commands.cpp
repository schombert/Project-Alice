...
bool can_start_research(sys::state& state, dcon::nation_id source, dcon::technology_id tech) {
	...
	if(state.current_date.to_ymd(state.start_date).year >= state.world.technology_get_year(tech)) {
		...
		if(tech.index() != 0 &&
			state.world.technology_get_folder_index(prev_tech) == state.world.technology_get_folder_index(tech)) {
...
bool can_give_war_subsidies(sys::state& state, dcon::nation_id source, dcon::nation_id target) {
	...
	return state.world.nation_get_diplomatic_points(source) >=
		   state.defines.warsubsidy_diplomatic_cost; // Enough diplomatic points
}
...
void execute_begin_province_building_construction(sys::state& state, dcon::nation_id source, dcon::province_id p,
	...
	if(type == economy::province_building_type::railroad &&
		source != state.world.province_get_nation_from_province_ownership(p)) {
		float amount = 0.0f;
...
bool can_begin_factory_building_construction(sys::state& state, dcon::nation_id source, dcon::state_instance_id location,
	...
	*/
	...
	if(state.world.nation_get_active_building(source, type) == false &&
		!state.world.factory_type_get_is_available_from_start(type))
		return false;
	...
	if(!is_upgrade) {
		...
		bool has_dup = false;
		economy::for_each_new_factory(state, location,
			[&](economy::new_factory const& nf) { has_dup = has_dup || nf.type == type; });
		if(has_dup)
			return false;
...
void execute_change_budget_settings(sys::state& state, dcon::nation_id source, budget_settings_data const& values) {
	...
	if(values.administrative_spending != int8_t(-127)) {
		state.world.nation_set_administrative_spending(source,
			std::clamp(values.administrative_spending, int8_t(0), int8_t(100)));
	}
...
void execute_upgrade_colony_to_state(sys::state& state, dcon::nation_id source, dcon::state_instance_id si) {
	...
	event::fire_fixed_event(state, state.national_definitions.on_colony_to_state, trigger::to_generic(si),
...
void execute_intervene_in_war(sys::state& state, dcon::nation_id source, dcon::war_id w, bool for_attacker) {
	...
	if(!state.world.war_get_is_great(w)) {
		...
		if(!status_quo_added) {
			...
			assert(status_quo);
			military::add_wargoal(state, w, source, state.world.war_get_primary_attacker(w), status_quo,
				dcon::state_definition_id{}, dcon::national_identity_id{}, dcon::nation_id{});
		}
	}
}
	...
	*/
	...
	if(e) {
		...
		if(!t || trigger::evaluate(state, t, trigger::to_generic(source), trigger::to_generic(source), 0))
			effect::execute(state, e, trigger::to_generic(source), trigger::to_generic(source), 0,
				uint32_t(state.current_date.value), uint32_t(source.index()));
	}
...
void execute_enact_issue(sys::state& state, dcon::nation_id source, dcon::issue_option_id i) {
	...
	for(auto m : state.world.nation_get_movement_within(source)) {
		if(m.get_movement().get_associated_issue_option() && m.get_movement().get_associated_issue_option() != i &&
			...
			m.get_movement().get_transient_radicalism() +=
				std::min(3.0f, m.get_movement().get_pop_support() / winner_support - 1.0f) *
	...
	*/
	...
	for(auto id : state.world.in_ideology) {
		...
		if(condition) {
			auto factor = trigger::evaluate_additive_modifier(state, condition, trigger::to_generic(source),
				trigger::to_generic(source), 0);
				...
				for(auto pr : state.world.nation_get_province_ownership(source)) {
					for(auto pop : pr.get_province().get_pop_location()) {
						...
						pop.get_pop().set_militancy(
							base_mil + pop.get_pop().get_demographics(idsupport_key) * factor *
...
void execute_make_event_choice(sys::state& state, dcon::nation_id source, pending_human_n_event_data const& e) {
	event::take_option(state,
		event::pending_human_n_event{e.r_lo, e.r_hi, e.primary_slot, e.pt, e.from_slot, e.ft, e.e, source, e.date}, e.opt_choice);
}
...
void execute_pending_commands(sys::state& state) {
	...
	while(c) {
		...
		case command_type::change_factory_settings:
			execute_change_factory_settings(state, c->source, c->data.factory.location, c->data.factory.type,
				c->data.factory.priority, c->data.factory.subsidize);
			break;
		...
		case command_type::expel_advisors:
			execute_expel_advisors(state, c->source, c->data.influence_action.influence_target,
				c->data.influence_action.gp_target);
			break;
		...
		case command_type::discredit_advisors:
			execute_discredit_advisors(state, c->source, c->data.influence_action.influence_target,
				c->data.influence_action.gp_target);
			break;
		case command_type::decrease_opinion:
			execute_decrease_opinion(state, c->source, c->data.influence_action.influence_target,
				c->data.influence_action.gp_target);
			break;
		case command_type::remove_from_sphere:
			execute_remove_from_sphere(state, c->source, c->data.influence_action.influence_target,
				c->data.influence_action.gp_target);
			break;
		...
		case command_type::upgrade_colony_to_state:
			execute_upgrade_colony_to_state(state, c->source,
				state.world.province_get_state_membership(c->data.generic_location.prov));
			break;
		...
		case command_type::respond_to_diplomatic_message:
			execute_respond_to_diplomatic_message(state, c->source, c->data.message.from, c->data.message.type,
				c->data.message.accept);
			break;
		...
		case command_type::add_war_goal:
			execute_add_war_goal(state, c->source, c->data.new_war_goal.war, c->data.new_war_goal.target,
				c->data.new_war_goal.cb_type, c->data.new_war_goal.cb_state, c->data.new_war_goal.cb_tag,
				c->data.new_war_goal.cb_secondary_nation);
			break;
