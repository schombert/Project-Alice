...
template<typename T>
size_t serialize_size(std::vector<T> const& vec) { return sizeof(uint32_t) + sizeof(T) * vec.size(); }
...
constexpr inline uint32_t scenario_file_version = 55 + save_file_version;
