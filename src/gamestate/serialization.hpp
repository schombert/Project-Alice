...
inline size_t serialize_size(ankerl::unordered_dense::map<province::continent_list, dcon::modifier_id> const& vec) {
	return serialize_size(vec.values());
}

inline uint8_t* serialize(uint8_t* ptr_in, ankerl::unordered_dense::map<province::continent_list, dcon::modifier_id> const& vec) {
	return serialize(ptr_in, vec.values());
}
uint8_t const* deserialize(uint8_t const* ptr_in, ankerl::unordered_dense::map<province::continent_list, dcon::modifier_id>& vec) {
	uint32_t length = 0;
	memcpy(&length, ptr_in, sizeof(uint32_t));
...
constexpr inline uint32_t scenario_file_version = 18 + save_file_version;
