...
void decline(sys::state& state, message const& m) {
	...
	case type::access_request:
		...
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_access_refused_1", text::variable_type::x, target, text::variable_type::y, source); }, "msg_access_refused_title", m.to,
									  sys::message_setting_type::mil_access_declined_by_nation});
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_access_refused_1", text::variable_type::x, target, text::variable_type::y, source); }, "msg_access_refused_title", m.from,
	...
	case type::alliance_request:
		...
		notification::post(state, notification::message{[from = m.from, to = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_alliance_declined_1", text::variable_type::x, to, text::variable_type::y, from); }, "msg_alliance_declined_title", m.to,
									  sys::message_setting_type::alliance_declined_by_nation});
		notification::post(state, notification::message{[from = m.from, to = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_alliance_declined_1", text::variable_type::x, to, text::variable_type::y, from); }, "msg_alliance_declined_title", m.from,
	...
	case type::call_ally_request: {
		...
		if(was_defensive) {
			...
			if(rel) {
				...
				notification::post(state, notification::message{[from = m.from, to = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_alliance_ends_1", text::variable_type::x, to, text::variable_type::y, from); }, "msg_alliance_ends_title", m.from,
											  sys::message_setting_type::alliance_ends});

				notification::post(state, notification::message{[from = m.from, to = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_alliance_ends_1", text::variable_type::x, to, text::variable_type::y, from); }, "msg_alliance_ends_title", m.to,
											  sys::message_setting_type::alliance_ends});
			}
		}

		notification::post(state, notification::message{[from = m.from, to = m.to, pa = state.world.war_get_primary_attacker(m.data.war), pd = state.world.war_get_primary_defender(m.data.war), name = state.world.war_get_name(m.data.war), tag = state.world.war_get_over_tag(m.data.war),
															st = state.world.war_get_over_state(m.data.war)](sys::state& state, text::layout_base& contents) {
									  ...
									  "msg_ally_call_declined_title", m.from, sys::message_setting_type::ally_called_declined_by_nation});
	...
	case type::take_crisis_side_offer:
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
															text::add_line(state, contents, "msg_crisis_joffer_reject_1", text::variable_type::x, target, text::variable_type::y, source);
														},
									  "msg_crisis_joffer_rejected", m.to, sys::message_setting_type::crisis_join_offer_declined_by_nation});
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
															text::add_line(state, contents, "msg_crisis_joffer_reject_1", text::variable_type::x, target, text::variable_type::y, source);
														},
									  "msg_crisis_joffer_rejected", m.from, sys::message_setting_type::crisis_join_offer_declined_from_nation});
		break;
	case type::crisis_peace_offer:
		...
		*/
		...
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
									  ...
									  "msg_crisis_not_settled_title", m.from, sys::message_setting_type::crisis_resolution_declined_from_nation});
...
bool can_accept_crisis_offer(sys::state& state, dcon::nation_id from, dcon::nation_id to, sys::crisis_join_offer const& offer) {
	...
	if(!military::cb_instance_conditions_satisfied(state, to, offer.target, offer.wargoal_type, offer.wargoal_state, offer.wargoal_tag, offer.wargoal_secondary_nation)) {
		...
		bool found = [&]() {
			for(auto& par : state.crisis_participants) {
				...
				if(par.id == wg.get_wargoal().get_added_by()) {
					if(wg.get_wargoal().get_associated_state() == par.joined_with_offer.wargoal_state && wg.get_wargoal().get_associated_tag() == par.joined_with_offer.wargoal_tag && wg.get_wargoal().get_secondary_nation() == par.joined_with_offer.wargoal_secondary_nation &&
						wg.get_wargoal().get_target_nation() == par.joined_with_offer.target && wg.get_wargoal().get_type() == par.joined_with_offer.wargoal_type)
						return true;
					else
						return false;
				}
			...
			if(wg.get_wargoal().get_added_by() == state.primary_crisis_attacker) {
				if(state.current_crisis == sys::crisis_type::colonial) {
					...
					if(num_colonizers >= 2) {
						...
						if(wg.get_wargoal().get_associated_state() == state.crisis_colony && wg.get_wargoal().get_type() == state.military_definitions.crisis_colony &&
							((wg.get_wargoal().get_added_by() == col_a && wg.get_wargoal().get_target_nation() == col_b) || (wg.get_wargoal().get_added_by() == col_b && wg.get_wargoal().get_target_nation() == col_a))) {

							found = true;
						}
					}
				} else if(state.current_crisis == sys::crisis_type::liberation) {
					if(wg.get_wargoal().get_associated_tag() == state.crisis_liberation_tag && wg.get_wargoal().get_associated_state() == state.world.state_instance_get_definition(state.crisis_state) && wg.get_wargoal().get_type() == state.military_definitions.crisis_liberate &&
...
void accept(sys::state& state, message const& m) {
	...
	case type::access_request: {
		...
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_access_granted_1", text::variable_type::x, target, text::variable_type::y, source); }, "msg_access_granted_title", m.to,
									  sys::message_setting_type::mil_access_start_by_nation});
		notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_access_granted_1", text::variable_type::x, target, text::variable_type::y, source); }, "msg_access_granted_title", m.from,
	...
	case type::call_ally_request: {
		...
		notification::post(state, notification::message{[from = m.from, to = m.to, pa = state.world.war_get_primary_attacker(m.data.war), pd = state.world.war_get_primary_defender(m.data.war), name = state.world.war_get_name(m.data.war), tag = state.world.war_get_over_tag(m.data.war),
									  ...
									  "msg_ally_call_accepted_title", m.from, sys::message_setting_type::ally_called_accepted_by_nation});
	...
	case type::take_crisis_side_offer:
		if(can_accept_crisis_offer(state, m.from, m.to, m.data.crisis_offer)) {
			...
			notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
										  ...
										  "msg_crisis_joffer_accepted", m.to, sys::message_setting_type::crisis_join_offer_accepted_by_nation});
			notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
																text::add_line(state, contents, "msg_crisis_joffer_accepted_1", text::variable_type::x, target, text::variable_type::y, source);
															},
										  "msg_crisis_joffer_accepted", m.from, sys::message_setting_type::crisis_join_offer_accepted_from_nation});
		}
		break;
	case type::crisis_peace_offer:
		if(can_accept_crisis_peace_offer(state, m.from, m.to, m.data.peace)) {
			...
			notification::post(state, notification::message{[source = m.from, target = m.to](sys::state& state, text::layout_base& contents) {
										  ...
										  "msg_crisis_settled_title", state.local_player_nation, sys::message_setting_type::crisis_resolution_accepted});
		}
		break;
	}
}
