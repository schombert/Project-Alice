...
namespace sys {
	...
	void state::on_rbutton_down(int32_t x, int32_t y, key_modifiers mod) {
		if(ui_state.under_mouse != nullptr) {
			...
			ui_state.under_mouse->impl_on_rbutton_down(*this, int32_t(x / user_settings.ui_scale), int32_t(y / user_settings.ui_scale), mod);
		}
	}
	...
	void state::on_lbutton_down(int32_t x, int32_t y, key_modifiers mod) {
		if(ui_state.under_mouse != nullptr) {
			// TODO: look at return value
			ui_state.under_mouse->impl_on_lbutton_down(*this, int32_t(x / user_settings.ui_scale), int32_t(y / user_settings.ui_scale), mod);
		}
	}
	...
	void state::on_mouse_move(int32_t x, int32_t y, key_modifiers mod) {
		if(ui_state.under_mouse != nullptr) {
			...
			auto r = ui_state.under_mouse->impl_on_mouse_move(*this, int32_t(x / user_settings.ui_scale), int32_t(y / user_settings.ui_scale), mod);
	...
	void state::on_mouse_wheel(int32_t x, int32_t y, key_modifiers mod, float amount) { // an amount of 1.0 is one "click" of the wheel
		if(ui_state.under_mouse != nullptr) {
			auto r = ui_state.under_mouse->impl_on_scroll(*this, int32_t(x / user_settings.ui_scale), int32_t(y / user_settings.ui_scale), amount, mod);
			if(r != ui::message_result::consumed) {
