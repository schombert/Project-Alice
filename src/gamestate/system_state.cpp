	...
	if(ui_state.under_mouse != nullptr) {
		...
		if(r != ui::message_result::consumed) {
			...
			if(ui_state.province_window) {
				static_cast<ui::province_view_window*>(ui_state.province_window)
					->set_active_province(*this, map_state.selected_province);
			}
		}
	} else {
		map_state.on_lbutton_down(*this, x, y, x_size, y_size, mod);
...
void state::on_mouse_wheel(int32_t x, int32_t y, key_modifiers mod,
	...
	if(ui_state.scroll_target != nullptr) {
		auto r = ui_state.scroll_target->impl_on_scroll(*this, ui_state.relative_mouse_location.x,
			ui_state.relative_mouse_location.y, amount, mod);
		if(r != ui::message_result::consumed) {
...
void state::render() { // called to render the frame may (and should) delay returning until the frame is rendered, including
	...
	if(ui_state.last_tooltip && ui_state.tooltip->is_visible()) {
		...
		if(ui_state.tooltip->base_data.size.y <=
			...
			ui_state.tooltip->base_data.position.x =
				std::clamp(int16_t(target_location.x + (ui_state.last_tooltip->base_data.size.x / 2) -
								   (ui_state.tooltip->base_data.size.x / 2)),
			...
			ui_state.tooltip->base_data.position.y =
				std::clamp(int16_t(target_location.y + (ui_state.last_tooltip->base_data.size.y / 2) -
								   (ui_state.tooltip->base_data.size.y / 2)),
	...
	if(bg_gfx_id) {
		...
		if(gfx_def.primary_texture_handle) {
			ogl::render_textured_rect(*this, ui::get_color_modification(false, false, false), 0.f, 0.f, float(x_size),
				float(y_size), ogl::get_texture_handle(*this, gfx_def.primary_texture_handle, gfx_def.is_partially_transparent()),
...
void state::on_create() {
	...
	ui_defs.gui[ui_state.defs_by_name.find("ledger_legend_entry")->second.definition].flags &=
		~ui::element_data::orientation_mask;
	...
	ui_defs.gui[ui_state.defs_by_name.find("trade_flow")->second.definition].data.window.flags |=
		ui::window_data::is_moveable_mask;
	...
	ui_defs.gui[ui_state.defs_by_name.find("invest_project_window")->second.definition].data.window.flags |=
		ui::window_data::is_moveable_mask;
	//if(!user_settings.guimode) {	TODO - this should only trigger if youre not on faithful mode, in Vic2, none of these windows are moveable
