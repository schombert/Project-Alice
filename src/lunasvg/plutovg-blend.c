...
static void plutovg_blend_subpx_texture(plutovg_canvas_t* canvas, const plutovg_subpx_texture_paint_t* texture, const plutovg_span_buffer_t* span_buffer) {
        //setup

        plutovg_state_t* state = canvas->state;
        plutovg_surface_t* surface = canvas->surface;
        plutovg_matrix_t final_matrix = texture->matrix;
        const int final_alpha = lroundf(state->opacity * texture->opacity * 256);
        plutovg_matrix_multiply(&final_matrix, &final_matrix, &state->matrix);
        if(!plutovg_matrix_invert(&final_matrix, &final_matrix))
                return;
        float large_x = ceilf(texture->real_x_extent);
        float small_x = floorf(texture->real_x_extent);
        float large_y = ceilf(texture->real_y_extent);
        float small_y = floorf(texture->real_y_extent);


        // perform blend

        composition_function_t func = composition_table[state->op];
        uint32_t buffer[BUFFER_SIZE];
        ...
        while(count--) {
                ...
                const int coverage = (spans->coverage * final_alpha) >> 8;
                int length = spans->len;
                int hoffset = 0;
                while(length) {
                        int l = plutovg_min(length, BUFFER_SIZE);
                        ...
                        uint32_t* b = buffer;
                        while(b < end) {
                                float px = (texture->matrix.c * cy + texture->matrix.a * (cx + hoffset) + texture->matrix.e);
                                float py = (texture->matrix.d * cy + texture->matrix.b * (cx + hoffset) + texture->matrix.f);
                                float x_tile = floorf(px / texture->real_x_extent);
                                float y_tile = floorf(py / texture->real_y_extent);

                                float real_x_end = roundf((x_tile + 1.0f) * texture->real_x_extent);
                                if(px >= real_x_end) {
                                        real_x_end = roundf((x_tile + 2.0f) * texture->real_x_extent);
                                        x_tile += 1;
                                }
                                float real_x_start = roundf(x_tile * texture->real_x_extent);
                                if(px < real_x_start) {
                                        real_x_end = real_x_start;
                                        real_x_start = roundf((x_tile - 1.0f) * texture->real_x_extent);
                                        x_tile -= 1;
                                }
                                bool is_large_x = (real_x_end - real_x_start) > small_x;

                                float real_y_end = roundf((y_tile + 1.0f) * texture->real_y_extent);
                                if(py >= real_y_end) {
                                        real_y_end = roundf((y_tile + 2.0f) * texture->real_y_extent);
                                        y_tile += 1;
                                }
                                float real_y_start = roundf(y_tile * texture->real_y_extent);
                                if(py < real_y_start) {
                                        real_y_end = real_y_start;
                                        real_y_start = roundf((y_tile - 1.0f) * texture->real_y_extent);
                                        y_tile -= 1;
                                }
                                bool is_large_y = (real_y_end - real_y_start) > small_y;

                                int tile_sub_x_position = (int)(px - real_x_start);
                                int tile_sub_y_position = (int)(py - real_y_start);
                                
                                plutovg_surface_t* chosen_tile = NULL;
                                if(is_large_x && is_large_y)
                                        chosen_tile = texture->surface_lx_ly;
                                else if(!is_large_x && is_large_y)
                                        chosen_tile = texture->surface_sx_ly;
                                else if(is_large_x && !is_large_y)
                                        chosen_tile = texture->surface_lx_sy;
                                else
                                        chosen_tile = texture->surface_sx_sy;

                                int y_offset = tile_sub_y_position * (chosen_tile->stride / 4);

                                assert(tile_sub_x_position >= 0 && tile_sub_x_position < chosen_tile->width);
                                assert(tile_sub_y_position >= 0 && tile_sub_y_position < chosen_tile->height);

                                const uint32_t* image_bits = (const uint32_t*)(chosen_tile->data);

                                *b = image_bits[y_offset + tile_sub_x_position];
                                ++b;
                                ++hoffset;
                        }
...
void plutovg_blend(plutovg_canvas_t* canvas, const plutovg_span_buffer_t* span_buffer)
{
    ...
    if(paint->type == PLUTOVG_PAINT_TYPE_COLOR) {
        ...
        plutovg_blend_texture(canvas, texture, span_buffer);
            ...
            plutovg_blend_subpx_texture(canvas, texture, span_buffer);
    }
}
