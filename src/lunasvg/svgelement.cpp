...
void SVGImageElement::render(SVGRenderState& state) const
{
    ...
    Rect srcRect(0.0f, 0.0f, float(m_image.width()), float(m_image.height()));
...
static Bitmap loadImageResource(const std::string& href, std::function<std::pair<void const*, int>(std::string_view)> const& f)
{
    if(href.compare(0, 5, "data:") == 0) {
        ...
        if(!f) {
                return plutovg_surface_load_from_image_base64(input.data(), int(input.length()));
        } else {
                auto file_result = f(input);
                return plutovg_surface_load_from_image_data(file_result.first, file_result.second);
        }
    }
...
void SVGImageElement::parseAttribute(PropertyID id, const std::string& value)
{
    if(id == PropertyID::Href) {
        m_image = loadImageResource(value, document()->file_loader);
    } else {
