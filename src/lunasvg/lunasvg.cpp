...
std::unique_ptr<Document> Document::loadFromFile(const std::string& filename, std::function<std::pair<const void*, int>(std::string_view)> const& f)
{
    std::ifstream fs;
    fs.open(filename);
    if(!fs.is_open())
    ...
    fs.close();
    return loadFromData(content, f);
}

std::unique_ptr<Document> Document::loadFromData(const std::string& string, std::function<std::pair<const void*, int>(std::string_view)> const& f)
{
    return loadFromData(string.data(), string.size(), f);
}

std::unique_ptr<Document> Document::loadFromData(const char* data, std::function<std::pair<const void*, int>(std::string_view)> const& f)
{
    return loadFromData(data, std::strlen(data), f);
}

std::unique_ptr<Document> Document::loadFromData(const char* data, size_t length, std::function<std::pair<const void*, int>(std::string_view)> const& f)
{
