...
{
    return plutovg_matrix_parse(&m_matrix, data, int(length));
}
...
bool Path::parse(const char* data, size_t length)
{
    ...
    return plutovg_path_parse(m_data, data, int(length));
}
...
FontFace::FontFace(const void* data, size_t length, plutovg_destroy_func_t destroy_func, void* closure)
    : m_face(plutovg_font_face_load_from_data(data, (unsigned int)(length), 0, destroy_func, closure))
{
}
...
{
    if(m_size > 0.f && !m_face.isNull())
        return plutovg_font_face_text_extents(m_face.get(), m_size, text.data(), int(text.length()), PLUTOVG_TEXT_ENCODING_UTF32, nullptr);
    return 0;
}
...
{
    plutovg_canvas_set_linear_gradient(m_canvas, x1, y1, x2, y2, static_cast<plutovg_spread_method_t>(spread), stops.data(), int(stops.size()), &transform.matrix());
}
...
{
    plutovg_canvas_set_radial_gradient(m_canvas, cx, cy, r, fx, fy, 0.f, static_cast<plutovg_spread_method_t>(spread), stops.data(), int(stops.size()), &transform.matrix());
}
...
void Canvas::strokePath(const Path& path, const StrokeData& strokeData, const Transform& transform)
{
    ...
    plutovg_canvas_set_dash_array(m_canvas, strokeData.dashArray().data(), int(strokeData.dashArray().size()));
    plutovg_canvas_set_operator(m_canvas, PLUTOVG_OPERATOR_SRC_OVER);
...
void Canvas::fillText(const std::u32string_view& text, const Font& font, const Point& origin, const Transform& transform)
{
    ...
    plutovg_canvas_fill_text(m_canvas, text.data(), int(text.length()), PLUTOVG_TEXT_ENCODING_UTF32, origin.x, origin.y);
}
...
void Canvas::strokeText(const std::u32string_view& text, float strokeWidth, const Font& font, const Point& origin, const Transform& transform)
{
    ...
    plutovg_canvas_set_operator(m_canvas, PLUTOVG_OPERATOR_SRC_OVER);
    plutovg_canvas_set_font(m_canvas, font.face().get(), font.size());
    plutovg_canvas_stroke_text(m_canvas, text.data(), int(text.length()), PLUTOVG_TEXT_ENCODING_UTF32, origin.x, origin.y);
}
