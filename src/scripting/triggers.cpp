...
template<typename A, typename B> [[nodiscard]] auto compare_values(uint16_t trigger_code, A value_a, B value_b) -> decltype(value_a == value_b) {
...
template<typename A, typename B> [[nodiscard]] auto compare_values_eq(uint16_t trigger_code, A value_a, B value_b) -> decltype(value_a == value_b) {
	switch(trigger_code & trigger::association_mask) {
	case trigger::association_eq:
		return value_a == value_b;
	case trigger::association_gt:
...
template<typename B> [[nodiscard]] auto compare_values_eq(uint16_t trigger_code, dcon::nation_fat_id value_a, B value_b) -> decltype(value_a.id == value_b) {
	switch(trigger_code & trigger::association_mask) {
	case trigger::association_eq:
...
template<typename T> struct gathered_s { using type = T; };

template<> struct gathered_s<ve::contiguous_tags<int32_t>> { using type = ve::tagged_vector<int32_t>; };

template<> struct gathered_s<ve::unaligned_contiguous_tags<int32_t>> { using type = ve::tagged_vector<int32_t>; };

template<> struct gathered_s<ve::partial_contiguous_tags<int32_t>> { using type = ve::tagged_vector<int32_t>; };

template<typename T> using gathered_t = typename gathered_s<T>::type;

template<typename return_type, typename primary_type, typename this_type, typename from_type> return_type CALLTYPE test_trigger_generic(uint16_t const* tval, sys::state& ws, primary_type primary_slot, this_type this_slot, from_type from_slot);

#define TRIGGER_FUNCTION(function_name) template<typename return_type, typename primary_type, typename this_type, typename from_type> return_type CALLTYPE function_name(uint16_t const* tval, sys::state& ws, primary_type primary_slot, this_type this_slot, from_type from_slot)

template<typename T> struct full_mask { };

template<> struct full_mask<bool> { static constexpr bool value = true; };
template<> struct full_mask<ve::vbitfield_type> { static constexpr ve::vbitfield_type value = ve::vbitfield_type{ve::vbitfield_type::storage(-1)}; };

template<typename T> struct empty_mask { };

template<> struct empty_mask<bool> { static constexpr bool value = false; };
template<> struct empty_mask<ve::vbitfield_type> { static constexpr ve::vbitfield_type value = ve::vbitfield_type{ve::vbitfield_type::storage(0)}; };
...
TRIGGER_FUNCTION(tf_generic_scope) { return apply_subtriggers<return_type, primary_type, this_type, from_type>(tval, ws, primary_slot, this_slot, from_slot); }
...
template<typename F> auto make_false_accumulator(F&& f) -> false_accumulator<F> { return false_accumulator<F>(std::forward<F>(f)); }

template<typename TAG, typename F> auto make_value_accumulator(F&& f) -> value_accumulator<TAG, F> { return value_accumulator<TAG, F>(std::forward<F>(f)); }

auto existence_accumulator(sys::state& ws, uint16_t const* tval, int32_t this_slot, int32_t from_slot) {
	return make_true_accumulator([&ws, tval, this_slot, from_slot](ve::tagged_vector<int32_t> v) { return apply_subtriggers<ve::mask_vector, ve::tagged_vector<int32_t>, int32_t, int32_t>(tval, ws, v, this_slot, from_slot); });
}

auto universal_accumulator(sys::state& ws, uint16_t const* tval, int32_t this_slot, int32_t from_slot) {
	return make_false_accumulator([&ws, tval, this_slot, from_slot](ve::tagged_vector<int32_t> v) { return apply_subtriggers<ve::mask_vector, ve::tagged_vector<int32_t>, int32_t, int32_t>(tval, ws, v, this_slot, from_slot); });
}
...
TRIGGER_FUNCTION(tf_this_scope) { return apply_subtriggers<return_type, this_type, this_type, from_type>(tval, ws, this_slot, this_slot, from_slot); }
TRIGGER_FUNCTION(tf_from_scope) { return apply_subtriggers<return_type, from_type, this_type, from_type>(tval, ws, from_slot, this_slot, from_slot); }
...
TRIGGER_FUNCTION(tf_crisis_state_scope) { return apply_subtriggers<return_type, int32_t, this_type, from_type>(tval, ws, to_generic(ws.crisis_state), this_slot, from_slot); }
...
TRIGGER_FUNCTION(tf_country_scope_nation) { return apply_subtriggers<return_type, primary_type, this_type, from_type>(tval, ws, primary_slot, this_slot, from_slot); }
...
TRIGGER_FUNCTION(tf_life_rating_province) { return compare_values(tval[0], ws.world.province_get_life_rating(to_prov(primary_slot)), trigger::payload(tval[1]).signed_value); }
...
auto empty_province_from_state_accumulator(sys::state const& ws, dcon::state_instance_id sid) {
	return make_true_accumulator([&ws, vector_sid = ve::tagged_vector<dcon::state_instance_id>(sid), sea_index = ws.province_definitions.first_sea_province.index()](ve::tagged_vector<int32_t> v) {
		auto owners = ws.world.province_get_nation_from_province_ownership(to_prov(v));
...
TRIGGER_FUNCTION(tf_state_id_province) {
	...
	return compare_values_eq(tval[0], ws.world.abstract_state_membership_get_state(ws.world.province_get_abstract_state_membership(to_prov(primary_slot))), ws.world.abstract_state_membership_get_state(ws.world.province_get_abstract_state_membership(pid)));
}
TRIGGER_FUNCTION(tf_state_id_state) {
	auto pid = trigger::payload(tval[1]).prov_id;
	return compare_values_eq(tval[0], ws.world.state_instance_get_definition(to_state(primary_slot)), ws.world.abstract_state_membership_get_state(ws.world.province_get_abstract_state_membership(pid)));
}
...
TRIGGER_FUNCTION(tf_unemployment_nation) {
	...
	return compare_values(tval[0], ve::select(total_employable > 0.0f, 1.0f - (total_employed / total_employable), 0.0f), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_unemployment_pop) {
	...
	return compare_values(tval[0], ve::select(ws.world.pop_type_get_has_unemployment(ptype), 1.0f - (employed / total_pop), 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_is_slave_nation) { return compare_to_true(tval[0], (ws.world.nation_get_combined_issue_rules(to_nation(primary_slot)) & issue_rule::slavery_allowed) == issue_rule::slavery_allowed); }
TRIGGER_FUNCTION(tf_is_slave_state) { return compare_to_true(tval[0], ws.world.province_get_is_slave(ws.world.state_instance_get_capital(to_state(primary_slot)))); }
TRIGGER_FUNCTION(tf_is_slave_province) { return compare_to_true(tval[0], ws.world.province_get_is_slave(to_prov(primary_slot))); }
TRIGGER_FUNCTION(tf_is_slave_pop) { return compare_values_eq(tval[0], ws.world.pop_get_poptype(to_pop(primary_slot)), ws.culture_definitions.slaves); }
TRIGGER_FUNCTION(tf_is_independant) { return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), dcon::nation_id()); }
...
TRIGGER_FUNCTION(tf_capital) { return compare_values_eq(tval[0], ws.world.nation_get_capital(to_nation(primary_slot)), payload(tval[1]).prov_id); }
TRIGGER_FUNCTION(tf_tech_school) { return compare_values_eq(tval[0], ws.world.nation_get_tech_school(to_nation(primary_slot)), trigger::payload(tval[1]).mod_id); }
TRIGGER_FUNCTION(tf_primary_culture) { return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(to_nation(primary_slot)), trigger::payload(tval[1]).cul_id); }
...
TRIGGER_FUNCTION(tf_culture_pop) { return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), trigger::payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_culture_state) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), trigger::payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_culture_province) { return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), trigger::payload(tval[1]).cul_id); }
...
TRIGGER_FUNCTION(tf_culture_pop_reb) { return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), ws.world.rebel_faction_get_primary_culture(to_rebel(from_slot))); }
TRIGGER_FUNCTION(tf_culture_state_reb) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), ws.world.rebel_faction_get_primary_culture(to_rebel(from_slot))); }
TRIGGER_FUNCTION(tf_culture_province_reb) { return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), ws.world.rebel_faction_get_primary_culture(to_rebel(from_slot))); }
TRIGGER_FUNCTION(tf_culture_nation_reb) { return compare_to_true(tval[0], nations::nation_accepts_culture(ws, to_nation(primary_slot), ws.world.rebel_faction_get_primary_culture(to_rebel(from_slot)))); }
...
TRIGGER_FUNCTION(tf_culture_group_nation) { return compare_values_eq(tval[0], nations::primary_culture_group(ws, to_nation(primary_slot)), trigger::payload(tval[1]).culgrp_id); }
...
TRIGGER_FUNCTION(tf_culture_group_nation_this_nation) { return compare_values_eq(tval[0], nations::primary_culture_group(ws, to_nation(primary_slot)), nations::primary_culture_group(ws, to_nation(this_slot))); }
...
TRIGGER_FUNCTION(tf_culture_group_nation_from_nation) { return compare_values_eq(tval[0], nations::primary_culture_group(ws, to_nation(primary_slot)), nations::primary_culture_group(ws, to_nation(from_slot))); }
...
TRIGGER_FUNCTION(tf_culture_group_nation_this_province) {
	...
	return compare_values_eq(tval[0], nations::primary_culture_group(ws, to_nation(primary_slot)), nations::primary_culture_group(ws, owners));
}
...
TRIGGER_FUNCTION(tf_culture_group_nation_this_pop) {
	...
	return compare_values_eq(tval[0], nations::primary_culture_group(ws, to_nation(primary_slot)), nations::primary_culture_group(ws, owner));
}
...
TRIGGER_FUNCTION(tf_religion) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), trigger::payload(tval[1]).rel_id); }
TRIGGER_FUNCTION(tf_religion_reb) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), ws.world.rebel_faction_get_religion(to_rebel(from_slot))); }
TRIGGER_FUNCTION(tf_religion_from_nation) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), ws.world.nation_get_religion(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_religion_this_nation) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), ws.world.nation_get_religion(to_nation(this_slot))); }
...
TRIGGER_FUNCTION(tf_terrain_province) { return compare_values_eq(tval[0], ws.world.province_get_terrain(to_prov(primary_slot)), trigger::payload(tval[1]).mod_id); }
...
TRIGGER_FUNCTION(tf_trade_goods) { return compare_values_eq(tval[0], ws.world.province_get_rgo(to_prov(primary_slot)), trigger::payload(tval[1]).com_id); }

TRIGGER_FUNCTION(tf_is_secondary_power_nation) { return compare_to_true(tval[0], ws.world.nation_get_rank(to_nation(primary_slot)) <= uint32_t(ws.defines.colonial_rank)); }
...
TRIGGER_FUNCTION(tf_can_build_factory_nation) { return compare_values_eq(tval[0], ws.world.nation_get_combined_issue_rules(to_nation(primary_slot)) & issue_rule::pop_build_factory, issue_rule::pop_build_factory); }
TRIGGER_FUNCTION(tf_can_build_factory_province) {
	...
	return compare_values_eq(tval[0], ws.world.nation_get_combined_issue_rules(p_owner) & issue_rule::pop_build_factory, issue_rule::pop_build_factory);
}
...
TRIGGER_FUNCTION(tf_war_exhaustion_nation) { return compare_values(tval[0], ws.world.nation_get_war_exhaustion(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_blockade) { return compare_values(tval[0], nations::central_blockaded_fraction(ws, to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_num_of_revolts) { return compare_values(tval[0], ws.world.nation_get_rebel_controlled_count(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_revolt_percentage) { return compare_values(tval[0], nations::central_reb_controlled_fraction(ws, to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_num_of_cities_int) { return compare_values(tval[0], ws.world.nation_get_owned_province_count(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_num_of_cities_from_nation) { return compare_values(tval[0], ws.world.nation_get_owned_province_count(to_nation(primary_slot)), ws.world.nation_get_owned_province_count(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_num_of_cities_this_nation) { return compare_values(tval[0], ws.world.nation_get_owned_province_count(to_nation(primary_slot)), ws.world.nation_get_owned_province_count(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_num_of_cities_this_state) {
	auto owner = ws.world.state_instance_get_nation_from_state_ownership(to_state(this_slot));
	return compare_values(tval[0], ws.world.nation_get_owned_province_count(to_nation(primary_slot)), ws.world.nation_get_owned_province_count(owner));
}
...
TRIGGER_FUNCTION(tf_num_of_ports) { return compare_values(tval[0], ws.world.nation_get_central_ports(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_num_of_allies) { return compare_values(tval[0], ws.world.nation_get_allies_count(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_num_of_vassals) { return compare_values(tval[0], ws.world.nation_get_vassals_count(to_nation(primary_slot)), tval[1]); }
...
TRIGGER_FUNCTION(tf_owned_by_this_nation) { return compare_values_eq(tval[0], to_nation(this_slot), ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot))); }
TRIGGER_FUNCTION(tf_owned_by_from_nation) { return compare_values_eq(tval[0], to_nation(from_slot), ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot))); }
TRIGGER_FUNCTION(tf_owned_by_this_province) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_ownership(to_prov(this_slot)), ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot))); }
...
TRIGGER_FUNCTION(tf_exists_bool) { return compare_to_true(tval[0], ws.world.nation_get_owned_province_count(to_nation(primary_slot)) != 0); }
TRIGGER_FUNCTION(tf_exists_tag) {
	auto holders = ws.world.national_identity_get_nation_from_identity_holder(trigger::payload(tval[1]).tag_id);
	return compare_to_true(tval[0], ws.world.nation_get_owned_province_count(holders) != 0);
}
TRIGGER_FUNCTION(tf_has_country_flag) { return compare_to_true(tval[0], ws.world.nation_get_flag_variables(to_nation(primary_slot), payload(tval[1]).natf_id)); }
...
TRIGGER_FUNCTION(tf_continent_province) { return compare_values_eq(tval[0], ws.world.province_get_continent(to_prov(primary_slot)), trigger::payload(tval[1]).mod_id); }
...
TRIGGER_FUNCTION(tf_continent_nation_this) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(this_slot)))); }
TRIGGER_FUNCTION(tf_continent_state_this) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.state_instance_get_capital(to_state(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(this_slot)))); }
TRIGGER_FUNCTION(tf_continent_province_this) { return compare_values_eq(tval[0], ws.world.province_get_continent(to_prov(primary_slot)), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(this_slot)))); }
TRIGGER_FUNCTION(tf_continent_pop_this) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.pop_get_province_from_pop_location(to_pop(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(this_slot)))); }
TRIGGER_FUNCTION(tf_continent_nation_from) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(from_slot)))); }
TRIGGER_FUNCTION(tf_continent_state_from) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.state_instance_get_capital(to_state(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(from_slot)))); }
TRIGGER_FUNCTION(tf_continent_province_from) { return compare_values_eq(tval[0], ws.world.province_get_continent(to_prov(primary_slot)), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(from_slot)))); }
TRIGGER_FUNCTION(tf_continent_pop_from) { return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.pop_get_province_from_pop_location(to_pop(primary_slot))), ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(from_slot)))); }
TRIGGER_FUNCTION(tf_casus_belli_tag) {
	...
	auto result = ve::apply([&ws, tag_holder](dcon::nation_id n) { return military::can_use_cb_against(ws, n, tag_holder); }, to_nation(primary_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_casus_belli_from) {
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id target) { return military::can_use_cb_against(ws, n, target); }, to_nation(primary_slot), to_nation(from_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_casus_belli_this_nation) {
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id target) { return military::can_use_cb_against(ws, n, target); }, to_nation(primary_slot), to_nation(this_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_casus_belli_this_state) {
	auto owner = ws.world.state_instance_get_nation_from_state_ownership(to_state(this_slot));
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id target) { return military::can_use_cb_against(ws, n, target); }, to_nation(primary_slot), owner);
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_military_access_tag) {
	...
	auto result = ve::apply([&ws, holder](dcon::nation_id n) { return ws.world.unilateral_relationship_get_military_access(ws.world.get_unilateral_relationship_by_unilateral_pair(holder, n)); }, to_nation(primary_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_military_access_from) {
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id t) { return ws.world.unilateral_relationship_get_military_access(ws.world.get_unilateral_relationship_by_unilateral_pair(t, n)); }, to_nation(primary_slot), to_nation(from_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_military_access_this_nation) {
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id t) { return ws.world.unilateral_relationship_get_military_access(ws.world.get_unilateral_relationship_by_unilateral_pair(t, n)); }, to_nation(primary_slot), to_nation(this_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_military_access_this_state) {
	auto owner = ws.world.state_instance_get_nation_from_state_ownership(to_state(this_slot));
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id t) { return ws.world.unilateral_relationship_get_military_access(ws.world.get_unilateral_relationship_by_unilateral_pair(t, n)); }, to_nation(primary_slot), owner);
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_prestige_value) { return compare_values(tval[0], ws.world.nation_get_prestige(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_prestige_from) { return compare_values(tval[0], ws.world.nation_get_prestige(to_nation(primary_slot)), ws.world.nation_get_prestige(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_prestige_this_nation) { return compare_values(tval[0], ws.world.nation_get_prestige(to_nation(primary_slot)), ws.world.nation_get_prestige(to_nation(this_slot))); }
...
TRIGGER_FUNCTION(tf_badboy) { return compare_values(tval[0], ws.world.nation_get_infamy(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_has_building_fort) { return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) != 0); }
TRIGGER_FUNCTION(tf_has_building_railroad) { return compare_to_true(tval[0], ws.world.province_get_railroad_level(to_prov(primary_slot)) != 0); }
TRIGGER_FUNCTION(tf_has_building_naval_base) { return compare_to_true(tval[0], ws.world.province_get_naval_base_level(to_prov(primary_slot)) != 0); }
...
TRIGGER_FUNCTION(tf_has_building_state) {
	auto result = ve::apply([&ws, f = payload(tval[1]).fac_id](dcon::state_instance_id s) { return economy::has_building(ws, s, f); }, to_state(primary_slot));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_empty) { return compare_to_true(tval[0], ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot)) == dcon::nation_id()); }
...
TRIGGER_FUNCTION(tf_region) { return compare_values_eq(tval[0], ws.world.province_get_state_from_abstract_state_membership(to_prov(primary_slot)), trigger::payload(tval[1]).state_id); }
TRIGGER_FUNCTION(tf_tag_tag) { return compare_values_eq(tval[0], ws.world.nation_get_identity_from_identity_holder(to_nation(primary_slot)), trigger::payload(tval[1]).tag_id); }
TRIGGER_FUNCTION(tf_tag_this_nation) { return compare_values_eq(tval[0], to_nation(primary_slot), to_nation(this_slot)); }
TRIGGER_FUNCTION(tf_tag_this_province) { return compare_values_eq(tval[0], to_nation(primary_slot), ws.world.province_get_nation_from_province_ownership(to_prov(this_slot))); }
TRIGGER_FUNCTION(tf_tag_from_nation) { return compare_values_eq(tval[0], to_nation(primary_slot), to_nation(from_slot)); }
TRIGGER_FUNCTION(tf_tag_from_province) { return compare_values_eq(tval[0], to_nation(primary_slot), ws.world.province_get_nation_from_province_ownership(to_prov(from_slot))); }
...
TRIGGER_FUNCTION(tf_war_with_tag) {
	auto holder = ws.world.national_identity_get_nation_from_identity_holder(trigger::payload(tval[1]).tag_id);
	auto result = ve::apply([&ws, holder](dcon::nation_id a) { return military::are_at_war(ws, holder, a); }, to_nation(primary_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_war_with_from) {
	auto result = ve::apply([&ws](dcon::nation_id a, dcon::nation_id b) { return military::are_at_war(ws, a, b); }, to_nation(primary_slot), to_nation(from_slot));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_war_with_this_nation) {
	auto result = ve::apply([&ws](dcon::nation_id a, dcon::nation_id b) { return military::are_at_war(ws, a, b); }, to_nation(primary_slot), to_nation(this_slot));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_unit_in_battle) { return compare_to_true(tval[0], military::battle_is_ongoing_in_province(ws, to_prov(primary_slot))); }
...
TRIGGER_FUNCTION(tf_money) { return compare_values(tval[0], ws.world.nation_get_stockpiles(to_nation(primary_slot), economy::money), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_lost_national) { return compare_values(tval[0], 1.0f - ws.world.nation_get_revanchism(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_is_vassal) { return compare_to_true(tval[0], ws.world.nation_get_overlord_as_subject(to_nation(primary_slot)) != dcon::overlord_id()); }
...
TRIGGER_FUNCTION(tf_is_ideology_enabled) { return compare_to_true(tval[0], ws.world.ideology_get_enabled(trigger::payload(tval[1]).ideo_id)); }
...
TRIGGER_FUNCTION(tf_political_reform_want_pop) { return compare_values(tval[0], ws.world.pop_get_political_reform_desire(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_social_reform_want_pop) { return compare_values(tval[0], ws.world.pop_get_social_reform_desire(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_plurality) { return compare_values(tval[0], ws.world.nation_get_plurality(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_corruption) { return compare_values(tval[0], nations::central_has_crime_fraction(ws, to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_is_state_religion_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot));
	return compare_values_eq(tval[0], ws.world.nation_get_religion(owner), ws.world.province_get_dominant_religion(to_prov(primary_slot)));
}
TRIGGER_FUNCTION(tf_is_state_religion_state) {
	auto owner = ws.world.state_instance_get_nation_from_state_ownership(to_state(primary_slot));
	return compare_values_eq(tval[0], ws.world.nation_get_religion(owner), ws.world.state_instance_get_dominant_religion(to_state(primary_slot)));
}
...
TRIGGER_FUNCTION(tf_is_primary_culture_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot));
	return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(owner), ws.world.province_get_dominant_culture(to_prov(primary_slot)));
}
TRIGGER_FUNCTION(tf_is_primary_culture_state) {
	auto owner = ws.world.state_instance_get_nation_from_state_ownership(to_state(primary_slot));
	return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(owner), ws.world.state_instance_get_dominant_culture(to_state(primary_slot)));
}
TRIGGER_FUNCTION(tf_is_primary_culture_nation_this_pop) { return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(to_nation(primary_slot)), ws.world.pop_get_culture(to_pop(this_slot))); }
TRIGGER_FUNCTION(tf_is_primary_culture_nation_this_nation) { return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(to_nation(primary_slot)), ws.world.nation_get_primary_culture(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_is_primary_culture_nation_this_state) {
	...
	return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(to_nation(primary_slot)), ws.world.nation_get_primary_culture(this_owner));
}
...
TRIGGER_FUNCTION(tf_is_primary_culture_state_this_nation) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), ws.world.nation_get_primary_culture(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_is_primary_culture_state_this_pop) {
	auto this_owner = nations::owner_of_pop(ws, to_pop(this_slot));
	return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), ws.world.nation_get_primary_culture(this_owner));
}
...
TRIGGER_FUNCTION(tf_is_primary_culture_province_this_nation) { return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), ws.world.nation_get_primary_culture(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_is_primary_culture_province_this_pop) {
	auto this_owner = nations::owner_of_pop(ws, to_pop(this_slot));
	return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), ws.world.nation_get_primary_culture(this_owner));
}
...
TRIGGER_FUNCTION(tf_is_primary_culture_pop_this_nation) { return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), ws.world.nation_get_primary_culture(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_is_primary_culture_pop_this_pop) {
	auto this_owner = nations::owner_of_pop(ws, to_pop(this_slot));
	return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), ws.world.nation_get_primary_culture(this_owner));
}
...
TRIGGER_FUNCTION(tf_in_sphere_tag) { return compare_values_eq(tval[0], ws.world.nation_get_in_sphere_of(to_nation(primary_slot)), ws.world.national_identity_get_nation_from_identity_holder(payload(tval[1]).tag_id)); }
TRIGGER_FUNCTION(tf_in_sphere_from) { return compare_values_eq(tval[0], ws.world.nation_get_in_sphere_of(to_nation(primary_slot)), to_nation(from_slot)); }
TRIGGER_FUNCTION(tf_in_sphere_this_nation) { return compare_values_eq(tval[0], ws.world.nation_get_in_sphere_of(to_nation(primary_slot)), to_nation(this_slot)); }
...
TRIGGER_FUNCTION(tf_produces_province) { return compare_to_true(tval[0], (ws.world.province_get_rgo(to_prov(primary_slot)) == payload(tval[1]).com_id) || (ws.world.province_get_artisan_production(to_prov(primary_slot)) == payload(tval[1]).com_id)); }
...
TRIGGER_FUNCTION(tf_produces_pop) {
	...
	return compare_to_true(tval[0], (ws.world.pop_get_poptype(to_pop(primary_slot)) == ws.culture_definitions.artisans) && (ws.world.province_get_artisan_production(pop_location) == good));
}
...
TRIGGER_FUNCTION(tf_nationalism) { return compare_values(tval[0], ws.world.province_get_nationalism(to_prov(primary_slot)), float(tval[1])); }
TRIGGER_FUNCTION(tf_is_overseas) { return compare_to_true(tval[0], province::is_overseas(ws, to_prov(primary_slot))); }
TRIGGER_FUNCTION(tf_controlled_by_rebels) { return compare_to_true(tval[0], ws.world.province_get_rebel_faction_from_province_rebel_control(to_prov(primary_slot)) != dcon::rebel_faction_id()); }
TRIGGER_FUNCTION(tf_controlled_by_tag) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), ws.world.national_identity_get_nation_from_identity_holder(payload(tval[1]).tag_id)); }
TRIGGER_FUNCTION(tf_controlled_by_from) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), to_nation(from_slot)); }
TRIGGER_FUNCTION(tf_controlled_by_this_nation) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), to_nation(this_slot)); }
TRIGGER_FUNCTION(tf_controlled_by_this_province) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), ws.world.province_get_nation_from_province_ownership(to_prov(this_slot))); }
TRIGGER_FUNCTION(tf_controlled_by_this_state) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), ws.world.state_instance_get_nation_from_state_ownership(to_state(this_slot))); }
TRIGGER_FUNCTION(tf_controlled_by_this_pop) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_control(to_prov(primary_slot)), nations::owner_of_pop(ws, to_pop(this_slot))); }
TRIGGER_FUNCTION(tf_controlled_by_owner) { return compare_values_eq(tval[0], ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot)), ws.world.province_get_nation_from_province_control(to_prov(primary_slot))); }
TRIGGER_FUNCTION(tf_controlled_by_reb) { return compare_values_eq(tval[0], ws.world.province_get_rebel_faction_from_province_rebel_control(to_prov(primary_slot)), to_rebel(from_slot)); }
TRIGGER_FUNCTION(tf_is_canal_enabled) { return compare_to_true(tval[0], (ws.world.province_adjacency_get_type(ws.province_definitions.canals[tval[1] - 1]) & province::border::impassible_bit) == 0); }
...
TRIGGER_FUNCTION(tf_total_pops_nation) { return compare_values(tval[0], ws.world.nation_get_demographics(to_nation(primary_slot), demographics::total), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_total_pops_state) { return compare_values(tval[0], ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::total), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_total_pops_province) { return compare_values(tval[0], ws.world.province_get_demographics(to_prov(primary_slot), demographics::total), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_is_possible_vassal) {
	...
	auto result = ve::apply([&ws, tag](dcon::nation_id n) { return nations::can_release_as_vassal(ws, n, tag); }, to_nation(primary_slot));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_vassal_of_tag) {
	...
	return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), tag_holder);
}
TRIGGER_FUNCTION(tf_vassal_of_from) { return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), to_nation(from_slot)); }
TRIGGER_FUNCTION(tf_vassal_of_this_nation) { return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), to_nation(this_slot)); }
TRIGGER_FUNCTION(tf_vassal_of_this_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(this_slot));
	return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), owner);
}
...
TRIGGER_FUNCTION(tf_substate_of_tag) {
	auto tag_holder = ws.world.national_identity_get_nation_from_identity_holder(payload(tval[1]).tag_id);
	return compare_to_true(tval[0], ws.world.nation_get_is_substate(to_nation(primary_slot)) && (ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))) == tag_holder));
}
TRIGGER_FUNCTION(tf_substate_of_from) { return compare_to_true(tval[0], ws.world.nation_get_is_substate(to_nation(primary_slot)) && (ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))) == to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_substate_of_this_nation) { return compare_to_true(tval[0], ws.world.nation_get_is_substate(to_nation(primary_slot)) && (ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))) == to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_substate_of_this_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(this_slot));
	return compare_to_true(tval[0], ws.world.nation_get_is_substate(to_nation(primary_slot)) && (ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))) == owner));
}
...
TRIGGER_FUNCTION(tf_mobilisation_size) { return compare_values(tval[0], ws.world.nation_get_modifier_values(to_nation(primary_slot), sys::national_mod_offsets::mobilization_size), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_crime_higher_than_education_nation) { return compare_to_true(tval[0], ws.world.nation_get_administrative_spending(to_nation(primary_slot)) >= ws.world.nation_get_education_spending(to_nation(primary_slot))); }
...
TRIGGER_FUNCTION(tf_is_colonial_province) { return compare_to_true(tval[0], ws.world.province_get_is_colonial(to_prov(primary_slot))); }
...
TRIGGER_FUNCTION(tf_in_default_tag) {
	auto holder = ws.world.national_identity_get_nation_from_identity_holder(payload(tval[1]).tag_id);
	return compare_to_true(tval[0], ve::apply([&ws, holder](dcon::nation_id n) { return economy::is_bankrupt_debtor_to(ws, n, holder); }, to_nation(primary_slot)));
}
TRIGGER_FUNCTION(tf_in_default_from) {
	return compare_to_true(tval[0], ve::apply([&ws](dcon::nation_id n, dcon::nation_id h) { return economy::is_bankrupt_debtor_to(ws, n, h); }, to_nation(primary_slot), to_nation(from_slot)));
}
TRIGGER_FUNCTION(tf_in_default_this_nation) {
	return compare_to_true(tval[0], ve::apply([&ws](dcon::nation_id n, dcon::nation_id h) { return economy::is_bankrupt_debtor_to(ws, n, h); }, to_nation(primary_slot), to_nation(this_slot)));
}
TRIGGER_FUNCTION(tf_in_default_this_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(this_slot));
	return compare_to_true(tval[0], ve::apply([&ws](dcon::nation_id n, dcon::nation_id h) { return economy::is_bankrupt_debtor_to(ws, n, h); }, to_nation(primary_slot), owner));
}
...
TRIGGER_FUNCTION(tf_total_num_of_ports) { return compare_values(tval[0], ws.world.nation_get_total_ports(to_nation(primary_slot)), tval[1]); }
...
TRIGGER_FUNCTION(tf_has_global_flag) { return compare_to_true(tval[0], ws.national_definitions.is_global_flag_variable_set(payload(tval[2]).glob_id)); }
TRIGGER_FUNCTION(tf_is_capital) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot));
	return compare_values_eq(tval[0], ws.world.nation_get_capital(owner), to_prov(primary_slot));
}
TRIGGER_FUNCTION(tf_nationalvalue_nation) { return compare_values_eq(tval[0], ws.world.nation_get_national_value(to_nation(primary_slot)), payload(tval[1]).mod_id); }
...
TRIGGER_FUNCTION(tf_industrial_score_value) { return compare_values(tval[0], ws.world.nation_get_industrial_score(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_industrial_score_from_nation) { return compare_values(tval[0], ws.world.nation_get_industrial_score(to_nation(primary_slot)), ws.world.nation_get_industrial_score(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_industrial_score_this_nation) { return compare_values(tval[0], ws.world.nation_get_industrial_score(to_nation(primary_slot)), ws.world.nation_get_industrial_score(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_industrial_score_this_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(this_slot));
	return compare_values(tval[0], ws.world.nation_get_industrial_score(to_nation(primary_slot)), ws.world.nation_get_industrial_score(owner));
}
...
TRIGGER_FUNCTION(tf_military_score_value) { return compare_values(tval[0], ws.world.nation_get_military_score(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_military_score_from_nation) { return compare_values(tval[0], ws.world.nation_get_military_score(to_nation(primary_slot)), ws.world.nation_get_military_score(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_military_score_this_nation) { return compare_values(tval[0], ws.world.nation_get_military_score(to_nation(primary_slot)), ws.world.nation_get_military_score(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_military_score_this_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(this_slot));
	return compare_values(tval[0], ws.world.nation_get_military_score(to_nation(primary_slot)), ws.world.nation_get_military_score(owner));
}
...
TRIGGER_FUNCTION(tf_civilized_nation) { return compare_to_true(tval[0], ws.world.nation_get_is_civilized(to_nation(primary_slot))); }
...
TRIGGER_FUNCTION(tf_national_provinces_occupied) { return compare_values(tval[0], nations::occupied_provinces_fraction(ws, to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_is_greater_power_nation) { return compare_to_true(tval[0], ws.world.nation_get_rank(to_nation(primary_slot)) <= uint16_t(ws.defines.great_nations_count)); }
...
TRIGGER_FUNCTION(tf_rich_tax) { return compare_values(tval[0], ws.world.nation_get_rich_tax(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_middle_tax) { return compare_values(tval[0], ws.world.nation_get_middle_tax(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_poor_tax) { return compare_values(tval[0], ws.world.nation_get_poor_tax(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_social_spending_nation) { return compare_values(tval[0], ws.world.nation_get_social_spending(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_social_spending_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(primary_slot));
	return compare_values(tval[0], ws.world.nation_get_social_spending(owner), payload(tval[1]).signed_value);
}
TRIGGER_FUNCTION(tf_social_spending_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(primary_slot));
	return compare_values(tval[0], ws.world.nation_get_social_spending(owner), payload(tval[1]).signed_value);
}
TRIGGER_FUNCTION(tf_military_spending_nation) { return compare_values(tval[0], ws.world.nation_get_military_spending(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_military_spending_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(primary_slot));
	return compare_values(tval[0], ws.world.nation_get_military_spending(owner), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_administration_spending_nation) { return compare_values(tval[0], ws.world.nation_get_administrative_spending(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_administration_spending_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(primary_slot));
	return compare_values(tval[0], ws.world.nation_get_administrative_spending(owner), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_education_spending_nation) { return compare_values(tval[0], ws.world.nation_get_education_spending(to_nation(primary_slot)), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_education_spending_pop) {
	auto owner = nations::owner_of_pop(ws, to_pop(primary_slot));
	return compare_values(tval[0], ws.world.nation_get_education_spending(owner), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_colonial_nation) { return compare_to_true(tval[0], ws.world.nation_get_is_colonial_nation(to_nation(primary_slot))); }
TRIGGER_FUNCTION(tf_pop_majority_religion_nation) { return compare_values_eq(tval[0], ws.world.nation_get_dominant_religion(to_nation(primary_slot)), payload(tval[1]).rel_id); }
TRIGGER_FUNCTION(tf_pop_majority_religion_state) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_religion(to_state(primary_slot)), payload(tval[1]).rel_id); }
TRIGGER_FUNCTION(tf_pop_majority_religion_province) { return compare_values_eq(tval[0], ws.world.province_get_dominant_religion(to_prov(primary_slot)), payload(tval[1]).rel_id); }
TRIGGER_FUNCTION(tf_pop_majority_culture_nation) { return compare_values_eq(tval[0], ws.world.nation_get_dominant_culture(to_nation(primary_slot)), payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_pop_majority_culture_state) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_pop_majority_culture_province) { return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_pop_majority_issue_nation) { return compare_values_eq(tval[0], ws.world.nation_get_dominant_issue_option(to_nation(primary_slot)), payload(tval[1]).opt_id); }
TRIGGER_FUNCTION(tf_pop_majority_issue_state) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_issue_option(to_state(primary_slot)), payload(tval[1]).opt_id); }
TRIGGER_FUNCTION(tf_pop_majority_issue_province) { return compare_values_eq(tval[0], ws.world.province_get_dominant_issue_option(to_prov(primary_slot)), payload(tval[1]).opt_id); }
TRIGGER_FUNCTION(tf_pop_majority_issue_pop) { return compare_values_eq(tval[0], ws.world.pop_get_dominant_issue_option(to_pop(primary_slot)), payload(tval[1]).opt_id); }
TRIGGER_FUNCTION(tf_pop_majority_ideology_nation) { return compare_values_eq(tval[0], ws.world.nation_get_dominant_ideology(to_nation(primary_slot)), payload(tval[1]).ideo_id); }
TRIGGER_FUNCTION(tf_pop_majority_ideology_state) { return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_ideology(to_state(primary_slot)), payload(tval[1]).ideo_id); }
TRIGGER_FUNCTION(tf_pop_majority_ideology_province) { return compare_values_eq(tval[0], ws.world.province_get_dominant_ideology(to_prov(primary_slot)), payload(tval[1]).ideo_id); }
TRIGGER_FUNCTION(tf_pop_majority_ideology_pop) { return compare_values_eq(tval[0], ws.world.pop_get_dominant_ideology(to_pop(primary_slot)), payload(tval[1]).ideo_id); }
...
TRIGGER_FUNCTION(tf_poor_strata_militancy_pop) {
	...
	return compare_values(tval[0], ve::select(is_poor, ws.world.pop_get_militancy(to_pop(primary_slot)), 0.0f), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_middle_strata_militancy_pop) {
	...
	return compare_values(tval[0], ve::select(is_middle, ws.world.pop_get_militancy(to_pop(primary_slot)), 0.0f), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_rich_strata_militancy_pop) {
	...
	return compare_values(tval[0], ve::select(is_rich, ws.world.pop_get_militancy(to_pop(primary_slot)), 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_rich_tax_above_poor) { return compare_to_true(tval[0], ws.world.nation_get_rich_tax(to_nation(primary_slot)) > ws.world.nation_get_poor_tax(to_nation(primary_slot))); }
...
TRIGGER_FUNCTION(tf_revanchism_nation) { return compare_values(tval[0], ws.world.nation_get_revanchism(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_has_crime) { return compare_to_true(tval[0], ws.world.province_get_crime(to_prov(primary_slot)) != dcon::crime_id()); }
TRIGGER_FUNCTION(tf_num_of_substates) { return compare_values(tval[0], ws.world.nation_get_substates_count(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_num_of_vassals_no_substates) { return compare_values(tval[0], ws.world.nation_get_vassals_count(to_nation(primary_slot)) - ws.world.nation_get_substates_count(to_nation(primary_slot)), tval[1]); }
TRIGGER_FUNCTION(tf_brigades_compare_this) {
	...
	auto this_brigades = ve::apply(
		...
		to_nation(this_slot));
	return compare_values(tval[0], ve::select(this_brigades != 0.0f, main_brigades / this_brigades, 1'000'000.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_brigades_compare_from) {
	...
	auto from_brigades = ve::apply(
		...
		to_nation(from_slot));
	return compare_values(tval[0], ve::select(from_brigades != 0.0f, main_brigades / from_brigades, 1'000'000.0f), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_constructing_cb_from) { return compare_values_eq(tval[0], ws.world.nation_get_constructing_cb_target(to_nation(primary_slot)), to_nation(from_slot)); }
TRIGGER_FUNCTION(tf_constructing_cb_this_nation) { return compare_values_eq(tval[0], ws.world.nation_get_constructing_cb_target(to_nation(primary_slot)), to_nation(this_slot)); }
...
TRIGGER_FUNCTION(tf_constructing_cb_discovered) { return compare_to_true(tval[0], ws.world.nation_get_constructing_cb_is_discovered(to_nation(primary_slot))); }
TRIGGER_FUNCTION(tf_constructing_cb_progress) { return compare_values(tval[0], ws.world.nation_get_constructing_cb_progress(to_nation(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_civilization_progress) { return compare_values(tval[0], ws.world.nation_get_modifier_values(to_nation(primary_slot), sys::national_mod_offsets::civilization_progress_modifier), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_constructing_cb_type) { return compare_values_eq(tval[0], ws.world.nation_get_constructing_cb_type(to_nation(primary_slot)), payload(tval[1]).cb_id); }
...
TRIGGER_FUNCTION(tf_is_our_vassal_from) { return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(from_slot))), to_nation(primary_slot)); }
TRIGGER_FUNCTION(tf_is_our_vassal_this_nation) { return compare_values_eq(tval[0], ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(this_slot))), to_nation(primary_slot)); }
...
TRIGGER_FUNCTION(tf_part_of_sphere) { return compare_to_true(tval[0], ws.world.nation_get_in_sphere_of(to_nation(primary_slot)) != dcon::nation_id()); }
...
TRIGGER_FUNCTION(tf_is_sphere_leader_of_from) { return compare_values_eq(tval[0], ws.world.nation_get_in_sphere_of(to_nation(from_slot)), to_nation(primary_slot)); }
TRIGGER_FUNCTION(tf_is_sphere_leader_of_this_nation) { return compare_values_eq(tval[0], ws.world.nation_get_in_sphere_of(to_nation(this_slot)), to_nation(primary_slot)); }
...
TRIGGER_FUNCTION(tf_number_of_states) { return compare_values(tval[0], ws.world.nation_get_owned_state_count(to_nation(primary_slot)), tval[1]); }
...
TRIGGER_FUNCTION(tf_has_recent_imigration) {
	return compare_values(tval[0], int32_t(tval[1]) + ve::apply([&ws](dcon::province_id p) { return ws.world.province_get_last_immigration(p).to_raw_value(); }, to_prov(primary_slot)), ws.current_date.to_raw_value());
}
TRIGGER_FUNCTION(tf_province_control_days) {
	return compare_values(tval[0], ws.current_date.to_raw_value() - ve::apply([&ws](dcon::province_id p) { return ws.world.province_get_last_control_change(p).to_raw_value(); }, to_prov(primary_slot)), int32_t(tval[1]));
}
...
TRIGGER_FUNCTION(tf_social_movement) {
	return compare_to_true(tval[0], ve::apply(
										[&ws](dcon::pop_id p) {
											...
											return m && m.get_associated_issue_option() && culture::issue_type(m.get_associated_issue_option().get_parent_issue().get_issue_type()) == culture::issue_type::social;
										},
										to_pop(primary_slot)));
}
TRIGGER_FUNCTION(tf_political_movement) {
	return compare_to_true(tval[0], ve::apply(
										[&ws](dcon::pop_id p) {
											auto m = fatten(ws.world, ws.world.pop_get_movement_from_pop_movement_membership(p));
											return m && m.get_associated_issue_option() && culture::issue_type(m.get_associated_issue_option().get_parent_issue().get_issue_type()) == culture::issue_type::political;
										},
										to_pop(primary_slot)));
}
...
TRIGGER_FUNCTION(tf_has_pop_culture_pop_this_pop) { return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), ws.world.pop_get_culture(to_pop(this_slot))); }
TRIGGER_FUNCTION(tf_has_pop_culture_state_this_pop) {
	auto culture = ws.world.pop_get_culture(to_pop(this_slot));
	auto result = ve::apply([&ws](dcon::state_instance_id s, dcon::culture_id c) { return bool(c) ? ws.world.state_instance_get_demographics(s, demographics::to_key(ws, c)) > 0.0f : false; }, to_state(primary_slot), culture);
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_has_pop_culture_province_this_pop) {
	auto culture = ws.world.pop_get_culture(to_pop(this_slot));
	auto result = ve::apply([&ws](dcon::province_id s, dcon::culture_id c) { return bool(c) ? ws.world.province_get_demographics(s, demographics::to_key(ws, c)) > 0.0f : false; }, to_prov(primary_slot), culture);
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_has_pop_culture_nation_this_pop) {
	auto culture = ws.world.pop_get_culture(to_pop(this_slot));
	auto result = ve::apply([&ws](dcon::nation_id s, dcon::culture_id c) { return bool(c) ? ws.world.nation_get_demographics(s, demographics::to_key(ws, c)) > 0.0f : false; }, to_nation(primary_slot), culture);
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_has_pop_culture_pop) { return compare_values_eq(tval[0], ws.world.pop_get_culture(to_pop(primary_slot)), payload(tval[1]).cul_id); }
TRIGGER_FUNCTION(tf_has_pop_culture_state) { return compare_to_true(tval[0], ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_has_pop_culture_province) { return compare_to_true(tval[0], ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_has_pop_culture_nation) { return compare_to_true(tval[0], ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_has_pop_religion_pop_this_pop) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), ws.world.pop_get_religion(to_pop(this_slot))); }
TRIGGER_FUNCTION(tf_has_pop_religion_state_this_pop) {
	auto rel = ws.world.pop_get_religion(to_pop(this_slot));
	auto result = ve::apply([&ws](dcon::state_instance_id s, dcon::religion_id r) { return bool(r) ? ws.world.state_instance_get_demographics(s, demographics::to_key(ws, r)) > 0.0f : false; }, to_state(primary_slot), rel);
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_has_pop_religion_province_this_pop) {
	auto rel = ws.world.pop_get_religion(to_pop(this_slot));
	auto result = ve::apply([&ws](dcon::province_id s, dcon::religion_id r) { return bool(r) ? ws.world.province_get_demographics(s, demographics::to_key(ws, r)) > 0.0f : false; }, to_prov(primary_slot), rel);
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_has_pop_religion_pop) { return compare_values_eq(tval[0], ws.world.pop_get_religion(to_pop(primary_slot)), payload(tval[1]).rel_id); }
TRIGGER_FUNCTION(tf_has_pop_religion_state) { return compare_to_true(tval[0], ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_has_pop_religion_province) { return compare_to_true(tval[0], ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_has_pop_religion_nation) { return compare_to_true(tval[0], ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f); }
TRIGGER_FUNCTION(tf_life_needs) { return compare_values(tval[0], ws.world.pop_get_life_needs_satisfaction(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_everyday_needs) { return compare_values(tval[0], ws.world.pop_get_everyday_needs_satisfaction(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_luxury_needs) { return compare_values(tval[0], ws.world.pop_get_luxury_needs_satisfaction(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_consciousness_pop) { return compare_values(tval[0], ws.world.pop_get_consciousness(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_consciousness_province) {
	auto total_pop = ws.world.province_get_demographics(to_prov(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.province_get_demographics(to_prov(primary_slot), demographics::consciousness) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_consciousness_state) {
	auto total_pop = ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::consciousness) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_consciousness_nation) {
	auto total_pop = ws.world.nation_get_demographics(to_nation(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.nation_get_demographics(to_nation(primary_slot), demographics::consciousness) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_literacy_pop) { return compare_values(tval[0], ws.world.pop_get_literacy(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_literacy_province) {
	auto total_pop = ws.world.province_get_demographics(to_prov(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.province_get_demographics(to_prov(primary_slot), demographics::literacy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_literacy_state) {
	auto total_pop = ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::literacy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_literacy_nation) {
	auto total_pop = ws.world.nation_get_demographics(to_nation(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.nation_get_demographics(to_nation(primary_slot), demographics::literacy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_militancy_pop) { return compare_values(tval[0], ws.world.pop_get_militancy(to_pop(primary_slot)), read_float_from_payload(tval + 1)); }
TRIGGER_FUNCTION(tf_militancy_province) {
	auto total_pop = ws.world.province_get_demographics(to_prov(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.province_get_demographics(to_prov(primary_slot), demographics::militancy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_militancy_state) {
	auto total_pop = ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::militancy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_militancy_nation) {
	auto total_pop = ws.world.nation_get_demographics(to_nation(primary_slot), demographics::total);
	return compare_values(tval[0], ve::select(total_pop != 0.0f, ws.world.nation_get_demographics(to_nation(primary_slot), demographics::militancy) / total_pop, 0.0f), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_has_flashpoint) { return compare_to_true(tval[0], ws.world.state_instance_get_flashpoint_tag(to_state(primary_slot)) != dcon::national_identity_id()); }
TRIGGER_FUNCTION(tf_flashpoint_tension) { return compare_values(tval[0], ws.world.state_instance_get_flashpoint_tension(to_state(primary_slot)), read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_crisis_temperature) { return compare_values(tval[0], ws.crisis_temperature, read_float_from_payload(tval + 1)); }
...
TRIGGER_FUNCTION(tf_unemployment_by_type_nation) { return tf_pop_unemployment_nation<return_type>(tval, ws, primary_slot, int32_t(), int32_t()); }
TRIGGER_FUNCTION(tf_unemployment_by_type_state) { return tf_pop_unemployment_state<return_type>(tval, ws, primary_slot, int32_t(), int32_t()); }
TRIGGER_FUNCTION(tf_unemployment_by_type_province) { return tf_pop_unemployment_province<return_type>(tval, ws, primary_slot, int32_t(), int32_t()); }
...
TRIGGER_FUNCTION(tf_party_loyalty_nation_province_id) { return compare_values(tval[0], ws.world.province_get_party_loyalty(payload(tval[1]).prov_id, payload(tval[3]).ideo_id), payload(tval[2]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_from_nation_province_id) { return compare_values(tval[0], ws.world.province_get_party_loyalty(payload(tval[1]).prov_id, payload(tval[3]).ideo_id), payload(tval[2]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_province_province_id) { return compare_values(tval[0], ws.world.province_get_party_loyalty(payload(tval[1]).prov_id, payload(tval[3]).ideo_id), payload(tval[2]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_from_province_province_id) { return compare_values(tval[0], ws.world.province_get_party_loyalty(payload(tval[1]).prov_id, payload(tval[3]).ideo_id), payload(tval[2]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_nation_from_province) { return compare_values(tval[0], ws.world.province_get_party_loyalty(to_prov(from_slot), payload(tval[2]).ideo_id), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_generic) { return compare_values(tval[0], ws.world.province_get_party_loyalty(to_prov(primary_slot), payload(tval[2]).ideo_id), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_from_nation_scope_province) { return compare_values(tval[0], ws.world.province_get_party_loyalty(to_prov(primary_slot), payload(tval[2]).ideo_id), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_party_loyalty_from_province_scope_province) { return compare_values(tval[0], ws.world.province_get_party_loyalty(to_prov(primary_slot), payload(tval[2]).ideo_id), payload(tval[1]).signed_value); }
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_no_limit_from_nation) {
	return compare_to_true(tval[0], ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) + ws.world.province_get_modifier_values(to_prov(primary_slot), sys::provincial_mod_offsets::min_build_railroad) < ve::to_float(ws.world.nation_get_max_railroad_level(to_nation(from_slot))));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_no_limit_this_nation) {
	return compare_to_true(tval[0], ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) + ws.world.province_get_modifier_values(to_prov(primary_slot), sys::provincial_mod_offsets::min_build_railroad) < ve::to_float(ws.world.nation_get_max_railroad_level(to_nation(this_slot))));
}
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_yes_limit_this_nation) {
	return compare_to_true(tval[0], ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) + ws.world.province_get_modifier_values(to_prov(primary_slot), sys::provincial_mod_offsets::min_build_railroad) < ve::to_float(ws.world.nation_get_max_railroad_level(to_nation(this_slot))));
}
TRIGGER_FUNCTION(tf_can_build_in_province_fort_no_limit_from_nation) { return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) < ws.world.nation_get_max_fort_level(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_can_build_in_province_fort_yes_limit_from_nation) { return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) < ws.world.nation_get_max_fort_level(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_can_build_in_province_fort_no_limit_this_nation) { return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) < ws.world.nation_get_max_fort_level(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_can_build_in_province_fort_yes_limit_this_nation) { return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) < ws.world.nation_get_max_fort_level(to_nation(this_slot))); }
TRIGGER_FUNCTION(tf_can_build_in_province_naval_base_no_limit_from_nation) {
	auto result = ws.world.province_get_is_coast(to_prov(primary_slot)) && (ws.world.province_get_naval_base_level(to_prov(primary_slot)) < ws.world.nation_get_max_naval_base_level(to_nation(from_slot))) &&
				  (ws.world.province_get_naval_base_level(to_prov(primary_slot)) != 0 || ve::apply([&ws](dcon::state_instance_id i) { return !(military::state_has_naval_base(ws, i)); }, ws.world.province_get_state_membership(to_prov(primary_slot))));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_naval_base_no_limit_this_nation) {
	auto result = ws.world.province_get_is_coast(to_prov(primary_slot)) && (ws.world.province_get_naval_base_level(to_prov(primary_slot)) < ws.world.nation_get_max_naval_base_level(to_nation(this_slot))) &&
				  (ws.world.province_get_naval_base_level(to_prov(primary_slot)) != 0 || ve::apply([&ws](dcon::state_instance_id i) { return !(military::state_has_naval_base(ws, i)); }, ws.world.province_get_state_membership(to_prov(primary_slot))));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_work_available_nation) {
	auto type = payload(tval[1]).popt_id;
	auto result = ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_employment_key(ws, type)) >= ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_key(ws, type));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_work_available_state) {
	auto type = payload(tval[1]).popt_id;
	auto result = ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::to_employment_key(ws, type)) >= ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::to_key(ws, type));
	return compare_to_true(tval[0], result);
}
TRIGGER_FUNCTION(tf_work_available_province) {
	auto type = payload(tval[1]).popt_id;
	auto result = ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_employment_key(ws, type)) >= ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_key(ws, type));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_religion_nation) { return compare_values_eq(tval[0], ws.world.nation_get_religion(to_nation(primary_slot)), payload(tval[1]).rel_id); }
...
TRIGGER_FUNCTION(tf_religion_nation_from_nation) { return compare_values_eq(tval[0], ws.world.nation_get_religion(to_nation(primary_slot)), ws.world.nation_get_religion(to_nation(from_slot))); }
TRIGGER_FUNCTION(tf_religion_nation_this_nation) { return compare_values_eq(tval[0], ws.world.nation_get_religion(to_nation(primary_slot)), ws.world.nation_get_religion(to_nation(this_slot))); }
...
template<typename return_type, typename primary_type, typename this_type, typename from_type> struct trigger_container {
	constexpr static return_type(CALLTYPE* trigger_functions[])(uint16_t const*, sys::state&, primary_type, this_type, from_type) = {

		tf_none<return_type, primary_type, this_type, from_type>, tf_year<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t year = 0x0001;
		...
		tf_life_rating_state<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t life_rating_state = 0x0008;
		...
		tf_has_country_flag<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t has_country_flag = 0x0078;
		tf_continent_nation<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t continent_nation = 0x0079;
		tf_continent_state<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t continent_state = 0x007A;
		tf_continent_province<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t continent_province = 0x007B;
		...
		tf_continent_pop_this<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t continent_pop_this = 0x0080;
		...
		tf_continent_pop_from<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t continent_pop_from = 0x0084;
		tf_casus_belli_tag<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t casus_belli_tag = 0x0085;
		tf_casus_belli_from<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t casus_belli_from = 0x0086;
		...
		tf_prestige_this_pop<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t prestige_this_pop = 0x0096;
		tf_badboy<return_type, primary_type, this_type, from_type>,				// constexpr inline uint16_t badboy = 0x0097;
		tf_has_building_state<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t has_building_state = 0x0098;
		tf_has_building_fort<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t has_building_fort = 0x0099;
		...
		tf_tag_this_nation<return_type, primary_type, this_type, from_type>,   // constexpr inline uint16_t tag_this_nation = 0x00A2;
		tf_tag_this_province<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t tag_this_province = 0x00A3;
		tf_tag_from_nation<return_type, primary_type, this_type, from_type>,   // constexpr inline uint16_t tag_from_nation = 0x00A4;
		tf_tag_from_province<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t tag_from_province = 0x00A5;
		...
		tf_is_next_reform_pop<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t is_next_reform_pop = 0x00F0;
		...
		tf_has_culture_core<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t has_culture_core = 0x00F4;
		...
		tf_controlled_by_tag<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t controlled_by_tag = 0x00F8;
		tf_controlled_by_from<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t controlled_by_from = 0x00F9;
		...
		tf_controlled_by_reb<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t controlled_by_reb = 0x00FF;
		tf_is_canal_enabled<return_type, primary_type, this_type, from_type>,  // constexpr inline uint16_t is_canal_enabled = 0x0100;
		tf_is_state_capital<return_type, primary_type, this_type, from_type>,  // constexpr inline uint16_t is_state_capital = 0x0101;
		tf_truce_with_tag<return_type, primary_type, this_type, from_type>,	   // constexpr inline uint16_t truce_with_tag = 0x0102;
		tf_truce_with_from<return_type, primary_type, this_type, from_type>,   // constexpr inline uint16_t truce_with_from = 0x0103;
		...
		tf_total_pops_nation<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t total_pops_nation = 0x0108;
		tf_total_pops_state<return_type, primary_type, this_type, from_type>,  // constexpr inline uint16_t total_pops_state = 0x0109;
		...
		tf_has_pop_type_state<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t has_pop_type_state = 0x010D;
		...
		tf_has_pop_type_pop<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t has_pop_type_pop = 0x010F;
		...
		tf_can_create_vassals<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t can_create_vassals = 0x0113;
		tf_is_possible_vassal<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t is_possible_vassal = 0x0114;
		...
		tf_vassal_of_this_pop<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t vassal_of_this_pop = 0x011B;
		tf_alliance_with_tag<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t alliance_with_tag = 0x011C;
		tf_alliance_with_from<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t alliance_with_from = 0x011D;
		...
		tf_mobilisation_size<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t mobilisation_size = 0x0124;
		...
		tf_is_colonial_state<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t is_colonial_state = 0x012A;
		...
		tf_in_default_from<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t in_default_from = 0x012E;
		...
		tf_total_num_of_ports<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t total_num_of_ports = 0x0133;
		...
		tf_has_global_flag<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t has_global_flag = 0x0136;
		...
		tf_civilized_nation<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t civilized_nation = 0x0145;
		tf_civilized_pop<return_type, primary_type, this_type, from_type>,		// constexpr inline uint16_t civilized_pop = 0x0146;
		tf_civilized_province<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t civilized_province = 0x0147;
		...
		tf_colonial_nation<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t colonial_nation = 0x0151;
		...
		tf_number_of_states<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t number_of_states = 0x01A3;
		...
		tf_social_movement<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t social_movement = 0x01B0;
		tf_political_movement<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t political_movement = 0x01B1;
		tf_has_cultural_sphere<return_type, primary_type, this_type,
			from_type>,															// constexpr inline uint16_t has_cultural_sphere = 0x01B2;
		tf_world_wars_enabled<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t world_wars_enabled = 0x01B3;
		...
		tf_consciousness_pop<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t consciousness_pop = 0x01C7;
		...
		tf_literacy_province<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t literacy_province = 0x01CC;
		tf_literacy_state<return_type, primary_type, this_type, from_type>,		// constexpr inline uint16_t literacy_state = 0x01CD;
		tf_literacy_nation<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t literacy_nation = 0x01CE;
		tf_militancy_pop<return_type, primary_type, this_type, from_type>,		// constexpr inline uint16_t militancy_pop = 0x01CF;
		tf_militancy_province<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t militancy_province = 0x01D0;
		tf_militancy_state<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t militancy_state = 0x01D1;
		tf_militancy_nation<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t militancy_nation = 0x01D2;
		...
		tf_flashpoint_tension<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t flashpoint_tension = 0x01E2;
		...
		tf_is_claim_crisis<return_type, primary_type, this_type, from_type>,	// constexpr inline uint16_t is_claim_crisis = 0x01E5;
		...
		tf_variable_good_name<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t variable_good_name = 0x0251;
		...
		tf_nationalvalue_pop<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t nationalvalue_pop = 0x0257;
		...
		tf_religion_nation<return_type, primary_type, this_type, from_type>, // constexpr inline uint16_t religion_nation = 0x025F;
		...
		tf_x_neighbor_province_scope<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t x_neighbor_province_scope = 0x0001;
		tf_x_neighbor_country_scope_nation<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t x_neighbor_country_scope_nation = 0x0002;
		tf_x_neighbor_country_scope_pop<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t x_neighbor_country_scope_pop = 0x0003;
		tf_x_war_countries_scope_nation<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t x_war_countries_scope_nation = 0x0004;
		tf_x_war_countries_scope_pop<return_type, primary_type, this_type,
			from_type>,															   // constexpr uint16_t x_war_countries_scope_pop = 0x0005;
		tf_x_greater_power_scope<return_type, primary_type, this_type, from_type>, // constexpr uint16_t x_greater_power_scope = 0x0006;
		tf_x_owned_province_scope_state<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t x_owned_province_scope_state = 0x0007;
		tf_x_owned_province_scope_nation<return_type, primary_type, this_type,
			from_type>,															   // constexpr uint16_t x_owned_province_scope_nation = 0x0008;
		...
		tf_x_provinces_in_variable_region<return_type, primary_type, this_type,
			from_type>,																// constexpr uint16_t x_provinces_in_variable_region = 0x0011; // variable name
		...
		tf_cultural_union_scope_pop<return_type, primary_type, this_type,
			from_type>, // constexpr uint16_t cultural_union_scope_pop = 0x002E;
	};
};
...
bool evaluate(sys::state& state, dcon::trigger_key key, int32_t primary, int32_t this_slot, int32_t from_slot) { return test_trigger_generic<bool>(state.trigger_data.data() + state.trigger_data_indices[key.index()], state, primary, this_slot, from_slot); }
bool evaluate(sys::state& state, uint16_t const* data, int32_t primary, int32_t this_slot, int32_t from_slot) { return test_trigger_generic<bool>(data, state, primary, this_slot, from_slot); }
...
ve::mask_vector evaluate(sys::state& state, uint16_t const* data, ve::contiguous_tags<int32_t> primary, ve::tagged_vector<int32_t> this_slot, int32_t from_slot) { return test_trigger_generic<ve::mask_vector>(data, state, primary, this_slot, from_slot); }
...
ve::mask_vector evaluate(sys::state& state, uint16_t const* data, ve::tagged_vector<int32_t> primary, ve::tagged_vector<int32_t> this_slot, int32_t from_slot) { return test_trigger_generic<ve::mask_vector>(data, state, primary, this_slot, from_slot); }
...
ve::mask_vector evaluate(sys::state& state, uint16_t const* data, ve::contiguous_tags<int32_t> primary, ve::contiguous_tags<int32_t> this_slot, int32_t from_slot) { return test_trigger_generic<ve::mask_vector>(data, state, primary, this_slot, from_slot); }
