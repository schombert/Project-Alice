...
TRIGGER_FUNCTION(tf_controller_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(controller), this_slot,
	                                                                                      from_slot);
}
...
TRIGGER_FUNCTION(tf_capital_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(cap), this_slot,
	                                                                                      from_slot);
}
...
TRIGGER_FUNCTION(tf_sea_zone_scope) {
	auto sea_zones = ve::apply(
	    ...
	    primary_slot);
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(sea_zones), this_slot,
	                                                                                      from_slot);
}
TRIGGER_FUNCTION(tf_cultural_union_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(group_holders), this_slot,
	                                                                                      from_slot);
}
...
TRIGGER_FUNCTION(tf_independence_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(r_holders), this_slot,
	                                                                                      from_slot);
}
TRIGGER_FUNCTION(tf_flashpoint_tag_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(fp_nations), this_slot,
	                                                                                      from_slot);
}
...
TRIGGER_FUNCTION(tf_state_scope_province) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(state_instance), this_slot,
	                                                                                      from_slot);
}
...
TRIGGER_FUNCTION(tf_tag_scope) {
	...
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(tag_holder), this_slot,
	                                                                                      from_slot);
}
		    ...
		    for(auto sp : ws.world.state_definition_get_abstract_state_membership(region_id)) {
			    if(sp.get_province().get_nation_from_province_ownership() == region_owner) {
				    for(auto p : ws.world.province_get_province_adjacency(sp.get_province())) {
					    auto other = p.get_connected_provinces(0) == sp.get_province() ? p.get_connected_provinces(1)
...
TRIGGER_FUNCTION(tf_state_id_province) {
	...
	return compare_values_eq(
	    tval[0], ws.world.abstract_state_membership_get_state(ws.world.province_get_abstract_state_membership(to_prov(primary_slot))),
...
TRIGGER_FUNCTION(tf_is_slave_nation) {
	return compare_to_true(tval[0], (ws.world.nation_get_combined_issue_rules(to_nation(primary_slot)) & issue_rule::slavery_allowed) ==
	                                    issue_rule::slavery_allowed);
}
...
TRIGGER_FUNCTION(tf_primary_culture) {
	return compare_values_eq(tval[0], ws.world.nation_get_primary_culture(to_nation(primary_slot)), trigger::payload(tval[1]).cul_id);
}
...
TRIGGER_FUNCTION(tf_culture_province) {
	return compare_values_eq(tval[0], ws.world.province_get_dominant_culture(to_prov(primary_slot)), trigger::payload(tval[1]).cul_id);
}
...
TRIGGER_FUNCTION(tf_can_build_factory_nation) {
	return compare_values_eq(tval[0], ws.world.nation_get_combined_issue_rules(to_nation(primary_slot)) & issue_rule::pop_build_factory,
...
TRIGGER_FUNCTION(tf_blockade) {
	return compare_values(tval[0], nations::central_blockaded_fraction(ws, to_nation(primary_slot)), read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_continent_pop_this) {
	return compare_values_eq(tval[0], ws.world.province_get_continent(ws.world.pop_get_province_from_pop_location(to_pop(primary_slot))),
	                         ws.world.province_get_continent(ws.world.nation_get_capital(to_nation(this_slot))));
}
...
TRIGGER_FUNCTION(tf_casus_belli_from) {
	auto result = ve::apply([&ws](dcon::nation_id n, dcon::nation_id target) { return military::can_use_cb_against(ws, n, target); },
...
TRIGGER_FUNCTION(tf_has_building_state) {
	auto result = ve::apply([&ws, f = payload(tval[1]).fac_id](dcon::state_instance_id s) { return economy::has_building(ws, s, f); },
...
TRIGGER_FUNCTION(tf_neighbour_tag) {
	auto tag_holder = ws.world.national_identity_get_nation_from_identity_holder(trigger::payload(tval[1]).tag_id);
	auto result = ve::apply(
	    [&ws, tag_holder](dcon::nation_id n) { return bool(ws.world.get_nation_adjacency_by_nation_adjacency_pair(n, tag_holder)); },
	    to_nation(primary_slot));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_war_with_this_province) {
	auto owner = ws.world.province_get_nation_from_province_ownership(to_prov(this_slot));
	auto result =
	    ve::apply([&ws](dcon::nation_id a, dcon::nation_id b) { return military::are_at_war(ws, a, b); }, to_nation(primary_slot), owner);
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_controlled_by_rebels) {
	return compare_to_true(tval[0], ws.world.province_get_rebel_faction_from_province_rebel_control(to_prov(primary_slot)) !=
...
TRIGGER_FUNCTION(tf_substate_of_tag) {
	...
	return compare_to_true(
	    tval[0], ws.world.nation_get_is_substate(to_nation(primary_slot)) &&
...
TRIGGER_FUNCTION(tf_in_default_tag) {
	auto holder = ws.world.national_identity_get_nation_from_identity_holder(payload(tval[1]).tag_id);
	return compare_to_true(tval[0], ve::apply([&ws, holder](dcon::nation_id n) { return economy::is_bankrupt_debtor_to(ws, n, holder); },
	                                          to_nation(primary_slot)));
}
...
TRIGGER_FUNCTION(tf_social_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_social_spending(to_nation(primary_slot)), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_military_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_military_spending(to_nation(primary_slot)), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_administration_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_administrative_spending(to_nation(primary_slot)), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_education_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_education_spending(to_nation(primary_slot)), payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_pop_majority_religion_state) {
	return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_religion(to_state(primary_slot)), payload(tval[1]).rel_id);
}
...
TRIGGER_FUNCTION(tf_pop_majority_culture_state) {
	return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_culture(to_state(primary_slot)), payload(tval[1]).cul_id);
}
...
TRIGGER_FUNCTION(tf_pop_majority_issue_state) {
	return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_issue_option(to_state(primary_slot)), payload(tval[1]).opt_id);
}
...
TRIGGER_FUNCTION(tf_pop_majority_ideology_state) {
	return compare_values_eq(tval[0], ws.world.state_instance_get_dominant_ideology(to_state(primary_slot)), payload(tval[1]).ideo_id);
}
...
TRIGGER_FUNCTION(tf_num_of_vassals_no_substates) {
	return compare_values(tval[0],
	                      ws.world.nation_get_vassals_count(to_nation(primary_slot)) -
	                          ws.world.nation_get_substates_count(to_nation(primary_slot)),
	                      tval[1]);
}
...
TRIGGER_FUNCTION(tf_civilization_progress) {
	return compare_values(
	    tval[0], ws.world.nation_get_modifier_values(to_nation(primary_slot), sys::national_mod_offsets::civilization_progress_modifier),
	    read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_has_pop_culture_state) {
	return compare_to_true(tval[0], ws.world.state_instance_get_demographics(to_state(primary_slot),
	                                                                         demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f);
}
TRIGGER_FUNCTION(tf_has_pop_culture_province) {
	return compare_to_true(
	    tval[0], ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f);
}
TRIGGER_FUNCTION(tf_has_pop_culture_nation) {
	return compare_to_true(
	    tval[0], ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_key(ws, payload(tval[1]).cul_id)) > 0.0f);
}
...
TRIGGER_FUNCTION(tf_has_pop_religion_state) {
	return compare_to_true(tval[0], ws.world.state_instance_get_demographics(to_state(primary_slot),
	                                                                         demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f);
}
TRIGGER_FUNCTION(tf_has_pop_religion_province) {
	return compare_to_true(
	    tval[0], ws.world.province_get_demographics(to_prov(primary_slot), demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f);
}
TRIGGER_FUNCTION(tf_has_pop_religion_nation) {
	return compare_to_true(
	    tval[0], ws.world.nation_get_demographics(to_nation(primary_slot), demographics::to_key(ws, payload(tval[1]).rel_id)) > 0.0f);
}
...
TRIGGER_FUNCTION(tf_consciousness_province) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.province_get_demographics(to_prov(primary_slot), demographics::consciousness) / total_pop,
	                                 0.0f),
	                      read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_consciousness_nation) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.nation_get_demographics(to_nation(primary_slot), demographics::consciousness) / total_pop,
	                                 0.0f),
	                      read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_literacy_province) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.province_get_demographics(to_prov(primary_slot), demographics::literacy) / total_pop, 0.0f),
	                      read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_literacy_state) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::literacy) / total_pop,
	                                 0.0f),
	                      read_float_from_payload(tval + 1));
}
TRIGGER_FUNCTION(tf_literacy_nation) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.nation_get_demographics(to_nation(primary_slot), demographics::literacy) / total_pop, 0.0f),
	                      read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_militancy_province) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.province_get_demographics(to_prov(primary_slot), demographics::militancy) / total_pop,
	                                 0.0f),
	                      read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_militancy_nation) {
	...
	return compare_values(tval[0],
	                      ve::select(total_pop != 0.0f,
	                                 ws.world.nation_get_demographics(to_nation(primary_slot), demographics::militancy) / total_pop,
	                                 0.0f),
	                      read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_no_limit_from_nation) {
	return compare_to_true(
	    tval[0], ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) +
	                     ws.world.province_get_modifier_values(to_prov(primary_slot), sys::provincial_mod_offsets::min_build_railroad) <
	                 ve::to_float(ws.world.nation_get_max_railroad_level(to_nation(from_slot))));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_no_limit_this_nation) {
	return compare_to_true(
	    tval[0], ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) +
	                     ws.world.province_get_modifier_values(to_prov(primary_slot), sys::provincial_mod_offsets::min_build_railroad) <
	                 ve::to_float(ws.world.nation_get_max_railroad_level(to_nation(this_slot))));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_fort_no_limit_from_nation) {
	return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) <
	                                    ws.world.nation_get_max_fort_level(to_nation(from_slot)));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_fort_no_limit_this_nation) {
	return compare_to_true(tval[0], ws.world.province_get_fort_level(to_prov(primary_slot)) <
	                                    ws.world.nation_get_max_fort_level(to_nation(this_slot)));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_naval_base_no_limit_from_nation) {
	auto result = ws.world.province_get_is_coast(to_prov(primary_slot)) &&
	              (ws.world.province_get_naval_base_level(to_prov(primary_slot)) <
	               ws.world.nation_get_max_naval_base_level(to_nation(from_slot))) &&
...
TRIGGER_FUNCTION(tf_can_build_in_province_naval_base_no_limit_this_nation) {
	auto result = ws.world.province_get_is_coast(to_prov(primary_slot)) &&
	              (ws.world.province_get_naval_base_level(to_prov(primary_slot)) <
	               ws.world.nation_get_max_naval_base_level(to_nation(this_slot))) &&
	              (ws.world.province_get_naval_base_level(to_prov(primary_slot)) != 0 ||
	               ve::apply([&ws](dcon::state_instance_id i) { return !(military::state_has_naval_base(ws, i)); },
	                         ws.world.province_get_state_membership(to_prov(primary_slot))));
	return compare_to_true(tval[0], result);
...
template<typename return_type, typename primary_type, typename this_type, typename from_type> struct trigger_container {
	constexpr static return_type(CALLTYPE* trigger_functions[])(uint16_t const*, sys::state&, primary_type, this_type, from_type) = {
	    ...
	    tf_can_build_in_province_railroad_no_limit_from_nation<
	        return_type, primary_type, this_type,
...
return_type CALLTYPE test_trigger_generic(uint16_t const* tval, sys::state& ws, primary_type primary_slot, this_type this_slot,
                                          from_type from_slot) {
	return trigger_container<return_type, primary_type, this_type, from_type>::trigger_functions[*tval & trigger::code_mask](
	    tval, ws, primary_slot, this_slot, from_slot);
}
...
ve::fp_vector evaluate_multiplicative_modifier(sys::state& state, dcon::value_modifier_key modifier, ve::contiguous_tags<int32_t> primary,
...
ve::fp_vector evaluate_additive_modifier(sys::state& state, dcon::value_modifier_key modifier, ve::contiguous_tags<int32_t> primary,
                                         ve::tagged_vector<int32_t> this_slot, int32_t from_slot) {
	auto base = state.value_modifiers[modifier];
...
bool evaluate(sys::state& state, dcon::trigger_key key, int32_t primary, int32_t this_slot, int32_t from_slot) {
	return test_trigger_generic<bool>(state.trigger_data.data() + state.trigger_data_indices[key.index()], state, primary, this_slot,
	                                  from_slot);
}
...
ve::mask_vector evaluate(sys::state& state, dcon::trigger_key key, ve::contiguous_tags<int32_t> primary,
                         ve::tagged_vector<int32_t> this_slot, int32_t from_slot) {
	return test_trigger_generic<ve::mask_vector>(state.trigger_data.data() + state.trigger_data_indices[key.index()], state, primary,
	                                             this_slot, from_slot);
}
