...
auto existence_accumulator(sys::state& ws, uint16_t const* tval, int32_t this_slot, int32_t from_slot) {
	return make_true_accumulator([&ws, tval, this_slot, from_slot](ve::tagged_vector<int32_t> v) {
		return apply_subtriggers<ve::mask_vector, ve::tagged_vector<int32_t>, int32_t, int32_t>(tval, ws, v, this_slot,
			from_slot);
	});
}
		...
		[&ws, tval](int32_t p_slot, int32_t t_slot, int32_t f_slot) {
			...
			if(*tval & trigger::is_existence_scope) {
				...
				for(auto adj : ws.world.nation_get_nation_adjacency(nid)) {
					auto iid =
						(nid == adj.get_connected_nations(0)) ? adj.get_connected_nations(1).id : adj.get_connected_nations(0).id;
...
TRIGGER_FUNCTION(tf_location_scope) {
	auto location = ws.world.pop_get_province_from_pop_location(to_pop(primary_slot));
	return apply_subtriggers<return_type, gathered_t<primary_type>, this_type, from_type>(tval, ws, to_generic(location),
		this_slot, from_slot);
}
...
auto empty_province_from_state_accumulator(sys::state const& ws, dcon::state_instance_id sid) {
	return make_true_accumulator(
		[&ws, vector_sid = ve::tagged_vector<dcon::state_instance_id>(sid),
...
TRIGGER_FUNCTION(tf_is_independant) {
	return compare_values_eq(tval[0],
		ws.world.overlord_get_ruler(ws.world.nation_get_overlord_as_subject(to_nation(primary_slot))), dcon::nation_id());
}
...
TRIGGER_FUNCTION(tf_war_exhaustion_nation) {
	return compare_values(tval[0], ws.world.nation_get_war_exhaustion(to_nation(primary_slot)),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_has_building_state_from_province) {
	...
	auto result = ve::apply(
		[&ws, f = payload(tval[1]).fac_id](dcon::state_instance_id s) { return economy::has_building(ws, s, f); }, state);
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_neighbour_this) {
	auto result = ve::apply([&ws](dcon::nation_id n,
								dcon::nation_id o) { return bool(ws.world.get_nation_adjacency_by_nation_adjacency_pair(n, o)); },
		to_nation(primary_slot), to_nation(this_slot));
	return compare_to_true(tval[0], result);
}
...
TRIGGER_FUNCTION(tf_social_reform_want_pop) {
	return compare_values(tval[0], ws.world.pop_get_social_reform_desire(to_pop(primary_slot)),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_is_greater_power_nation) {
	return compare_to_true(tval[0],
		ws.world.nation_get_rank(to_nation(primary_slot)) <= uint16_t(ws.defines.great_nations_count));
}
...
TRIGGER_FUNCTION(tf_administration_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_administrative_spending(to_nation(primary_slot)),
...
TRIGGER_FUNCTION(tf_education_spending_nation) {
	return compare_values(tval[0], ws.world.nation_get_education_spending(to_nation(primary_slot)),
		payload(tval[1]).signed_value);
}
...
TRIGGER_FUNCTION(tf_pop_majority_issue_nation) {
	return compare_values_eq(tval[0], ws.world.nation_get_dominant_issue_option(to_nation(primary_slot)),
...
TRIGGER_FUNCTION(tf_pop_majority_issue_province) {
	return compare_values_eq(tval[0], ws.world.province_get_dominant_issue_option(to_prov(primary_slot)),
		payload(tval[1]).opt_id);
}
...
TRIGGER_FUNCTION(tf_rich_strata_luxury_needs_state) {
	...
	return compare_values(tval[0],
		ve::select(accumulator > 0.0f,
			ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::rich_luxury_needs) / accumulator,
			0.0f),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_middle_strata_life_needs_state) {
	...
	return compare_values(tval[0],
		ve::select(accumulator > 0.0f,
			ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::middle_life_needs) / accumulator,
			0.0f),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_poor_strata_luxury_needs_state) {
	...
	return compare_values(tval[0],
		ve::select(accumulator > 0.0f,
			ws.world.state_instance_get_demographics(to_state(primary_slot), demographics::poor_luxury_needs) / accumulator,
			0.0f),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_upper_house) {
	...
	return compare_values(tval[0], ws.world.nation_get_upper_house(to_nation(primary_slot), id),
		read_float_from_payload(tval + 1));
}
...
TRIGGER_FUNCTION(tf_can_build_in_province_railroad_no_limit_from_nation) {
	return compare_to_true(tval[0],
		ve::to_float(ws.world.province_get_railroad_level(to_prov(primary_slot))) +
...
TRIGGER_FUNCTION(tf_variable_ideology_name_nation) {
	...
	return compare_values(tval[0], ve::select(total_pop > 0.0f, support_pop / total_pop, 0.0f),
...
float evaluate_multiplicative_modifier(sys::state& state, dcon::value_modifier_key modifier, int32_t primary, int32_t this_slot,
	...
	for(uint32_t i = 0; i < base.segments_count && product != 0; ++i) {
		...
		if(seg.condition) {
			if(test_trigger_generic<bool>(state.trigger_data.data() + state.trigger_data_indices[seg.condition.index()], state,
...
ve::fp_vector evaluate_multiplicative_modifier(sys::state& state, dcon::value_modifier_key modifier,
	...
	for(uint32_t i = 0; i < base.segments_count; ++i) {
		auto seg = state.value_modifier_segments[base.first_segment_offset + i];
		if(seg.condition) {
			auto res = test_trigger_generic<ve::mask_vector>(state.trigger_data.data() +
																 state.trigger_data_indices[seg.condition.index()],
				state, primary, this_slot, from_slot);
