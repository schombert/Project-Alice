...
void trigger_national_event(sys::state& state, dcon::national_event_id e, dcon::nation_id n, uint32_t r_lo, uint32_t r_hi,
	...
	if(ft == slot_type::province)
		assert(
		    dcon::fatten(state.world, state.world.province_get_nation_from_province_ownership(trigger::to_prov(from_slot))).is_valid());
...
void update_events(sys::state& state) {
	...
	concurrency::parallel_for(p_block_size * block_index, p_block_end, [&](uint32_t i) {
		...
		if(mod) {
			ve::execute_serial_fast<dcon::province_id>(
			    uint32_t(state.province_definitions.first_sea_province.index()), [&](ve::contiguous_tags<dcon::province_id> ids) {
				    /*
