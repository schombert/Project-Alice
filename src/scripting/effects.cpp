...
uint32_t ef_life_rating(EFFECT_PARAMTERS) {
	ws.world.province_set_life_rating(trigger::to_prov(primary_slot),
		uint8_t(
			std::clamp(int32_t(ws.world.province_get_life_rating(trigger::to_prov(primary_slot))) + trigger::payload(tval[1]).signed_value, 0, 255)
		)
	);
	return 0;
}
...
uint32_t ef_secede_province_state(EFFECT_PARAMTERS) {
	...
	province::for_each_province_in_state_instance(ws, trigger::to_state(primary_slot), [&](dcon::province_id p) {
		province::change_province_owner(ws, p, holder);
	});
	return 0;
}
...
uint32_t ef_release(EFFECT_PARAMTERS) {
	nations::release_nation_from(ws, trigger::payload(tval[1]).tag_id, trigger::to_nation(primary_slot));
	return 0;
}
uint32_t ef_release_this_nation(EFFECT_PARAMTERS) {
	nations::release_nation_from(ws, ws.world.nation_get_identity_from_identity_holder(trigger::to_nation(this_slot)), trigger::to_nation(primary_slot));
	return 0;
}
...
uint32_t ef_release_from_nation(EFFECT_PARAMTERS) {
	nations::release_nation_from(ws, ws.world.nation_get_identity_from_identity_holder(trigger::to_nation(from_slot)), trigger::to_nation(primary_slot));
	return 0;
}
...
uint32_t ef_infrastructure(EFFECT_PARAMTERS) {
	...
	rr_level =
		uint8_t(std::clamp(int32_t(rr_level) + int32_t(trigger::payload(tval[1]).signed_value), 0, 255));
	return 0;
}
...
uint32_t ef_release_vassal_this_province(EFFECT_PARAMTERS) {
	...
	auto hprovs = ws.world.nation_get_province_ownership(holder);
	if(hprovs.begin() == hprovs.end()) {
		nations::release_nation_from(ws, ws.world.nation_get_identity_from_identity_holder(holder), trigger::to_nation(primary_slot));
		ws.world.force_create_overlord(holder, trigger::to_nation(primary_slot));
	} else {
		auto rel = ws.world.nation_get_overlord_as_subject(holder);
		if(rel) {
...
uint32_t ef_release_vassal_reb(EFFECT_PARAMTERS) {
	...
	auto hprovs = ws.world.nation_get_province_ownership(holder);
	if(hprovs.begin() == hprovs.end()) {
		nations::release_nation_from(ws, itag, trigger::to_nation(primary_slot));
		ws.world.force_create_overlord(holder, trigger::to_nation(primary_slot));
	} else {
		auto rel = ws.world.nation_get_overlord_as_subject(holder);
		if(rel) {
...
uint32_t ef_add_war_goal(EFFECT_PARAMTERS) {
	if(auto w = military::find_war_between(ws, trigger::to_nation(primary_slot), trigger::to_nation(from_slot)); w) {
		military::add_wargoal(ws, w, trigger::to_nation(from_slot), trigger::to_nation(primary_slot), trigger::payload(tval[1]).cb_id, dcon::state_definition_id{}, dcon::national_identity_id{});
	}
	return 0;
}
...
uint32_t ef_build_fort_in_capital_yes_whole_state_yes_limit(EFFECT_PARAMTERS) {
	...
	auto cs = ws.world.province_get_state_membership(c);
	province::for_each_province_in_state_instance(ws, cs, [&](dcon::province_id p) {
		ws.world.province_get_fort_level(p) += uint8_t(1);
	});
	return 0;
}
