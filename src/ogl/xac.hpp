		...
		case xac_chunk_type::object: {
			...
			for(uint32_t i = 0; i < cd.vertex_block_count; ++i) {
				...
				case xac_vertex_block_type::normal: {
					auto vbd = *reinterpret_cast<const xac_vector3f*>(start); 
					start += sizeof(vbd);
				...
				case xac_vertex_block_type::texcoord: {
					auto vbd = *reinterpret_cast<const xac_vector2f*>(start); 
					start += sizeof(vbd);
				...
				case xac_vertex_block_type::weight: {
					auto vbd = *reinterpret_cast<const xac_vector4f*>(start); 
					start += sizeof(vbd);
