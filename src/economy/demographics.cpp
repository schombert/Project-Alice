...
dcon::demographics_key to_key(sys::state const& state, dcon::pop_type_id v) {
	return dcon::demographics_key(dcon::pop_demographics_key::value_base_t(count_special_keys + state.world.ideology_size() +
																		   state.world.issue_option_size() + v.index()));
}
dcon::demographics_key to_key(sys::state const& state, dcon::culture_id v) {
	return dcon::demographics_key(dcon::pop_demographics_key::value_base_t(
		count_special_keys + state.world.ideology_size() + state.world.issue_option_size() + state.world.pop_type_size() + v.index()));
}
...
void regenerate_from_pop_data(sys::state& state) {
	...
	concurrency::parallel_for(uint32_t(0), size(state), [&](uint32_t index) {
		...
		if(index < count_special_keys) {
			...
			dcon::religion_id pkey{dcon::religion_id::value_base_t(index - (count_special_keys + state.world.ideology_size() +
		...
		case 0: {
			...
			state.world.for_each_culture([&](dcon::culture_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()), [&](auto p) {
					...
					auto mask = v > old_max;
					state.world.province_set_dominant_culture(
						p, ve::select(mask, ve::tagged_vector<dcon::culture_id>(c), state.world.province_get_dominant_culture(p)));
		...
		case 1: {
			...
			state.world.for_each_culture([&](dcon::culture_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_culture(
						p, ve::select(mask, ve::tagged_vector<dcon::culture_id>(c), state.world.state_instance_get_dominant_culture(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 2: {
			...
			state.world.for_each_culture([&](dcon::culture_id c) {
				state.world.execute_serial_over_nation([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.nation_set_dominant_culture(
						p, ve::select(mask, ve::tagged_vector<dcon::culture_id>(c), state.world.nation_get_dominant_culture(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 3: {
			...
			state.world.for_each_religion([&](dcon::religion_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()), [&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.province_set_dominant_religion(
						p, ve::select(mask, ve::tagged_vector<dcon::religion_id>(c), state.world.province_get_dominant_religion(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 4: {
			...
			state.world.for_each_religion([&](dcon::religion_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_religion(p, ve::select(mask, ve::tagged_vector<dcon::religion_id>(c),
		...
		case 5: {
			...
			state.world.for_each_religion([&](dcon::religion_id c) {
				state.world.execute_serial_over_nation([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.nation_set_dominant_religion(
						p, ve::select(mask, ve::tagged_vector<dcon::religion_id>(c), state.world.nation_get_dominant_religion(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 6: {
			...
			state.world.for_each_ideology([&](dcon::ideology_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()), [&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.province_set_dominant_ideology(
						p, ve::select(mask, ve::tagged_vector<dcon::ideology_id>(c), state.world.province_get_dominant_ideology(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 7: {
			...
			state.world.for_each_ideology([&](dcon::ideology_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_ideology(p, ve::select(mask, ve::tagged_vector<dcon::ideology_id>(c),
		...
		case 8: {
			...
			state.world.for_each_ideology([&](dcon::ideology_id c) {
				state.world.execute_serial_over_nation([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.nation_set_dominant_ideology(
						p, ve::select(mask, ve::tagged_vector<dcon::ideology_id>(c), state.world.nation_get_dominant_ideology(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 9: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()), [&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.province_set_dominant_issue_option(p, ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c),
		...
		case 10: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_issue_option(
						p, ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c),
		...
		case 11: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				state.world.execute_serial_over_nation([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.nation_set_dominant_issue_option(p, ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c),
		...
		case 12: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				state.world.execute_serial_over_pop([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.pop_set_dominant_issue_option(
						p, ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c), state.world.pop_get_dominant_issue_option(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
		...
		case 13: {
			...
			state.world.for_each_ideology([&](dcon::ideology_id c) {
				state.world.execute_serial_over_pop([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.pop_set_dominant_ideology(
						p, ve::select(mask, ve::tagged_vector<dcon::ideology_id>(c), state.world.pop_get_dominant_ideology(p)));
					max_buffer.set(p, ve::select(mask, v, old_max));
				});
			});
			break;
...
void update_militancy(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto en_mod_a =
			ve::min(0.0f, (state.world.pop_get_everyday_needs_satisfaction(ids) - 0.5)) * state.defines.mil_lack_everyday_need;
		...
		state.world.pop_set_militancy(
			ids, ve::min(ve::max(0.0f, ve::select(owner != dcon::nation_id{},
...
void update_consciousness(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto lit_mod = (state.world.nation_get_plurality(owner) / 10000.0f) *
		...
		state.world.pop_set_consciousness(
			ids, ve::min(ve::max(ve::select(owner != dcon::nation_id{}, ((old_con + lx_mod) + (cl_mod + lit_mod)) + (local_mod + sep_mod),
									 0.0f),
							 0.0f),
					 10.f));
	});
...
void update_literacy(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto cmod = ve::min(1.0f, (cfrac - state.defines.base_clergy_for_literacy) /
		...
		auto new_lit =
			ve::min(ve::max(old_lit + (0.01f * state.defines.literacy_change_speed) * ((espending * cmod) * (tmod * nmod)), 0.01f), 1.0f);
...
float get_estimated_literacy_change(sys::state& state, dcon::nation_id n) {
	...
	for(auto prov_control : fat_id.get_province_control()) {
		...
		sum += (0.01f * state.defines.literacy_change_speed * state.world.nation_get_education_spending(n) *
				((prov.get_demographics(demographics::to_key(state, state.culture_definitions.clergy)) /
						 prov.get_demographics(demographics::total) -
					 state.defines.base_clergy_for_literacy) /
					(state.defines.max_clergy_for_literacy - state.defines.base_clergy_for_literacy)) *
				((state.world.nation_get_modifier_values(n, sys::national_mod_offsets::education_efficiency) + 1.0f) *
					(state.world.nation_get_modifier_values(n, sys::national_mod_offsets::education_efficiency_modifier) + 1.0f)));
	}
	return sum;
}
	...
	state.world.for_each_ideology([&](dcon::ideology_id i) {
		if(state.world.ideology_get_enabled(i)) {
			...
			execute_staggered_blocks(offset, divisions, std::min(state.world.pop_size(), pbuf.size), [&](auto ids) {
				...
				state.world.pop_set_demographics(
					ids, i_key,
	...
	*/
	...
	state.world.for_each_issue_option([&](dcon::issue_option_id iid) {
		...
		pexecute_staggered_blocks(offset, divisions, new_pop_count, [&](auto ids) {
			...
			auto amount =
				owner_modifier * ve::select(allowed_by_owner,
									 ve::apply(
										 [&](dcon::pop_id pid, dcon::pop_type_id ptid, dcon::nation_id o) {
											 if(auto mtrigger = state.world.pop_type_get_issues(ptid, iid); mtrigger) {
												 return trigger::evaluate_multiplicative_modifier(
													 state, mtrigger, trigger::to_generic(pid), trigger::to_generic(o), 0);
											 } else {
												 return 0.0f;
											 }
										 },
		...
		execute_staggered_blocks(offset, divisions, std::min(state.world.pop_size(), pbuf.size), [&](auto ids) {
			...
			state.world.pop_set_demographics(ids, i_key,
				ve::select(ttotal > 0.0f,
					issues_change_rate * owner_rate_modifier * avalue +
						(1.0f - issues_change_rate * owner_rate_modifier) * current,
					current));
		});
	});
}
...
void update_growth(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		state.world.pop_set_size(
			ids, ve::select((owner != dcon::nation_id{}) && (type != state.culture_definitions.slaves), new_size, old_size));
	});
}
			...
			[&](dcon::pop_id p, dcon::nation_id owner, float promotion_chance, float demotion_chance) {
				...
				*/
				...
				float amount = std::min(
					current_size, promoting ? (std::ceil(promotion_chance * state.world.nation_get_administrative_efficiency(owner) *
				...
				*/
				...
				if(promoting && promoted_type && state.world.pop_type_get_strata(promoted_type) >= strata &&
					...
					if(promote_mod) {
						auto chance = trigger::evaluate_additive_modifier(state, promote_mod, trigger::to_generic(p),
	...
	pexecute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		*/
		...
		auto amounts = ve::max(trigger::evaluate_additive_modifier(state, state.culture_definitions.migration_chance,
								   trigger::to_generic(ids), trigger::to_generic(owners), 0),
						   0.0f) *
					   pop_sizes * (state.world.province_get_modifier_values(loc, sys::provincial_mod_offsets::immigrant_push) + 1.0f) *
	...
	{ // initial issues
		...
		state.world.for_each_issue_option([&](dcon::issue_option_id iid) {
			...
			auto allowed_by_owner =
				...
				(!state.world.issue_get_is_next_step_only(parent_issue) || (current_issue_setting.id.index() == iid.index() - 1) ||
		...
		ve::apply(
			[&](dcon::pop_id p) {
				if(pbuf.amounts.get(p) > 0.0f && pbuf.types.get(p)) {
					auto target_pop =
						impl::find_or_make_pop(state, state.world.pop_get_province_from_pop_location(p), state.world.pop_get_culture(p),
							state.world.pop_get_religion(p), pbuf.types.get(p));
