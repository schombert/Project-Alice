...
void regenerate_from_pop_data(sys::state& state) {
		...
		if(index < count_special_keys) {
			...
			case 0: // constexpr inline dcon::demographics_key total(0);
				sum_over_demographics(state, key,
					[](sys::state const& state, dcon::pop_id p) { return state.world.pop_get_size(p); });
				break;
			case 1: // constexpr inline dcon::demographics_key employable(1);
				sum_over_demographics(state, key, [](sys::state const& state, dcon::pop_id p) {
					return state.world.pop_type_get_has_unemployment(state.world.pop_get_poptype(p)) ? state.world.pop_get_size(p)
																									 : 0.0f;
				});
				break;
			...
			dcon::issue_option_id pkey{
				dcon::issue_option_id::value_base_t(index - (count_special_keys + state.world.ideology_size()))};
			auto pdemo_key = pop_demographics::to_key(state, pkey);
			sum_over_demographics(state, key, [pdemo_key](sys::state const& state, dcon::pop_id p) {
				return state.world.pop_get_demographics(p, pdemo_key) * state.world.pop_get_size(p);
			});
	...
	concurrency::parallel_for(uint32_t(0), uint32_t(16), [&](uint32_t index) {
		switch(index) {
		case 0: {
			static ve::vectorizable_buffer<float, dcon::province_id> max_buffer =
			...
			state.world.for_each_culture([&](dcon::culture_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()),
					[&](auto p) {
						...
						auto mask = v > old_max;
						state.world.province_set_dominant_culture(p, ve::select(mask, ve::tagged_vector<dcon::culture_id>(c),
		...
		case 1: {
			...
			state.world.for_each_culture([&](dcon::culture_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_culture(p, ve::select(mask, ve::tagged_vector<dcon::culture_id>(c),
			...
			state.world.for_each_religion([&](dcon::religion_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()),
					[&](auto p) {
						...
						auto old_max = max_buffer.get(p);
						auto mask = v > old_max;
						state.world.province_set_dominant_religion(p, ve::select(mask, ve::tagged_vector<dcon::religion_id>(c),
			...
			state.world.for_each_ideology([&](dcon::ideology_id c) {
				ve::execute_serial<dcon::province_id>(uint32_t(state.province_definitions.first_sea_province.index()),
					[&](auto p) {
						...
						auto old_max = max_buffer.get(p);
						auto mask = v > old_max;
						state.world.province_set_dominant_ideology(p, ve::select(mask, ve::tagged_vector<dcon::ideology_id>(c),
		...
		case 10: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				state.world.execute_serial_over_state_instance([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.state_instance_set_dominant_issue_option(p,
						ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c),
		...
		case 12: {
			...
			state.world.for_each_issue_option([&](dcon::issue_option_id c) {
				state.world.execute_serial_over_pop([&](auto p) {
					...
					auto old_max = max_buffer.get(p);
					auto mask = v > old_max;
					state.world.pop_set_dominant_issue_option(p, ve::select(mask, ve::tagged_vector<dcon::issue_option_id>(c),
		...
		case 15: {
			...
			state.world.for_each_state_instance(
				[&, k = demographics::to_key(state, state.culture_definitions.bureaucrat)](dcon::state_instance_id s) {
					if(!state.world.province_get_is_colonial(state.world.state_instance_get_capital(s))) {
						auto location = state.world.state_instance_get_nation_from_state_ownership(s);
						state.world.nation_get_non_colonial_bureaucrats(location) +=
...
void update_militancy(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto lx_mod =
			ve::max(state.world.pop_get_luxury_needs_satisfaction(ids) - 0.5f, 0.0f) * state.defines.mil_has_luxury_need;
		...
		auto ruling_sup = ve::apply(
			[&](dcon::pop_id p, dcon::ideology_id i) {
				return i ? state.world.pop_get_demographics(p, pop_demographics::to_key(state, i)) *
		...
		auto en_mod_b =
			ve::max(0.0f, (state.world.pop_get_everyday_needs_satisfaction(ids) - 0.5)) * state.defines.mil_has_everyday_need;
...
void update_consciousness(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		auto loc = state.world.pop_get_province_from_pop_location(ids);
		auto owner = state.world.province_get_nation_from_province_ownership(loc);
		auto cfrac = state.world.province_get_demographics(loc, clergy_key) /
					 state.world.province_get_demographics(loc, demographics::total);
		...
		auto lit_mod =
			(state.world.nation_get_plurality(owner) / 10000.0f) *
		...
		state.world.pop_set_consciousness(ids,
			ve::min(ve::max(ve::select(owner != dcon::nation_id{},
								((old_con + lx_mod) + (cl_mod + lit_mod)) + (local_mod + sep_mod), 0.0f),
						0.0f),
				10.f));
	});
}
...
void update_literacy(sys::state& state, uint32_t offset, uint32_t divisions) {
	...
	*/
	...
	execute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto espending = (ve::to_float(state.world.nation_get_education_spending(owner)) / 100.0f) *
	...
	for(auto prov_control : fat_id.get_province_control()) {
		...
		sum += (0.01f * state.defines.literacy_change_speed * state.world.nation_get_education_spending(n) *
				...
				((state.world.nation_get_modifier_values(n, sys::national_mod_offsets::education_efficiency) + 1.0f) *
					(state.world.nation_get_modifier_values(n, sys::national_mod_offsets::education_efficiency_modifier) +
						1.0f)));
	}
	return sum;
}
...
float get_estimated_con_change(sys::state& state, dcon::nation_id n) {
	// TODO - We want this to return the estimated monthly change for conciousness, note the returned value is already divided by
	// 30
	return 0.0001f;
}
...
void update_issues(sys::state& state, uint32_t offset, uint32_t divisions, issues_buffer& ibuf) {
	...
	*/
	...
	state.world.for_each_issue_option([&](dcon::issue_option_id iid) {
		...
		auto modifier_key = is_social_issue ? sys::national_mod_offsets::social_reform_desire
											: sys::national_mod_offsets::political_reform_desire;
		...
		ve::apply(
			[&](dcon::pop_id p, dcon::nation_id owner, float promotion_chance, float demotion_chance) {
				...
				*/
				...
				float amount = std::min(current_size,
					promoting ? (std::ceil(promotion_chance * state.world.nation_get_administrative_efficiency(owner) *
				...
				*/
				...
				bool is_state_capital =
					state.world.state_instance_get_capital(state.world.province_get_state_membership(loc)) == loc;
...
void update_assimilation(sys::state& state, uint32_t offset, uint32_t divisions, assimilation_buffer& pbuf) {
	...
	*/
	...
	pexecute_staggered_blocks(offset, divisions, state.world.pop_size(), [&](auto ids) {
		...
		auto assimilation_chances =
			ve::max(trigger::evaluate_additive_modifier(state, state.culture_definitions.assimilation_chance,
						trigger::to_generic(ids), trigger::to_generic(owners), 0),
				0.0f);
	...
	for(auto loc : state.world.nation_get_province_ownership(n)) {
		if(loc.get_province().get_is_colonial() == false) {
			if(!limit_to_capitals || loc.get_province().get_state_membership().get_capital().id == loc.get_province().id) {
				auto weight = trigger::evaluate_multiplicative_modifier(state, modifier,
								  trigger::to_generic(loc.get_province().id), trigger::to_generic(p), 0) *
...
dcon::pop_id find_or_make_pop(sys::state& state, dcon::province_id loc, dcon::culture_id cid, dcon::religion_id rid,
	...
	{ // initial issues
		...
		state.world.for_each_issue_option([&](dcon::issue_option_id iid) {
			...
			if(allowed_by_owner) {
				if(auto mtrigger = state.world.pop_type_get_issues(ptid, iid); mtrigger) {
					auto amount = owner_modifier * trigger::evaluate_multiplicative_modifier(state, mtrigger,
													   trigger::to_generic(np.id), trigger::to_generic(owner), 0);
