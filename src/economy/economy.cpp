...
float average_capitalists_luxury_cost(
	...
	state.world.nation_for_each_state_ownership(s, [&](auto soid) {
		...
		total += state.world.market_get_luxury_needs_costs(
			market, state.culture_definitions.capitalists
		) * local_count / state.defines.alice_needs_scaling_factor;
		count += local_count;
	});
...
void register_demand(
	...
	if(amount > 0.4 && price(state, s, commodity_type) > 100'000.f) {
		bool something_is_wrong = true;
	}
...
template<typename T>
ve::fp_vector artisan_input_multiplier(
	sys::state& state,
	T nations
) {
...
template<typename T>
ve::fp_vector artisan_output_multiplier(
	sys::state& state,
	T nations
) {
...
template<typename T>
ve::fp_vector artisan_throughput_multiplier(
	sys::state& state,
	T nations
) {
...
void initialize(sys::state& state) {
	...
	state.world.for_each_pop([&](dcon::pop_id p) {
		...
		fp.set_life_needs_satisfaction(0.0f);
...
void update_rgo_employment(sys::state& state) {
	province::for_each_land_province(state, [&](dcon::province_id p) {
		...
		std::sort(ordered_rgo_goods.begin(), ordered_rgo_goods.end(), [&](dcon::commodity_id a, dcon::commodity_id b) {
			float profit_a = rgo_expected_worker_norm_profit(state, p, m, owner, a);
			float profit_b = rgo_expected_worker_norm_profit(state, p, m, owner, b);
...
float rgo_efficiency(sys::state & state, dcon::nation_id n, dcon::province_id p, dcon::commodity_id c) {
	...
	float saturation = std::min(1.f, state.world.province_get_rgo_employment_per_good(p, c)
...
float factory_throughput_multiplier(sys::state& state, dcon::factory_type_fat_id fac_type, dcon::nation_id n, dcon::province_id p, dcon::state_instance_id s) {
	return production_throughput_multiplier * (
...
float full_spending_cost(sys::state& state, dcon::nation_id n) {
	...
	state.world.nation_for_each_state_ownership(n, [&](auto soid) {
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			dcon::commodity_id cid{dcon::commodity_id::value_base_t(i)};
			total += state.world.market_get_construction_demand(local_market, cid)
				* c_spending * price(state, local_market, cid);
		}
	});
...
void update_national_consumption(sys::state& state, dcon::nation_id n, float spending_scale, float private_investment_scale) {
	...
	state.world.nation_for_each_state_ownership(n, [&](auto soid) {
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			auto sat_importance = std::min(1.f, 1.f / price(state, market, cid));
...
void update_pop_consumption(
	...
	state.world.execute_serial_over_pop([&](auto ids) {
		...
		auto old_life = state.world.pop_get_life_needs_satisfaction(ids);
		auto old_everyday = state.world.pop_get_everyday_needs_satisfaction(ids);
		auto old_luxury = state.world.pop_get_luxury_needs_satisfaction(ids);
...
void advance_construction(sys::state& state, dcon::nation_id n) {
	...
	float national_money = state.world.nation_get_stockpiles(n, economy::money);

	// we want construction to drive prices up as long as price is sane
	// that is, if we can actually afford paying it
	...
	state.world.nation_for_each_state_ownership(n, [&](auto soid) {
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			auto payment_coefficient =
				std::min(1.f,
				std::max(0.f,
					nat_demand * com_price / (national_money * 0.1f + 1.f)
				));
			...
			auto sat = state.world.market_get_demand_satisfaction(market, c);
			auto sat_importance = std::min(1.f, 1.f / com_price);
			auto sat_coefficient = (sat_importance + (1.f - sat_importance) * sat);
			nat_demand *= c_spending * sat_coefficient * (1.f - payment_coefficient);

			state.world.market_get_private_construction_demand(market, c)
				*= p_spending * sat_coefficient;
		}
	});
...
void daily_update(sys::state& state, bool presimulation) {
	...
	*/
	...
	concurrency::parallel_for(uint32_t(0), total_commodities, [&](uint32_t k) {
		...
		state.world.for_each_trade_route([&](auto trade_route) {
			...
			auto at_war = military::are_at_war(state, n_origin, n_target);

			auto origin_sphere = state.world.nation_get_in_sphere_of(n_origin);
			auto target_sphere = state.world.nation_get_in_sphere_of(n_target);

			auto import_tariff = effective_tariff_rate(state, n_target);
			auto export_tariff = effective_tariff_rate(state, n_origin);

			auto is_origin_civ = state.world.nation_get_is_civilized(n_target);
			auto is_target_civ = state.world.nation_get_is_civilized(n_target);
			...
			if(n_origin == target_sphere) {
				import_tariff = 0.f;
			}
			if(n_target == origin_sphere) {
				export_tariff = 0.f;
			}
			...
			auto transport_cost = distance * 0.4f;
			// effect of scale
			// volume reduces transport costs
			auto effect_of_scale = std::max(0.1f, 1.f - absolute_volume * 0.001f);
			...
			auto current_profit = price_target - price_origin * 1.05f - transport_cost * effect_of_scale;
	...
	*/
	...
	state.world.execute_serial_over_market([&](auto ids) {
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			// might be negative
			auto change_cost = stockpile_change * ve_price(state, ids, c) * state.inflation;
			...
			auto bought_from_nation_cost =
				buy_from_nation * ve_price(state, ids, c) * state.inflation;
	...
	*/
	...
	state.world.execute_parallel_over_pop([&](auto ids) {
		...
		ve::apply([](float v) { assert(std::isfinite(v) && v >= 0); }, state.world.pop_get_savings(ids));
	});
...

				{
					...
					for(auto pl : state.world.province_get_pop_location(p)) {
						if(pl.get_pop().get_poptype().get_is_paid_rgo_worker()) {
							pl.get_pop().set_savings(
								pl.get_pop().get_savings()
								+ state.inflation * pl.get_pop().get_size() * per_worker_profit);
							assert(
								std::isfinite(pl.get_pop().get_savings())
								&& pl.get_pop().get_savings() >= 0
							);
	...
	*/
	state.world.execute_serial_over_market([&](auto ids) {
		state.world.for_each_commodity([&](dcon::commodity_id c) {
			...
			state.world.market_set_price(ids, c, life_cost * state.world.commodity_get_cost(c) * state.inflation * state.inflation);
		});
	});
	...
	state.world.execute_serial_over_market([&](auto ids) {
		concurrency::parallel_for(uint32_t(0), total_commodities, [&](uint32_t k) {
			...
			state.world.market_set_price(ids, cid, ve::min(ve::max(current_price, 0.0001f), 1'000'000'000'000.f));
		});
	});
	...
	*/
	...
	// we want essentially normalise the vector of money
	// possible candidates:
	// l1 norm: just sum up all money
	// l2 norm: euclidian norm
	// while l1 norm is easy to calculate,
	// it leads to undesired effect
	// of gathering money in the "corner"
	// of high dimenstional polytop
	// but l2 norm is expensive
	// so we calculate combined norm:
	// l2 norm of vector of l1 norms per nation.
	...
	float target_money = 10.f * total_pop * primary_commodity_basket / state.defines.alice_needs_scaling_factor;
	...
	state.inflation =
		(0.5f * state.inflation)
		+ (0.5f * target_money / (total_pop_money + 0.0001f));
...
float estimate_construction_spending(sys::state& state, dcon::nation_id n) {
			...
			for(uint32_t i = 0; i < commodity_set::set_size; ++i) {
				if(base_cost.commodity_type[i]) {
					if(current_purchased.commodity_amounts[i] < base_cost.commodity_amounts[i] * admin_cost_factor)
						...
						* state.world.market_get_demand_satisfaction(market, base_cost.commodity_type[i])
						* base_cost.commodity_amounts[i]
						/ construction_time;
				} else {
					break;
	...
	for(auto po : state.world.nation_get_province_ownership(n)) {
		...
		auto local_state = state.world.province_get_state_membership(p);
		auto market = state.world.state_instance_get_market_from_local_market(local_state);
	...
	for(auto c : state.world.nation_get_province_building_construction(n)) {
		auto local_state = c.get_province().get_state_membership();
		auto market = local_state.get_market_from_local_market();
