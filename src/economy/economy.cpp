...
void update_single_factory_consumption(sys::state& state, dcon::factory_id f, dcon::nation_id n, dcon::province_id p,
	...
	bool profitable = ((0.75f + 0.25f * min_e_input_available) * output_total * throughput_multiplier * output_multiplier *
							  min_input_available -
	...
	if(profitable || state.world.factory_get_subsidized(f)) {
		...
		state.world.factory_set_actual_production(f,
			fac_type.get_output_amount() * throughput_multiplier * output_multiplier * effective_production_scale);
		...
		state.world.factory_set_production_scale(f,
			std::max(0.0f, state.world.factory_get_production_scale(f) - production_scale_delta));
	...
	if(production > 0) {
		...
		for(uint32_t i = 0; i < commodity_set::set_size; ++i) {
			if(fac_type.get_inputs().commodity_type[i]) {
				min_input =
					std::min(min_input, state.world.nation_get_demand_satisfaction(n, fac_type.get_inputs().commodity_type[i]));
			} else {
				break;
			}
		}
...
void update_province_rgo_consumption(sys::state& state, dcon::province_id p, dcon::nation_id n, float mobilization_impact,
	...
	auto pop_wages =
		std::min(pops_max, state.world.province_get_demographics(p,
							   demographics::to_employment_key(state,
								   is_mine ? state.culture_definitions.laborers : state.culture_definitions.farmers))) *
...
void update_province_artisan_consumption(sys::state& state, dcon::province_id p, dcon::nation_id n,
	...
	float artisan_pop = std::max(0.01f,
		state.world.province_get_demographics(p, demographics::to_key(state, state.culture_definitions.artisans)));
	...
	if(profitable) {
		...
		state.world.province_set_artisan_actual_production(p, state.world.commodity_get_artisan_output_amount(artisan_prod_type) *
																  throughput_multiplier * output_multiplier *
...
void populate_construction_consumption(sys::state& state) {
	...
	for(auto lc : state.world.in_province_land_construction) {
		...
		if(owner && state.world.province_get_nation_from_province_control(province) == owner) {
			...
			float construction_time = float(
				state.military_definitions.unit_base_definitions[state.world.province_land_construction_get_type(lc)].build_time);
	...
	for(auto c : state.world.in_province_building_construction) {
		...
		if(owner &&
			c.get_province().get_nation_from_province_ownership() == c.get_province().get_nation_from_province_control() &&
...
float full_spending_cost(sys::state& state, dcon::nation_id n,
	...
	auto overseas_factor = state.defines.province_overseas_penalty * float(state.world.nation_get_owned_province_count(n) -
	...
	if(overseas_factor > 0) {
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			auto kf = state.world.commodity_get_key_factory(cid);
			if(state.world.commodity_get_overseas_penalty(cid) && (state.world.commodity_get_is_available_from_start(cid) ||
	...
	state.world.for_each_pop_type([&](dcon::pop_type_id pt) {
		...
		if(ln_type == culture::income_type::administration) {
			...
			if(state.world.pop_type_get_has_unemployment(pt)) {
				auto emp =
					state.world.nation_get_demographics(n, demographics::to_employment_key(state, pt)) / needs_scaling_factor;
...
void update_national_consumption(sys::state& state, dcon::nation_id n,
	...
	for(uint32_t i = 1; i < total_commodities; ++i) {
		...
		state.world.nation_get_real_demand(n, cid) +=
			state.world.nation_get_construction_demand(n, cid) * c_spending * spending_scale;
		assert(std::isfinite(state.world.nation_get_real_demand(n, cid)));
	}
	for(uint32_t i = 1; i < total_commodities; ++i) {
		dcon::commodity_id cid{dcon::commodity_id::value_base_t(i)};
...
void update_pop_consumption(sys::state& state, dcon::nation_id n, dcon::province_id p, float base_demand,
	...
	bool nation_allows_investment = state.world.nation_get_is_civilized(n) &&
	...
	for(auto pl : state.world.province_get_pop_location(p)) {
		...
		float luxury_needs_fraction = [&]() {
			if(!nation_allows_investment ||
				(t != state.culture_definitions.aristocrat && t != state.culture_definitions.capitalists))
	...
	state.world.for_each_pop_type([&](dcon::pop_type_id t) {
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			if(state.world.commodity_get_is_available_from_start(cid) || (kf && state.world.nation_get_active_building(n, kf))) {
				...
				state.world.nation_get_real_demand(n, cid) += state.world.pop_type_get_everyday_needs(t, cid) *
															  en_demand_vector.get(t) * base_demand * invention_factor *
															  en_mul[strata];

				state.world.nation_get_real_demand(n, cid) += state.world.pop_type_get_luxury_needs(t, cid) *
															  lx_demand_vector.get(t) * base_demand * invention_factor *
															  lx_mul[strata];
	...
	*/
	...
	}
	{
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			if(state.world.commodity_get_is_available_from_start(c) || (kf && state.world.nation_get_active_building(n, kf))) {
				state.world.for_each_pop_type([&](auto ids) {
					auto ln = state.world.pop_type_get_everyday_needs(ids, c) * effective_prices.get(c) * base_demand *
							  invention_factor * mul[state.world.pop_type_get_strata(ids)];
	...
	}
	{
		...
		for(uint32_t i = 1; i < total_commodities; ++i) {
			...
			if(state.world.commodity_get_is_available_from_start(c) || (kf && state.world.nation_get_active_building(n, kf))) {
				state.world.for_each_pop_type([&](auto ids) {
					auto ln = state.world.pop_type_get_luxury_needs(ids, c) * effective_prices.get(c) * base_demand *
							  invention_factor * mul[state.world.pop_type_get_strata(ids)];
			...
			for(uint32_t i = 0; i < commodity_set::set_size; ++i) {
				if(base_cost.commodity_type[i]) {
					if(current_purchased.commodity_amounts[i] < base_cost.commodity_amounts[i] * factory_mod) {
						...
						auto& source = state.world.nation_get_construction_demand(n, base_cost.commodity_type[i]);
						auto delta =
							std::max(0.0f, std::min(source, base_cost.commodity_amounts[i] * factory_mod / construction_time));
		...
		if(global_price_multiplier >= 1.0f) { // prefer domestic
			for(uint32_t i = 1; i < total_commodities; ++i) {
				...
				auto domestic_supply =
					state.world.nation_get_domestic_market_pool(n, c) +
		...
		*/
		...
		for(auto p : state.world.nation_get_province_ownership(n)) {
			for(auto f : state.world.province_get_factory_location(p.get_province())) {
				...
				update_single_factory_consumption(state, f.get_factory(), n, p.get_province(),
					p.get_province().get_state_membership(), effective_prices, mobilization_impact, factory_min_wage,
			...
			// artisan
			update_province_artisan_consumption(state, p.get_province(), n, effective_prices, mobilization_impact,
				artisan_min_wage, p.get_province().get_nation_from_province_control() != n);
	...
	state.world.execute_parallel_over_pop([&](auto ids) {
		...
		auto en_costs =
			ve::apply([&](dcon::pop_type_id pt,
						  dcon::nation_id n) { return pt ? state.world.nation_get_everyday_needs_costs(n, pt) : 0.0f; },
				types, owners);
		...
		auto acc_e =
			ve::select(ln_types == int32_t(culture::income_type::education), e_spending * adj_pop_of_type * ln_costs, 0.0f);
		auto acc_m =
			ve::select(ln_types == int32_t(culture::income_type::military), m_spending * adj_pop_of_type * ln_costs, 0.0f);
		...
		acc_a = acc_a + ve::select(en_types == int32_t(culture::income_type::administration),
							a_spending * adj_pop_of_type * en_costs, 0.0f);
		acc_e = acc_e +
				ve::select(en_types == int32_t(culture::income_type::education), e_spending * adj_pop_of_type * en_costs, 0.0f);
		acc_m = acc_m +
				ve::select(en_types == int32_t(culture::income_type::military), m_spending * adj_pop_of_type * en_costs, 0.0f);

		acc_a = acc_a + ve::select(lx_types == int32_t(culture::income_type::administration),
							a_spending * adj_pop_of_type * lx_costs, 0.0f);
		acc_e = acc_e +
				ve::select(lx_types == int32_t(culture::income_type::education), e_spending * adj_pop_of_type * lx_costs, 0.0f);
		acc_m = acc_m +
				ve::select(lx_types == int32_t(culture::income_type::military), m_spending * adj_pop_of_type * lx_costs, 0.0f);
		...
		for(auto p : state.world.nation_get_province_ownership(n)) {
			...
			*/

			{
				...
				float num_artisans =
					state.world.province_get_demographics(p.get_province(), demographics::to_key(state, artisan_type));
				if(num_artisans > 0) {
	...
	state.world.for_each_commodity([&](dcon::commodity_id c) {
		state.world.for_each_pop_type([&](dcon::pop_type_id pt) {
			primary_commodity_basket +=
				0.7f * state.world.commodity_get_current_price(c) * state.world.pop_type_get_life_needs(pt, c);
			primary_commodity_basket +=
	...
	for(auto n : state.world.in_nation) {
		...
		if(n.get_private_investment() > 0.001 &&
			...
			if(!found_investment) {
				...
				std::sort(states_in_order.begin(), states_in_order.end(),
						...
						if(p.get_province().get_nation_from_province_ownership() == n) {
							for(auto f : p.get_province().get_factory_location()) {
								...
								if((nation_rules & issue_rule::pop_expand_factory) != 0 &&
									...
									if(auto new_p = f.get_factory().get_full_profit() / f.get_factory().get_level();
										new_p > profit) {
										profit = new_p;
...
float unit_construction_progress(sys::state& state, dcon::province_naval_construction_id c) {
	auto& goods =
		state.military_definitions.unit_base_definitions[state.world.province_naval_construction_get_type(c)].build_cost;
...
void bound_budget_settings(sys::state& state, dcon::nation_id n) {
	...
	{
		auto min_spend =
			int32_t(100.0f * state.world.nation_get_modifier_values(n, sys::national_mod_offsets::min_social_spending));
		auto max_spend =
			int32_t(100.0f * state.world.nation_get_modifier_values(n, sys::national_mod_offsets::max_social_spending));
		if(max_spend <= 0)
			max_spend = 100;
		max_spend = std::max(min_spend, max_spend);
