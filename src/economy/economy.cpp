...
int32_t previous_price_record_index(sys::state& state) { return ((state.current_date.value >> 4) - 1) % 32; }
...
void update_single_factory_production(sys::state& state, dcon::factory_id f, dcon::nation_id n, float expected_min_wage) {
	...
	if(production > 0) {
		...
		if(!fac.get_subsidized()) {
			...
			float min_wages = expected_min_wage * fac.get_level() * fac.get_primary_employment() * (factory_per_level_employment / needs_scaling_factor);
	...
	concurrency::parallel_for(uint32_t(0), state.world.nation_size(), [&](uint32_t i) {
		...
		*/
		...
		}
		{
			...
			state.world.nation_set_effective_land_spending(n, nations_commodity_spending * max_sp * float(state.world.nation_get_land_spending(n)) / 100.0f);
		}
		{
			float max_sp = 0.0f;
			float total = 0.0f;
			...
			state.world.nation_set_effective_construction_spending(n, nations_commodity_spending * max_sp * float(state.world.nation_get_construction_spending(n)) / 100.0f);
		}
		/*
		fill stockpiles
		*/
		...
		for(auto uni : n.get_unilateral_relationship_as_source()) {
			if(uni.get_war_subsidies()) {
				...
				if(target_m_costs <= n.get_stockpiles(money)) {
					...
					notification::post(state, notification::message{[source = n.id, target = uni.get_target().id](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_wsub_end_1", text::variable_type::x, source, text::variable_type::y, target); },
												  "msg_wsub_end_title", n.id, sys::message_setting_type::war_subsidies_end_by_nation});
					notification::post(state, notification::message{[source = n.id, target = uni.get_target().id](sys::state& state, text::layout_base& contents) { text::add_line(state, contents, "msg_wsub_end_1", text::variable_type::x, source, text::variable_type::y, target); },
												  "msg_wsub_end_title", uni.get_target().id, sys::message_setting_type::war_subsidies_end_on_nation});
				}
			}
...
float estimate_subsidy_spending(sys::state& state, dcon::nation_id n) { return state.world.nation_get_subsidies_spending(n); }
	...
	if(state.current_date < state.world.nation_get_reparations_until(n)) {
		for(auto uni : state.world.nation_get_unilateral_relationship_as_target(n)) {
			if(uni.get_reparations()) {
				...
				auto const tax_eff = std::clamp(state.defines.base_country_tax_efficiency + state.world.nation_get_modifier_values(source, sys::national_mod_offsets::tax_efficiency), 0.1f, 1.0f);
...
float estimate_diplomatic_balance(sys::state& state, dcon::nation_id n) { return estimate_war_subsidies_income(state, n) + estimate_reparations_income(state, n) - estimate_war_subsidies_spending(state, n) - estimate_reparations_spending(state, n); }
