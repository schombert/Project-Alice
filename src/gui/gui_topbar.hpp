...
public:
	void button_action(sys::state& state) noexcept override {
		auto override_and_show_tab = [&]() {
			state.ui_state.tab_background_image = tab_background_image;
			state.ui_state.tab_background_image->set_visible(state, true);
			state.ui_state.root->move_child_to_back(state.ui_state.tab_background_image);
			state.ui_state.tab_window = tab_window;
			state.ui_state.tab_window->set_visible(state, true);
			state.ui_state.root->move_child_to_back(state.ui_state.tab_window);
		};
	...
	void render(sys::state& state, int32_t x, int32_t y) noexcept override {
		frame = int32_t(state.ui_state.tab_window && state.ui_state.tab_window->is_visible());
...
public:
	...
	std::unique_ptr<element_base> make_child(sys::state& state, std::string_view name, dcon::gui_def_id id) noexcept override {
		if(name == "topbarbutton_production") {
			auto pw = make_element_by_type<topbar_tab_button>(state, id);

			auto topcolor = make_element_by_type<background_image>(state, "topcolor_production");
			topcolor->set_visible(state, false);
			pw->tab_background_image = topcolor.get();
			state.ui_state.tab_background_image = topcolor.get();
			...
			pw->tab_window = tab.get();
			state.ui_state.tab_window = tab.get(); // Default to this tab... but keep it hidden!
			...
			state.ui_state.root->add_child_to_back(std::move(tab));
			return pw;
		} else if(name == "topbarbutton_technology") {
			auto pw = make_element_by_type<topbar_tab_button>(state, id);
			...
			state.ui_state.root->add_child_to_back(std::move(tab));
			return pw;
		} else if(name == "topbarbutton_population") {
			auto pw = make_element_by_type<topbar_tab_button>(state, id);
