...
void display_value(int64_t value, bool positive_is_green, sys::state& ws, text::layout_base& layout, text::layout_box& box) {
	...
	if(value >= 0)
		text::add_to_layout_box(layout, ws, box, "+", positive_is_green ? text::text_color::green : text::text_color::red,
		                        std::monostate{});
...
uint32_t es_x_neighbor_province_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return ((tval[0] & effect::is_random_scope) != 0 ? 1 : 0) +
	       display_subeffects(ws, tval, layout, -1, this_slot, from_slot, r_lo, r_hi + ((tval[0] & effect::is_random_scope) != 0 ? 1 : 0),
	                          indentation + indentation_amount);
}
...
uint32_t es_random_list_scope(EFFECT_DISPLAY_PARAMS) {
	...
	while(sub_units_start < tval + source_size) {
		...
		rval = 1 +
		       display_subeffects(ws, tval, layout, primary_slot, this_slot, from_slot, r_hi, r_lo + 1, indentation + indentation_amount);
...
uint32_t es_owner_scope_province(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(
	    ws, tval, layout,
	    primary_slot != -1 ? trigger::to_generic(ws.world.province_get_nation_from_province_ownership(trigger::to_prov(primary_slot)))
	                       : -1,
	    this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_capital_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(
	    ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.nation_get_capital(trigger::to_nation(primary_slot))) : -1,
	    this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_overlord_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, trigger::to_generic(ws.world.overlord_get_ruler(olr)), this_slot, from_slot, r_hi, r_lo,
	                          indentation + indentation_amount);
}
uint32_t es_sphere_owner_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(
	    ws, tval, layout,
	    primary_slot != -1 ? trigger::to_generic(ws.world.nation_get_in_sphere_of((trigger::to_nation(primary_slot)))) : -1, this_slot,
...
uint32_t ef_prestige(EFFECT_DISPLAY_PARAMS) {
	...
	if(primary_slot != -1) {
		...
		auto new_prestige = std::max(0.0f, ws.world.nation_get_prestige(trigger::to_nation(primary_slot)) +
		                                       (delta > 0 ? (delta * prestige_multiplier) : delta));
