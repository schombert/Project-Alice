...
uint32_t es_x_neighbor_province_scope(EFFECT_DISPLAY_PARAMS) {
	if((tval[0] & effect::is_random_scope) != 0) {
		if(primary_slot != -1) {
			...
			if(rlist.size() != 0) {
				...
				return 1 + display_subeffects(ws, tval, layout, trigger::to_generic(rlist[r]), this_slot, from_slot, r_hi,
							   r_lo + 1, indentation + indentation_amount);
			}
			return 0;
		}
	}
	...
	return ((tval[0] & effect::is_random_scope) != 0 ? 1 : 0) +
		   display_subeffects(ws, tval, layout, -1, this_slot, from_slot, r_lo,
			   r_hi + ((tval[0] & effect::is_random_scope) != 0 ? 1 : 0), indentation + indentation_amount);
}
...
uint32_t es_controller_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout,
		primary_slot != -1
			? trigger::to_generic(ws.world.province_get_nation_from_province_control(trigger::to_prov(primary_slot)))
			: -1,
		this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_independence_scope(EFFECT_DISPLAY_PARAMS) {
	auto rtag = from_slot != -1 ? ws.world.rebel_faction_get_defection_target(trigger::to_rebel(from_slot))
...
uint32_t es_tag_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, tag_holder ? trigger::to_generic(tag_holder) : -1, this_slot, from_slot, r_hi,
		r_lo, indentation + indentation_amount);
}
...
uint32_t ef_trade_goods(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.commodity_get_name(trigger::payload(tval[1]).com_id));
...
uint32_t ef_change_tag_culture(EFFECT_DISPLAY_PARAMS) {
	{
		...
		if(primary_slot != -1) {
			...
			if(auto k = ws.key_to_text_sequence.find(std::string_view("cultural_union_nation"));
				k != ws.key_to_text_sequence.end()) {
...
uint32_t ef_pop_type(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.pop_type_get_name(trigger::payload(tval[1]).popt_id));
...
uint32_t ef_build_factory_in_capital_state(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.factory_type_get_name(trigger::payload(tval[1]).fac_id));
...
uint32_t ef_activate_technology(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.technology_get_name(trigger::payload(tval[1]).tech_id));
		text::localised_format_box(ws, layout, box, "enable_blank", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_activate_invention(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.invention_get_name(trigger::payload(tval[1]).invt_id));
		text::localised_format_box(ws, layout, box, "enable_blank", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_trigger_revolt_nation(EFFECT_DISPLAY_PARAMS) {
	...
	if(trigger::payload(tval[1]).reb_id) {
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.rebel_type_get_name(trigger::payload(tval[1]).reb_id));
	...
	if(trigger::payload(tval[4]).ideo_id) {
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.ideology_get_name(trigger::payload(tval[4]).ideo_id));
	...
	if(trigger::payload(tval[5]).cb_id) {
		{
			...
			text::substitution_map m;
			text::add_to_substitution_map(m, text::variable_type::text,
				ws.world.cb_type_get_name(trigger::payload(tval[5]).cb_id));
			text::localised_format_box(ws, layout, box, "attacker_wg_label", m);
			text::close_layout_box(layout, box);
		}
	...
	if(trigger::payload(tval[4]).cb_id) {
		{
			...
			text::substitution_map m;
			text::add_to_substitution_map(m, text::variable_type::text,
				ws.world.cb_type_get_name(trigger::payload(tval[4]).cb_id));
			text::localised_format_box(ws, layout, box, "attacker_wg_label", m);
			text::close_layout_box(layout, box);
		}
...
uint32_t ef_set_variable(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_substitution_map(m, text::variable_type::value,
			text::fp_one_place{trigger::read_float_from_payload(tval + 2)});
...
uint32_t ef_ideology(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.ideology_get_name(trigger::payload(tval[1]).ideo_id));
...
uint32_t ef_scaled_militancy_issue(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_substitution_map(m, text::variable_type::value,
			text::fp_one_place{trigger::read_float_from_payload(tval + 2)});
		text::add_to_substitution_map(m, text::variable_type::name,
			ws.world.issue_option_get_name(trigger::payload(tval[1]).opt_id));
...
uint32_t ef_scaled_militancy_unemployment(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_substitution_map(m, text::variable_type::text, std::string_view{t});
		text::add_to_substitution_map(m, text::variable_type::value,
			text::fp_one_place{trigger::read_float_from_payload(tval + 1)});
...
uint32_t ef_move_issue_percentage_nation(EFFECT_DISPLAY_PARAMS) {
	...
	}
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.issue_option_get_name(trigger::payload(tval[2]).opt_id));
		text::localised_format_box(ws, layout, box, "support_for_blank", m);
		text::add_space_to_layout_box(layout, ws, box);
		display_value(text::fp_percentage{amount}, true, ws, layout, box);
		text::close_layout_box(layout, box);
...
uint32_t ef_party_loyalty(EFFECT_DISPLAY_PARAMS) {
	...
	}
	{
		...
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text,
			ws.world.ideology_get_name(trigger::payload(tval[2]).ideo_id));
		text::localised_format_box(ws, layout, box, "blank_loyalty", m);
		text::add_space_to_layout_box(layout, ws, box);
		display_value(text::fp_percentage{amount}, true, ws, layout, box);
		text::close_layout_box(layout, box);
