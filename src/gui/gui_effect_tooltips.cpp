...
#define EFFECT_DISPLAY_PARAMS sys::state &ws, uint16_t const *tval, text::layout_base &layout, int32_t primary_slot, int32_t this_slot, int32_t from_slot, uint32_t r_lo, uint32_t r_hi, int32_t indentation
...
void display_value(int64_t value, bool positive_is_green, sys::state& ws, text::layout_base& layout, text::layout_box& box) {
	...
	if(value >= 0)
		text::add_to_layout_box(layout, ws, box, "+", positive_is_green ? text::text_color::green : text::text_color::red, std::monostate{});

	if(positive_is_green)
		text::add_to_layout_box(layout, ws, box, value, value >= 0 ? text::text_color::green : text::text_color::red);
	else
		text::add_to_layout_box(layout, ws, box, value, value >= 0 ? text::text_color::red : text::text_color::green);
...
void display_value(text::fp_currency value, bool positive_is_green, sys::state& ws, text::layout_base& layout, text::layout_box& box) {
	...
	if(positive_is_green)
		text::add_to_layout_box(layout, ws, box, value, value.value >= 0 ? text::text_color::green : text::text_color::red);
	else
		text::add_to_layout_box(layout, ws, box, value, value.value >= 0 ? text::text_color::red : text::text_color::green);
...
void tag_type_from_nation_effect(int32_t this_slot, sys::state& ws, text::layout_base& layout, text::layout_box& box) {
	text::add_to_layout_box(layout, ws, box, this_slot != -1 ? text::produce_simple_string(ws, ws.world.nation_get_name(trigger::to_nation(this_slot))) : text::produce_simple_string(ws, "from_nation"));
}
void tag_type_from_province_effect(int32_t this_slot, sys::state& ws, text::layout_base& layout, text::layout_box& box) {
	text::add_to_layout_box(layout, ws, box, this_slot != -1 ? text::produce_simple_string(ws, ws.world.nation_get_name(ws.world.province_get_nation_from_province_ownership(trigger::to_prov(this_slot)))) : text::produce_simple_string(ws, "from_nation"));
}

uint32_t es_generic_scope(EFFECT_DISPLAY_PARAMS) { return display_subeffects(ws, tval, layout, primary_slot, this_slot, from_slot, r_lo, r_hi, indentation); }

inline auto random_or_every(uint16_t tval) { return (tval & effect::is_random_scope) != 0 ? "random" : "every"; }

void show_limit(sys::state& ws, uint16_t const* tval, text::layout_base& layout, int32_t this_slot, int32_t from_slot, int32_t indentation) {
		...
		if(primary_slot != -1) {
			...
			if((tval[0] & effect::scope_has_limit) != 0) {
				...
				for(auto p : neighbor_range) {
					auto other = p.get_connected_provinces(0) == trigger::to_prov(primary_slot) ? p.get_connected_provinces(1) : p.get_connected_provinces(0);
			...
			if(rlist.size() != 0) {
				...
				text::add_to_layout_box(layout, ws, box, rlist[r]);
				text::close_layout_box(layout, box);
...
uint32_t es_x_country_scope_nation(EFFECT_DISPLAY_PARAMS) {
...

	{
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, random_or_every(tval[0])));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "nation"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_x_country_scope(EFFECT_DISPLAY_PARAMS) { return es_x_country_scope_nation(ws, tval, layout, primary_slot, this_slot, from_slot, r_lo, r_hi, indentation); }
uint32_t es_x_empty_neighbor_province_scope(EFFECT_DISPLAY_PARAMS) {
...

	{
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, random_or_every(tval[0])));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "empty_neighboring_province"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_poor_strata_scope_nation(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, random_or_every(tval[0])));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "poor_strata_pop"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_middle_strata_scope_nation(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, random_or_every(tval[0])));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "middle_strata_pop"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_rich_strata_scope_nation(EFFECT_DISPLAY_PARAMS) {
	{
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, random_or_every(tval[0])));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "rich_strata_pop"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_x_owned_scope_nation(EFFECT_DISPLAY_PARAMS) {
...

	{
		...
		if(primary_slot != -1)
			...
			text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "singular_nation"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_x_owned_scope_state(EFFECT_DISPLAY_PARAMS) {
...

	{
		...
		if(primary_slot != -1)
			...
			text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "singular_state"));
		text::close_layout_box(layout, box);
	}
	show_limit(ws, tval, layout, this_slot, from_slot, indentation);
...
uint32_t es_random_list_scope(EFFECT_DISPLAY_PARAMS) {
	...
	while(sub_units_start < tval + source_size) {
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::fp_percentage{float(*sub_units_start) / float(chances_total)});
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "chance_of"));
		text::close_layout_box(layout, box);
...
uint32_t es_random_scope(EFFECT_DISPLAY_PARAMS) {
	...
	auto box = text::open_layout_box(layout, indentation);
	text::add_to_layout_box(layout, ws, box, text::fp_percentage{float(chance) / float(100)});
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "chance_of"));
	text::close_layout_box(layout, box);
...
uint32_t es_owner_scope_state(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_owner_scope_province(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.province_get_nation_from_province_ownership(trigger::to_prov(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_controller_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.province_get_nation_from_province_control(trigger::to_prov(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_location_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.pop_get_province_from_pop_location(trigger::to_pop(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_country_scope_pop(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(nations::owner_of_pop(ws, trigger::to_pop(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_capital_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.nation_get_capital(trigger::to_nation(primary_slot))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_sea_zone_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, sea_zone ? trigger::to_generic(sea_zone) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_cultural_union_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, trigger::to_generic(group_holder), this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_overlord_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, trigger::to_generic(ws.world.overlord_get_ruler(olr)), this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_sphere_owner_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, primary_slot != -1 ? trigger::to_generic(ws.world.nation_get_in_sphere_of((trigger::to_nation(primary_slot)))) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_independence_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, r_holder ? trigger::to_generic(r_holder) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_flashpoint_tag_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, fp_nation ? trigger::to_generic(fp_nation) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_state_scope_province(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, st ? trigger::to_generic(st) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
...
uint32_t es_tag_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, tag_holder ? trigger::to_generic(tag_holder) : -1, this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_integer_scope(EFFECT_DISPLAY_PARAMS) {
	...
	return display_subeffects(ws, tval, layout, trigger::to_generic(p), this_slot, from_slot, r_hi, r_lo, indentation + indentation_amount);
}
uint32_t es_pop_type_scope_nation(EFFECT_DISPLAY_PARAMS) {
	...
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "every"));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, ws.world.pop_type_get_name(type)));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "pop"));
		text::close_layout_box(layout, box);
	}
...
uint32_t es_region_scope(EFFECT_DISPLAY_PARAMS) {
	...
	auto region = (tval[0] & effect::scope_has_limit) != 0 ? trigger::payload(tval[3]).state_id : trigger::payload(tval[2]).state_id;
...
uint32_t ef_none(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "no_effect"));
		text::close_layout_box(layout, box);
	}
	return 0;
}
uint32_t ef_capital(EFFECT_DISPLAY_PARAMS) {
	...
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "move_capital_to"));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, new_capital);
		text::close_layout_box(layout, box);
	}
	return 0;
}
...
uint32_t ef_add_core_int(EFFECT_DISPLAY_PARAMS) {
	...
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "add_core_to"));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, prov);
		text::close_layout_box(layout, box);
	}
	return 0;
}
...
uint32_t ef_remove_core_int(EFFECT_DISPLAY_PARAMS) {
	...
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "remove_core_from"));
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, prov);
		text::close_layout_box(layout, box);
	}
...
dcon::nation_id convert_this(sys::state& ws, dcon::state_instance_id p) { return ws.world.state_instance_get_nation_from_state_ownership(p); }
dcon::nation_id convert_this(sys::state& ws, dcon::province_id p) { return ws.world.province_get_nation_from_province_ownership(p); }
...
template<typename T, typename F> void add_to_map(sys::state& ws, text::substitution_map& map, T v, std::string_view alt, F const& f) {
	if(v) {
...
uint32_t ef_primary_culture_this_nation(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		add_to_map(ws, m, trigger::to_nation(this_slot), "this_nation_culture", [&](auto id) { return ws.world.culture_get_name(ws.world.nation_get_primary_culture(id)); });
		text::localised_format_box(ws, layout, box, "primary_culture_changes_to", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_primary_culture_this_state(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		add_to_map(ws, m, convert_this(ws, trigger::to_state(this_slot)), "this_nation_culture", [&](auto id) { return ws.world.culture_get_name(ws.world.nation_get_primary_culture(id)); });
		text::localised_format_box(ws, layout, box, "primary_culture_changes_to", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_primary_culture_this_province(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		add_to_map(ws, m, convert_this(ws, trigger::to_prov(this_slot)), "this_nation_culture", [&](auto id) { return ws.world.culture_get_name(ws.world.nation_get_primary_culture(id)); });
		text::localised_format_box(ws, layout, box, "primary_culture_changes_to", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_primary_culture_this_pop(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		add_to_map(ws, m, convert_this(ws, trigger::to_pop(this_slot)), "this_nation_culture", [&](auto id) { return ws.world.culture_get_name(ws.world.nation_get_primary_culture(id)); });
		text::localised_format_box(ws, layout, box, "primary_culture_changes_to", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_primary_culture_from_nation(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		add_to_map(ws, m, convert_this(ws, trigger::to_nation(from_slot)), "from_nation_culture", [&](auto id) { return ws.world.culture_get_name(ws.world.nation_get_primary_culture(id)); });
		text::localised_format_box(ws, layout, box, "primary_culture_changes_to", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_government(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text, ws.culture_definitions.governments[trigger::payload(tval[1]).gov_id].name);
...
uint32_t ef_prestige(EFFECT_DISPLAY_PARAMS) {
	...
	float change = delta;
	if(primary_slot != -1) {
		auto prestige_multiplier = 1.0f + ws.world.nation_get_modifier_values(trigger::to_nation(primary_slot), sys::national_mod_offsets::prestige);
		auto new_prestige = std::max(0.0f, ws.world.nation_get_prestige(trigger::to_nation(primary_slot)) + (delta > 0 ? (delta * prestige_multiplier) : delta));
...
uint32_t ef_add_tax_relative_income(EFFECT_DISPLAY_PARAMS) {
	auto amount = trigger::read_float_from_payload(tval + 1);
	if(primary_slot != -1) {
		auto income = ws.world.nation_get_total_poor_income(trigger::to_nation(primary_slot)) + ws.world.nation_get_total_middle_income(trigger::to_nation(primary_slot)) + ws.world.nation_get_total_rich_income(trigger::to_nation(primary_slot));
...
uint32_t ef_add_crime(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text, ws.culture_definitions.crimes[trigger::payload(tval[1]).crm_id].name);
...
uint32_t ef_trigger_revolt_state(EFFECT_DISPLAY_PARAMS) { return ef_trigger_revolt_nation(ws, tval, layout, 0, 0, 0, r_lo, r_hi, indentation); }
uint32_t ef_trigger_revolt_province(EFFECT_DISPLAY_PARAMS) { return ef_trigger_revolt_nation(ws, tval, layout, 0, 0, 0, r_lo, r_hi, indentation); }
...
uint32_t ef_country_event_this_nation(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text, ws.world.national_event_get_name(trigger::payload(tval[1]).nev_id));
...
uint32_t ef_province_event_this_nation(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text, ws.world.provincial_event_get_name(trigger::payload(tval[1]).pev_id));
		text::localised_format_box(ws, layout, box, "event_fires", m);
		text::close_layout_box(layout, box);
	}
	return 0;
...
uint32_t ef_set_variable(EFFECT_DISPLAY_PARAMS) {
	{
		auto box = text::open_layout_box(layout, indentation);
		text::substitution_map m;
		text::add_to_substitution_map(m, text::variable_type::text, ws.national_definitions.variable_names[trigger::payload(tval[1]).natv_id]);
...
uint32_t ef_party_loyalty(EFFECT_DISPLAY_PARAMS) {
	...
	{
		auto box = text::open_layout_box(layout, indentation);
		text::add_to_layout_box(layout, ws, box, trigger::payload(tval[1]).prov_id);
		text::close_layout_box(layout, box);
	}
	{
		auto box = text::open_layout_box(layout, indentation + indentation_amount);
...
inline constexpr uint32_t (*effect_functions[])(EFFECT_DISPLAY_PARAMS) = {
	...
	ef_build_railway_in_capital_yes_whole_state_yes_limit, // constexpr inline uint16_t build_railway_in_capital_yes_whole_state_yes_limit
														   // = 0x010B;
	ef_build_railway_in_capital_yes_whole_state_no_limit,  // constexpr inline uint16_t build_railway_in_capital_yes_whole_state_no_limit =
														   // 0x010C;
	ef_build_railway_in_capital_no_whole_state_yes_limit,  // constexpr inline uint16_t build_railway_in_capital_no_whole_state_yes_limit =
														   // 0x010D;
	ef_build_railway_in_capital_no_whole_state_no_limit,   // constexpr inline uint16_t build_railway_in_capital_no_whole_state_no_limit =
														   // 0x010E;
	ef_build_fort_in_capital_yes_whole_state_yes_limit,	   // constexpr inline uint16_t build_fort_in_capital_yes_whole_state_yes_limit =
														   // 0x010F;
	...
	ef_scaled_consciousness_province_unemployment,		   // constexpr inline uint16_t scaled_consciousness_province_unemployment = 0x0131;
	...
	ef_country_event_immediate_province_this_nation,	   // constexpr inline uint16_t country_event_immediate_province_this_nation = 0x0141;
	ef_country_event_province_this_state,				   // constexpr inline uint16_t country_event_province_this_state = 0x0142;
	ef_country_event_immediate_province_this_state,		   // constexpr inline uint16_t country_event_immediate_province_this_state = 0x0143;
...
uint32_t internal_make_effect_description(EFFECT_DISPLAY_PARAMS) { return effect_functions[*tval & effect::code_mask](ws, tval, layout, primary_slot, this_slot, from_slot, r_lo, r_hi, indentation); }
...
void effect_description(sys::state& state, text::layout_base& layout, dcon::effect_key k, int32_t primary_slot, int32_t this_slot, int32_t from_slot, uint32_t r_lo, uint32_t r_hi) {
	effect_tooltip::internal_make_effect_description(state, state.effect_data.data() + k.index(), layout, primary_slot, this_slot, from_slot, r_lo, r_hi, 0);
}
