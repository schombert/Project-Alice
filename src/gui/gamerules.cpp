...
void gamerules_main_cancel_button_t::render(sys::state & state, int32_t x, int32_t y) noexcept {
	...
	auto fh = text::make_font_id(state, text_is_header, text_scale * 10);
...
void gamerules_main_t::render(sys::state & state, int32_t x, int32_t y) noexcept {
	...
	for (auto& __item : textures_to_render) {
		if (__item.texture_type == background_type::texture)
			...
			ogl::render_rect_with_repeated_border(state, cmod, float(5), float(x + __item.x), float(y + __item.y), float(__item.w), float(__item.h), ogl::get_late_load_texture_handle(state, __item.texture_id, __item.texture), base_data.get_rotation(), false, state_is_rtl(state));
		else if (__item.texture_type == background_type::textured_corners)
			ogl::render_rect_with_repeated_corner(state, cmod, float(5), float(x + __item.x), float(y + __item.y), float(__item.w), float(__item.h), ogl::get_late_load_texture_handle(state, __item.texture_id, __item.texture), base_data.get_rotation(), false, state_is_rtl(state));
	}
}
...
void gamerules_main_t::create_layout_level(sys::state& state, layout_level& lvl, char const* ldata, size_t sz) {
	...
	if(lvl.paged) {
		...
		lvl.page_controls->base_data.size.x = int16_t(50);
		lvl.page_controls->base_data.size.y = int16_t(10);
	}
