...
typedef std::variant<
    outliner_filter, dcon::army_id, dcon::navy_id, dcon::gp_relationship_id,
    dcon::state_building_construction_id,
    dcon::province_building_construction_id,
    dcon::province_land_construction_id, dcon::province_naval_construction_id>
    outliner_data;

class outliner_element_button
    : public generic_settable_element<button_element_base, outliner_data> {
  public:
	...
	void update_tooltip(sys::state &state, int32_t x, int32_t y,
		...
		if (std::holds_alternative<dcon::army_id>(content)) {
			...
			text::add_to_substitution_map(
			    sub, text::variable_type::army_name,
			    state.to_string_view(state.world.army_get_name(aid)));
			text::add_to_substitution_map(
			    sub, text::variable_type::location,
			    state.world.army_location_get_location(
			        state.world.army_get_army_location(aid)));
			text::localised_format_box(state, contents, box,
			                           std::string_view("ol_armies_tooltip"),
			                           sub);
			...
			text::substitution_map sub{};
			text::add_to_substitution_map(
			    sub, text::variable_type::navy_name,
			    state.to_string_view(state.world.navy_get_name(nid)));
			text::add_to_substitution_map(
			    sub, text::variable_type::location,
			    state.world.navy_location_get_location(
			        state.world.navy_get_navy_location(nid)));
			text::localised_format_box(state, contents, box,
			                           std::string_view("ol_navies_tooltip"),
			                           sub);
			text::close_layout_box(contents, box);
			...
			auto nid = state.world.gp_relationship_get_influence_target(grid);
		               ...
		               content)) {
			auto sbcid =
			    std::get<dcon::state_building_construction_id>(content);
			auto ftid = state.world.state_building_construction_get_type(sbcid);

		} else if (std::holds_alternative<
		               dcon::province_building_construction_id>(content)) {
			auto pbcid =
			    std::get<dcon::province_building_construction_id>(content);
			auto btid =
			    state.world.province_building_construction_get_type(pbcid);
			auto name = province_building_type_get_name(
			    economy::province_building_type(btid));

		} else if (std::holds_alternative<dcon::province_land_construction_id>(
		               content)) {
			...
			auto name =
			    state.military_definitions.unit_base_definitions[utid].name;
		               ...
		               content)) {
			auto pncid =
			    std::get<dcon::province_naval_construction_id>(content);
...
class outliner_element : public listbox_row_element_base<outliner_data> {
  ...
  public:
	...
	make_child(sys::state &state, std::string_view name,
		...
		if (name == "outliner_header") {
			...
			auto ptr =
			    make_element_by_type<simple_text_element_base>(state, id);
			header_text = ptr.get();
			return ptr;
	...
	void update(sys::state &state) noexcept override {
		...
		if (std::holds_alternative<outliner_filter>(content)) {
			...
			auto text =
			    text::produce_simple_string(state, get_filter_text_key(filter));
			...
			if (std::holds_alternative<dcon::army_id>(content)) {
				...
				info_text->set_text(state,
				                    std::string{state.to_string_view(
				                        state.world.army_get_name(aid))});

				auto strength = 0.f;
				state.world.army_for_each_army_membership_as_army(
				    aid, [&](dcon::army_membership_id amid) {
					    auto rid =
				...
				info_text->set_text(state,
				                    std::string{state.to_string_view(
				                        state.world.navy_get_name(nid))});
				// Navy membership
				auto strength = 0.f;
				state.world.navy_for_each_navy_membership_as_navy(
				    nid, [&](dcon::navy_membership_id nmid) {
					    auto sid = state.world.navy_membership_get_ship(nmid);
					    strength += state.world.ship_get_strength(sid);
				    });
				entry_text->set_text(state, text::prettify(int32_t(strength)));
			} else if (std::holds_alternative<dcon::gp_relationship_id>(
			               content)) {
				auto grid = std::get<dcon::gp_relationship_id>(content);
				auto nid =
				    state.world.gp_relationship_get_influence_target(grid);
				info_text->set_text(
				    state, text::produce_simple_string(
				               state, state.world.nation_get_name(nid)));
				auto status = state.world.gp_relationship_get_status(grid);
				entry_text->set_text(
				    state,
				    text::produce_simple_string(
				        state, text::get_influence_level_name(state, status)));
			} else if (std::holds_alternative<
			               dcon::state_building_construction_id>(content)) {
				auto sbcid =
				    std::get<dcon::state_building_construction_id>(content);
				auto ftid =
				    state.world.state_building_construction_get_type(sbcid);
				info_text->set_text(
				    state, text::produce_simple_string(
				               state, state.world.factory_type_get_name(ftid)));
				...
				auto name = province_building_type_get_name(
				    economy::province_building_type(btid));
				info_text->set_text(state,
				                    text::produce_simple_string(state, name));
				// TODO: Entry displays time left to build building
				entry_text->set_text(state, "");
			} else if (std::holds_alternative<
			               dcon::province_land_construction_id>(content)) {
				auto plcid =
				    std::get<dcon::province_land_construction_id>(content);
				auto utid =
				...
				entry_text->set_text(state, "");
			               ...
			               dcon::province_naval_construction_id>(content)) {
				auto pncid =
				    std::get<dcon::province_naval_construction_id>(content);
				auto utid =
...
class outliner_listbox
    : public listbox_element_base<outliner_element, outliner_data> {
  protected:
  ...
  public:
	void on_update(sys::state &state) noexcept override {
		...
		if (get_filter(state, outliner_filter::armies)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_army_control_as_controller(
		...
		if (get_filter(state, outliner_filter::navies)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_navy_control_as_controller(
		...
		if (get_filter(state, outliner_filter::factories)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_state_building_construction(
			    state.local_player_nation,
			    [&](dcon::state_building_construction_id sbcid) {
		...
		if (get_filter(state, outliner_filter::buildings)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_province_building_construction(
			    state.local_player_nation,
			    [&](dcon::province_building_construction_id pbcid) {
		...
		if (get_filter(state, outliner_filter::army_construction)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_province_land_construction(
			    state.local_player_nation,
		...
		if (get_filter(state, outliner_filter::navy_construction)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_province_naval_construction(
			    state.local_player_nation,
			    [&](dcon::province_naval_construction_id pncid) {
		...
		if (get_filter(state, outliner_filter::gp_influence)) {
			...
			auto old_size = row_contents.size();
			state.world.nation_for_each_gp_relationship_as_great_power(
...
class outliner_button : public button_element_base {
  public:
	...
	void button_action(sys::state &state) noexcept override {
		if (state.ui_state.outliner_window) {
			state.ui_state.outliner_window->set_visible(
			    state, !state.ui_state.outliner_window->is_visible());
			on_update(state);
		}
	}

	message_result test_mouse(sys::state &state, int32_t x,
	                          int32_t y) noexcept override {
	...
	void update_tooltip(sys::state &state, int32_t x, int32_t y,
		...
		if (!state.ui_state.outliner_window->is_visible()) {
			text::localised_format_box(
			    state, contents, box, std::string_view("topbar_open_outliner"));
		} else {
			text::localised_format_box(
			    state, contents, box,
			    std::string_view("topbar_close_outliner"));
		}
		text::close_layout_box(contents, box);
	}
};
...
class outliner_window : public window_element_base {
  ...
  public:
	void on_create(sys::state &state) noexcept override {
		...
		auto ptr = make_element_by_type<image_element_base>(
		    state, state.ui_state.defs_by_name.find("outliner_bottom")
		               ->second.definition);
	...
	make_child(sys::state &state, std::string_view name,
		...
		if (name == "outliner_top") {
			...
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::rebel_occupations>>(
			    state, id);
		} else if (name == "outliner_view_sieges") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::sieges>>(state, id);
		} else if (name == "outliner_view_hostile_sieges") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::hostile_sieges>>(
			    state, id);
		} else if (name == "outliner_view_combat") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::combat>>(state, id);
		} else if (name == "outliner_view_navalcombat") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::naval_combat>>(state,
			                                                             id);
		} else if (name == "outliner_view_armies") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::armies>>(state, id);
		} else if (name == "outliner_view_navies") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::navies>>(state, id);
		} else if (name == "outliner_view_factories") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::factories>>(state,
			                                                          id);
		} else if (name == "outliner_view_buildings") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::buildings>>(state,
			                                                          id);
		} else if (name == "outliner_view_army_construction") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::army_construction>>(
			    state, id);
		} else if (name == "outliner_view_navy_construction") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::navy_construction>>(
			    state, id);
		} else if (name == "outliner_view_gp_influence") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::gp_influence>>(state,
			                                                             id);
		} else if (name == "outliner_view_natfocus") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::national_focus>>(
			    state, id);
		} else if (name == "outliner_view_rallypoints") {
			return make_element_by_type<
			    outliner_filter_checkbox<outliner_filter::rally_points>>(state,
			                                                             id);
		} else {
			return nullptr;
		}
	...
	message_result set(sys::state &state,
	                   Cyto::Any &payload) noexcept override {
		if (payload.holds_type<outliner_filter>()) {
			auto filter = any_cast<outliner_filter>(payload);

			state.user_settings.outliner_views[uint8_t(filter)] =
			    !state.user_settings.outliner_views[uint8_t(filter)];
	...
	message_result get(sys::state &state,
	                   Cyto::Any &payload) noexcept override {
		if (payload.holds_type<outliner_filter>()) {
			auto filter = any_cast<outliner_filter>(payload);
			payload.emplace<bool>(
			    state.user_settings.outliner_views[uint8_t(filter)]);
			return message_result::consumed;
		}
		return message_result::unseen;
