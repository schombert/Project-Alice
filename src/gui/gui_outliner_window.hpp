...
typedef std::variant< outliner_filter, dcon::army_id, dcon::navy_id, dcon::gp_relationship_id, dcon::state_building_construction_id,
                      dcon::province_building_construction_id, dcon::province_land_construction_id, dcon::province_naval_construction_id>
    outliner_data;
	...
	void on_update(sys::state& state) noexcept override {
		...
		if(get_filter(state, outliner_filter::factories)) {
			...
			state.world.nation_for_each_state_building_construction(
			    state.local_player_nation, [&](dcon::state_building_construction_id sbcid) { row_contents.push_back(sbcid); });
			if(old_size == row_contents.size())
				row_contents.pop_back();
		}
		if(get_filter(state, outliner_filter::buildings)) {
			...
			state.world.nation_for_each_province_building_construction(
			    state.local_player_nation, [&](dcon::province_building_construction_id pbcid) { row_contents.push_back(pbcid); });
			if(old_size == row_contents.size())
				row_contents.pop_back();
		}
		if(get_filter(state, outliner_filter::army_construction)) {
			...
			state.world.nation_for_each_province_land_construction(
			    state.local_player_nation, [&](dcon::province_land_construction_id plcid) { row_contents.push_back(plcid); });
			if(old_size == row_contents.size())
				row_contents.pop_back();
		}
		if(get_filter(state, outliner_filter::navy_construction)) {
			...
			state.world.nation_for_each_province_naval_construction(
			    state.local_player_nation, [&](dcon::province_naval_construction_id pncid) { row_contents.push_back(pncid); });
			if(old_size == row_contents.size())
				row_contents.pop_back();
		}
		// TODO: gp_influence,
		if(get_filter(state, outliner_filter::gp_influence)) {
			...
			state.world.nation_for_each_gp_relationship_as_great_power(
			    state.local_player_nation, [&](dcon::gp_relationship_id grid) { row_contents.push_back(grid); });
			if(old_size == row_contents.size())
				row_contents.pop_back();
		}
		// TODO: national_focus,
