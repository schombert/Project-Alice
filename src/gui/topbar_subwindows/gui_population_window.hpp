...
public:
	void on_create(sys::state& state) noexcept override {
		...
		auto ptr1 = make_element_by_type<pop_left_side_country_window>(
		    state, state.ui_state.defs_by_name.find("poplistitem_country")->second.definition);
		country_window = ptr1.get();
		add_child_to_back(std::move(ptr1));

		auto ptr2 = make_element_by_type<pop_left_side_state_window>(
		    state, state.ui_state.defs_by_name.find("poplistitem_state")->second.definition);
		state_window = ptr2.get();
		add_child_to_back(std::move(ptr2));

		auto ptr3 = make_element_by_type<pop_left_side_province_window>(
		    state, state.ui_state.defs_by_name.find("poplistitem_province")->second.definition);
			...
			if(!state.world.province_get_is_colonial(prov_id)) {
				...
				for(int32_t i = start; i < end; i++) {
					...
					if(politics::political_party_is_active(state, pid)) {
						auto support = politics::party_total_support(
						    state, pop_id, pid, state.world.province_get_nation_from_province_ownership(prov_id), prov_id);
		...
		if(parent) {
			...
			for(auto const pop_id : pop_list) {
				...
				if constexpr(std::is_same_v<T, dcon::culture_id>)
					...
					distrib[typename T::value_base_t(state.world.pop_get_religion(pop_id).id.index())] +=
...
class pop_details_window : public generic_settable_element<window_element_base, pop_details_data> {
	...
	template<std::size_t... Targs> void generate_promotion_items(sys::state& state, std::integer_sequence<std::size_t, Targs...> const&) {
		...
		(([&] {
			 auto win = make_element_by_type<pop_details_promotion_window<Targs>>(
			     state, state.ui_state.defs_by_name.find("pop_promotion_item")->second.definition);
...
public:
	void on_create(sys::state& state) noexcept override {
		...
		generate_distribution_windows< pop_distribution_window<dcon::ideology_id, false>,
...
public:
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		text::add_to_substitution_map(sub, text::variable_type::val,
		                              text::pretty_integer{int32_t(state.world.nation_get_demographics(
		                                  state.local_player_nation, demographics::to_key(state, content)))});
	...
	void on_create(sys::state& state) noexcept override {
...

		{
			generate_distribution_windows<
			    pop_distribution_window<dcon::pop_type_id, true>, pop_distribution_window<dcon::religion_id, true>,
			    pop_distribution_window<dcon::ideology_id, true>, pop_distribution_window<dcon::culture_id, true>,
			    pop_distribution_window<dcon::issue_option_id, true>, pop_distribution_window<dcon::political_party_id, true>>(state);
...

		{
			...
			state.world.for_each_pop_type([&](dcon::pop_type_id id) {
				auto win = make_element_by_type<pop_filter_button>(
				    state, state.ui_state.defs_by_name.find("pop_filter_button")->second.definition);
				Cyto::Any payload = id;
				win->base_data.position = offset;
