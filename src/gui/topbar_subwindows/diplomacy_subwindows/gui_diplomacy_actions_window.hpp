...
public:
	...
	void button_action(sys::state& state) noexcept override { send(state, parent, trigger_gp_choice{gp_choice_actions::discredit}); }
...
public:
	...
	void button_action(sys::state& state) noexcept override { send(state, parent, trigger_gp_choice{gp_choice_actions::expel_advisors}); }
...
public:
	...
	void button_action(sys::state& state) noexcept override { send(state, parent, trigger_gp_choice{gp_choice_actions::ban_embassy}); }
...
public:
	...
	void button_action(sys::state& state) noexcept override { send(state, parent, trigger_gp_choice{gp_choice_actions::decrease_opinion}); }
...
public:
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		for(auto war_par : state.world.nation_get_war_participant(asker)) {
			if(!military::is_civil_war(state, war_par.get_war()) && military::standard_war_joining_is_possible(state, war_par.get_war(), target, military::is_attacker(state, war_par.get_war(), asker)) && (!war_par.get_war().get_is_crisis_war() || state.military_definitions.great_wars_enabled)) {
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		if(state.world.unilateral_relationship_get_military_access(urid)) {
			...
			if(state.defines.cancelgivemilaccess_diplomatic_cost > 0) {
				text::add_line_with_condition(state, contents, "cancel_given_access_explain_1", state.world.nation_get_diplomatic_points(state.local_player_nation) >= state.defines.cancelgivemilaccess_diplomatic_cost, text::variable_type::x,
					int64_t(state.defines.cancelgivemilaccess_diplomatic_cost));
			}
		} else {
...
public:
	...
	void button_action(sys::state& state) noexcept override { command::increase_relations(state, state.local_player_nation, retrieve<dcon::nation_id>(state, parent)); }
...
public:
	...
	void button_action(sys::state& state) noexcept override { command::decrease_relations(state, state.local_player_nation, retrieve<dcon::nation_id>(state, parent)); }
...
public:
	void on_update(sys::state& state) noexcept override {
		...
		disabled = subsidies ? !command::can_cancel_war_subsidies(state, state.local_player_nation, target) : !command::can_give_war_subsidies(state, state.local_player_nation, target);
	}
...
public:
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override { text::add_line_with_condition(state, contents, "not_implemented", false); }
};
...
public:
	void on_update(sys::state& state) noexcept override {
		auto contents = text::create_endless_layout(internal_layout, text::layout_parameters{0, 0, static_cast<int16_t>(base_data.size.x), static_cast<int16_t>(base_data.size.y), base_data.data.text.font_handle, 0, text::alignment::left, text::text_color::white, false});
...
public:
	bool show = false;
	void on_update(sys::state& state) noexcept override { show = retrieve<gp_choice_num>(state, parent).value == retrieve<gp_choice_get_selection>(state, parent).value; }
...
public:
	...
	void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
		...
		case gp_choice_actions::decrease_opinion: {
			...
			if((nations::influence::level_mask & state.world.gp_relationship_get_status(state.world.get_gp_relationship_by_gp_influence_pair(target, gp))) == nations::influence::level_hostile) {
