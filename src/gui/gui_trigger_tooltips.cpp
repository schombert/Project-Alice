...
void tf_generic_scope(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(tval, ws, layout, primary_slot, this_slot, from_slot,
		indentation + (st_count > 1 ? indentation_amount : 0), show_condition);
}
...
void tf_sea_zone_scope(TRIGGER_DISPLAY_PARAMS) {
	...
	if(sea_zone) {
		display_subtriggers(tval, ws, layout, trigger::to_generic(sea_zone), this_slot, from_slot,
			indentation + indentation_amount, show_condition);
	} else {
		display_subtriggers(tval, ws, layout, -1, this_slot, from_slot, indentation + indentation_amount, false);
	}
}
void tf_cultural_union_scope(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(tval, ws, layout, trigger::to_generic(group_holder), this_slot, from_slot,
		indentation + indentation_amount, show_condition);
}
...
void tf_state_scope_province(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(tval, ws, layout, st ? trigger::to_generic(st) : -1, this_slot, from_slot,
		indentation + indentation_amount, show_condition && st);
}
...
void tf_strata_rich(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "strata"), text::produce_simple_string(ws, "rich"), ws,
		layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_strata_poor(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "strata"), text::produce_simple_string(ws, "poor"), ws,
		layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_religion_this_pop(TRIGGER_DISPLAY_PARAMS) {
	auto rc = this_slot != -1 ? ws.world.nation_get_religion(nations::owner_of_pop(ws, trigger::to_pop(this_slot)))
							  : dcon::religion_id{};
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "religion"),
...
void tf_num_of_ports(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "num_of_ports_connected"), int64_t(tval[1]), ws, layout,
		box);
	text::close_layout_box(layout, box);
}
...
void tf_is_ideology_enabled(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0],
		text::produce_simple_string(ws, ws.world.ideology_get_name(trigger::payload(tval[1]).ideo_id)),
...
void tf_is_accepted_culture_pop(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "culture"), text::produce_simple_string(ws, "an_accepted"),
		ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_province_id(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0],
		text::produce_simple_string(ws, ws.world.province_get_name(trigger::payload(tval[1]).prov_id)), ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_this_culture_union_from(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(from_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "cultural_union"), trigger::to_nation(from_slot), ws,
			layout, box);
	else
...
void tf_this_culture_union_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(this_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "cultural_union"), trigger::to_nation(this_slot), ws,
			layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "cultural_union"),
			text::produce_simple_string(ws, "this_nation"), ws, layout, box);
...
void tf_is_our_vassal_this_province(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(this_slot != -1)
		display_with_has_comparison(tval[0], ws.world.province_get_nation_from_province_ownership(trigger::to_prov(this_slot)),
			ws, layout, box);
	else
		display_with_has_comparison(tval[0], text::produce_simple_string(ws, "this_nation"), ws, layout, box);
	text::add_space_to_layout_box(layout, ws, box);
...
void tf_is_releasable_vassal_from(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(from_slot != -1)
		display_with_comparison(tval[0], trigger::to_nation(from_slot), text::produce_simple_string(ws, "a_releasable_vassal"),
			ws, layout, box);
	else
...
void tf_has_pop_religion_state_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1) {
		display_with_has_comparison(tval[0],
			text::produce_simple_string(ws, ws.world.religion_get_name(ws.world.pop_get_religion(trigger::to_pop(this_slot)))),
			ws, layout, box);
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "pops"));
	} else {
...
void tf_trade_goods_in_state_state(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0],
		text::produce_simple_string(ws, ws.world.commodity_get_name(trigger::payload(tval[1]).com_id)),
...
void tf_crisis_exist(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "there"),
		text::produce_simple_string(ws, "an_ongoing_crisis"), ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_pop_unemployment_nation_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		text::add_to_layout_box(layout, ws, box,
			ws.world.pop_type_get_name(ws.world.pop_get_poptype(trigger::to_pop(this_slot))));
	else
...
void tf_relation_tag(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_space_to_layout_box(layout, ws, box);
	display_with_comparison(tval[0], trigger::payload(tval[2]).tag_id, int64_t(trigger::payload(tval[1]).signed_value), ws,
		layout, box);
	text::close_layout_box(layout, box);
}
void tf_relation_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_space_to_layout_box(layout, ws, box);
	if(this_slot != -1)
		display_with_comparison(tval[0], trigger::to_nation(this_slot), int64_t(trigger::payload(tval[1]).signed_value), ws,
			layout, box);
	else
...
void tf_relation_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_space_to_layout_box(layout, ws, box);
	if(from_slot != -1)
		display_with_comparison(tval[0], trigger::to_nation(from_slot), int64_t(trigger::payload(tval[1]).signed_value), ws,
			layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "from_nation"),
			int64_t(trigger::payload(tval[1]).signed_value), ws, layout, box);
...
void tf_party_loyalty_nation_province_id(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_space_to_layout_box(layout, ws, box);
	display_with_comparison(tval[0], trigger::payload(tval[1]).prov_id, int64_t(trigger::payload(tval[2]).signed_value), ws,
		layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_work_available_nation(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "work"), text::produce_simple_string(ws, "available_for"),
		ws, layout, box);
	text::add_to_layout_box(layout, ws, box, ws.world.pop_type_get_name(trigger::payload(tval[1]).popt_id));
	text::add_space_to_layout_box(layout, ws, box);
	text::close_layout_box(layout, box);
...
void tf_religion_nation_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
			text::produce_simple_string(ws,
				ws.world.religion_get_name(ws.world.nation_get_religion(trigger::to_nation(from_slot)))),
			ws, layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
...
void tf_religion_nation_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
			text::produce_simple_string(ws,
				ws.world.religion_get_name(ws.world.nation_get_religion(trigger::to_nation(this_slot)))),
			ws, layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
