...
void make_trigger_description(sys::state& ws, text::layout_base& layout, uint16_t const* tval, int32_t primary_slot, int32_t this_slot,
                              int32_t from_slot, int32_t indentation, bool show_condition);
...
void tf_x_owned_province_scope_nation(TRIGGER_DISPLAY_PARAMS) {
	{
		...
		text::add_to_layout_box(layout, ws, box,
		                        primary_slot != -1
		                            ? text::produce_simple_string(ws, ws.world.nation_get_name(trigger::to_nation(primary_slot)))
...
void tf_x_core_scope_province(TRIGGER_DISPLAY_PARAMS) {
	{
		...
		text::add_to_layout_box(layout, ws, box,
		                        primary_slot != -1
		                            ? text::produce_simple_string(ws, ws.world.province_get_name(trigger::to_prov(primary_slot)))
...
void tf_owner_scope_state(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(
	    tval, ws, layout,
	    primary_slot != -1 ? trigger::to_generic(ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(primary_slot)))
	                       : -1,
...
void tf_controller_scope(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(
	    tval, ws, layout,
	    primary_slot != -1 ? trigger::to_generic(ws.world.province_get_nation_from_province_control(trigger::to_prov(primary_slot))) : -1,
	    this_slot, from_slot, indentation + indentation_amount, show_condition && primary_slot != -1);
}
...
void tf_country_scope_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(tval, ws, layout,
	                    primary_slot != -1 ? trigger::to_generic(nations::owner_of_pop(ws, trigger::to_pop(primary_slot))) : -1,
	                    this_slot, from_slot, indentation + indentation_amount, show_condition && primary_slot != -1);
}
...
void tf_sphere_owner_scope(TRIGGER_DISPLAY_PARAMS) {
	...
	display_subtriggers(tval, ws, layout,
	                    primary_slot != -1 ? trigger::to_generic(ws.world.nation_get_in_sphere_of((trigger::to_nation(primary_slot))))
	                                       : -1,
	                    this_slot, from_slot, indentation + indentation_amount, show_condition && primary_slot != -1);
}
...
void tf_state_scope_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	auto st = primary_slot != -1
	              ? ws.world.province_get_state_membership(ws.world.pop_get_province_from_pop_location(trigger::to_pop(primary_slot)))
...
void tf_tag_scope(TRIGGER_DISPLAY_PARAMS) {
...

	{
		...
		make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
		text::add_to_layout_box(layout, ws, box,
		                        text::produce_simple_string(ws, tag_holder ? ws.world.nation_get_name(tag_holder)
		                                                                   : ws.world.national_identity_get_name(tag)));
		text::close_layout_box(layout, box);
	}
...
void tf_strata_middle(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "strata"), text::produce_simple_string(ws, "middle"), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_life_rating_province(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "life_rating"), int32_t(trigger::payload(tval[1]).signed_value), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_slave_nation(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "unemployment"), text::produce_simple_string(ws, "allowed"), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_culture_pop_reb(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "culture"),
	                        rc ? text::produce_simple_string(ws, ws.world.culture_get_name(rc))
	                           : text::produce_simple_string(ws, "rebel_culture"),
	                        ws, layout, box);
}
...
void tf_culture_nation_reb(TRIGGER_DISPLAY_PARAMS) {
	...
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(
	    tval[0], rc ? text::produce_simple_string(ws, ws.world.culture_get_name(rc)) : text::produce_simple_string(ws, "rebel_culture"),
	    text::produce_simple_string(ws, "a_primary_or_accepted"), ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_culture_group_nation_this_province(TRIGGER_DISPLAY_PARAMS) {
	auto rc = this_slot != -1
	              ? ws.world.nation_get_primary_culture(ws.world.province_get_nation_from_province_ownership(trigger::to_prov(this_slot)))
	              : dcon::culture_id{};
	auto cg = ws.world.culture_get_group_from_culture_group_membership(rc);
...
void tf_culture_group_nation_this_state(TRIGGER_DISPLAY_PARAMS) {
	auto rc =
	    this_slot != -1
	        ? ws.world.nation_get_primary_culture(ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot)))
	        : dcon::culture_id{};
	auto cg = ws.world.culture_get_group_from_culture_group_membership(rc);
...
void tf_culture_group_nation_this_pop(TRIGGER_DISPLAY_PARAMS) {
	auto rc =
	    this_slot != -1 ? ws.world.nation_get_primary_culture(nations::owner_of_pop(ws, trigger::to_pop(this_slot))) : dcon::culture_id{};
	auto cg = ws.world.culture_get_group_from_culture_group_membership(rc);
...
void tf_religion_reb(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "religion"),
	                        rc ? text::produce_simple_string(ws, ws.world.religion_get_name(rc))
	                           : text::produce_simple_string(ws, "rebel_religion"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_religion_this_state(TRIGGER_DISPLAY_PARAMS) {
	auto rc = this_slot != -1
	              ? ws.world.nation_get_religion(ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot)))
	              : dcon::religion_id{};
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "religion"),
...
void tf_religion_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "religion"),
	                        rc ? text::produce_simple_string(ws, ws.world.religion_get_name(rc))
	                           : text::produce_simple_string(ws, "this_pop_religion"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_cultural_union_this_self_pop(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "union_for_pop"), text::produce_simple_string(ws, "this_nation"), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_cultural_union_this_nation(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "union_for_nation"), text::produce_simple_string(ws, "this_nation"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_cultural_union_tag_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(
	    tval[0], text::produce_simple_string(ws, "union_for_nation"),
	    text::produce_simple_string(ws, holder ? ws.world.nation_get_name(holder) : ws.world.national_identity_get_name(tag)), ws, layout,
	    box);
	text::close_layout_box(layout, box);
}
...
void tf_owns(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(
	    layout, ws, box,
	    text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "owns" : "does_not_own"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, ws.world.province_get_name(p)));
	text::close_layout_box(layout, box);
}
void tf_controls(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(layout, ws, box,
	                        text::produce_simple_string(
	                            ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "controls" : "does_not_control"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, ws.world.province_get_name(p)));
	text::close_layout_box(layout, box);
}
...
void tf_is_core_this_province(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		text::add_to_layout_box(
		    layout, ws, box, ws.world.nation_get_name(ws.world.province_get_nation_from_province_ownership(trigger::to_prov(this_slot))));
	else
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "this_nation"));
	text::close_layout_box(layout, box);
}
...
void tf_num_of_cities_this_state(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(
	    ...
	    this_slot != -1
	        ? text::produce_simple_string(
	              ws, ws.world.nation_get_name(ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot))))
	        : text::produce_simple_string(ws, "this_nation"));
	text::close_layout_box(layout, box);
}
void tf_num_of_cities_this_province(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(
	    layout, ws, box,
	    this_slot != -1 ? text::produce_simple_string(ws, ws.world.nation_get_name(ws.world.province_get_nation_from_province_ownership(
...
void tf_num_of_cities_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(
	    layout, ws, box,
	    this_slot != -1 ? text::produce_simple_string(ws, ws.world.nation_get_name(nations::owner_of_pop(ws, trigger::to_pop(this_slot))))
	                    : text::produce_simple_string(ws, "this_nation"));
	text::close_layout_box(layout, box);
}
...
void tf_exists_bool(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(
	    layout, ws, box,
	    text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "exists" : "does_not_exist"));
	text::close_layout_box(layout, box);
}
...
void tf_has_country_flag(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(
	    tval[0], text::produce_simple_string(ws, ws.national_definitions.flag_variable_names[trigger::payload(tval[1]).natf_id]),
...
void tf_prestige_from(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "prestige"), text::produce_simple_string(ws, "prestige_of"), ws,
	                        layout, box);
	text::add_space_to_layout_box(layout, ws, box);
	if(from_slot != -1)
		text::add_to_layout_box(layout, ws, box, trigger::to_nation(from_slot));
...
void tf_has_building_state(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_has_comparison(
	    tval[0], text::produce_simple_string(ws, ws.world.factory_type_get_name(trigger::payload(tval[1]).fac_id)), ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_tag_this_province(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(this_slot != -1)
		display_with_comparison(tval[0], ws.world.province_get_nation_from_province_ownership(trigger::to_prov(this_slot)), ws, layout,
		                        box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "this_nation"), ws, layout, box);
	text::close_layout_box(layout, box);
...
void tf_tag_from_province(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(from_slot != -1)
		display_with_comparison(tval[0], ws.world.province_get_nation_from_province_ownership(trigger::to_prov(from_slot)), ws, layout,
		                        box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "from_nation"), ws, layout, box);
	text::close_layout_box(layout, box);
...
void tf_ruling_party_ideology_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "ruling_party_ideology"),
	                        text::produce_simple_string(ws, ws.world.ideology_get_name(trigger::payload(tval[1]).ideo_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_ruling_party(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "ruling_party"),
	                        text::produce_simple_string(ws, ws.world.political_party_get_name(trigger::payload(tval[1]).par_id)), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_primary_culture_pop(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "culture"), text::produce_simple_string(ws, "owner_primary_culture"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_primary_culture_nation_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "primary_culture"),
		                        text::produce_simple_string(
		                            ws, ws.world.culture_get_name(ws.world.nation_get_primary_culture(trigger::to_nation(this_slot)))),
		                        ws, layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "primary_culture"),
...
void tf_is_primary_culture_nation_this_state(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "primary_culture"),
...
void tf_is_primary_culture_state_this_state(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "dominant_culture"),
		    text::produce_simple_string(ws, ws.world.culture_get_name(ws.world.nation_get_primary_culture(
		                                        ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot))))),
		    ws, layout, box);
	else
...
void tf_is_primary_culture_pop_this_state(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "culture"),
		    text::produce_simple_string(ws, ws.world.culture_get_name(ws.world.nation_get_primary_culture(
		                                        ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot))))),
		    ws, layout, box);
	else
...
void tf_is_accepted_culture_province(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "dominant_culture"), text::produce_simple_string(ws, "an_accepted"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_next_reform_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "next_reform"),
	                        text::produce_simple_string(ws, ws.world.issue_option_get_name(trigger::payload(tval[1]).opt_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_is_next_rreform_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "next_reform"),
	                        text::produce_simple_string(ws, ws.world.reform_option_get_name(trigger::payload(tval[1]).ropt_id)), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_total_pops_nation(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "population"), int64_t(trigger::read_float_from_payload(tval + 1)),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_possible_vassal(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], trigger::payload(tval[1]).tag_id, text::produce_simple_string(ws, "a_possible_vassal"), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
void tf_province_id(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, ws.world.province_get_name(trigger::payload(tval[1]).prov_id)), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_has_global_flag(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(
	    tval[0], text::produce_simple_string(ws, ws.national_definitions.global_flag_variable_names[trigger::payload(tval[1]).glob_id]),
...
void tf_nationalvalue_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "national_value"),
	                        text::produce_simple_string(ws, ws.world.modifier_get_name(trigger::payload(tval[1]).mod_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_industrial_score_value(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "industrial_score"), int64_t(trigger::payload(tval[1]).signed_value),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_military_score_value(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "military_score"), int64_t(trigger::payload(tval[1]).signed_value),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_pop_majority_religion_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, "dominant_religion"),
	                        text::produce_simple_string(ws, ws.world.religion_get_name(trigger::payload(tval[1]).rel_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_pop_majority_culture_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "dominant_culture"),
	                        text::produce_simple_string(ws, ws.world.culture_get_name(trigger::payload(tval[1]).cul_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_rich_tax_above_poor(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "rich_tax"), text::produce_simple_string(ws, "gt_poor_tax"), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_this_culture_union_tag(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "cultural_union"), trigger::payload(tval[1]).tag_id, ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_is_our_vassal_this_state(TRIGGER_DISPLAY_PARAMS) {
	...
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	if(this_slot != -1)
		display_with_has_comparison(tval[0], ws.world.state_instance_get_nation_from_state_ownership(trigger::to_state(this_slot)), ws,
		                            layout, box);
	else
		display_with_has_comparison(tval[0], text::produce_simple_string(ws, "this_nation"), ws, layout, box);
	text::add_space_to_layout_box(layout, ws, box);
...
void tf_has_pop_culture_pop_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "culture"),
		    text::produce_simple_string(ws, ws.world.culture_get_name(ws.world.pop_get_culture(trigger::to_pop(this_slot)))), ws, layout,
		    box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "culture"), text::produce_simple_string(ws, "this_pop_culture"),
		                        ws, layout, box);
	text::close_layout_box(layout, box);
}
void tf_has_pop_culture_state_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1) {
		display_with_has_comparison(
		    tval[0], text::produce_simple_string(ws, ws.world.culture_get_name(ws.world.pop_get_culture(trigger::to_pop(this_slot)))), ws,
		    layout, box);
		text::add_space_to_layout_box(layout, ws, box);
...
void tf_has_pop_culture_state(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_has_comparison(tval[0], text::produce_simple_string(ws, ws.world.culture_get_name(trigger::payload(tval[1]).cul_id)), ws,
	                            layout, box);
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "pops"));
	text::close_layout_box(layout, box);
...
void tf_has_pop_religion_state_this_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1) {
		display_with_has_comparison(
		    tval[0], text::produce_simple_string(ws, ws.world.religion_get_name(ws.world.pop_get_religion(trigger::to_pop(this_slot)))),
		    ws, layout, box);
		text::add_space_to_layout_box(layout, ws, box);
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "pops"));
	} else {
...
void tf_has_pop_religion_state(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_has_comparison(tval[0], text::produce_simple_string(ws, ws.world.religion_get_name(trigger::payload(tval[1]).rel_id)),
	                            ws, layout, box);
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "pops"));
	text::close_layout_box(layout, box);
...
void tf_crisis_exist(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "there"), text::produce_simple_string(ws, "an_ongoing_crisis"), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_is_claim_crisis(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "current_crisis"), text::produce_simple_string(ws, "claim_crisis"),
	                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_relation_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		...
		display_with_comparison(tval[0], text::produce_simple_string(ws, "this_nation"), int64_t(trigger::payload(tval[1]).signed_value),
		                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_relation_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		...
		display_with_comparison(tval[0], text::produce_simple_string(ws, "from_nation"), int64_t(trigger::payload(tval[1]).signed_value),
		                        ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_party_loyalty_generic(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(layout, ws, box, ws.world.ideology_get_name(trigger::payload(tval[2]).ideo_id));
	text::add_space_to_layout_box(layout, ws, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "support"), int64_t(trigger::payload(tval[1]).signed_value), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_variable_issue_group_name_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, ws.world.issue_get_name(trigger::payload(tval[1]).iss_id)),
	                        text::produce_simple_string(ws, ws.world.issue_option_get_name(trigger::payload(tval[2]).opt_id)), ws, layout,
	                        box);
	text::close_layout_box(layout, box);
}
...
void tf_variable_reform_group_name_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	display_with_comparison(tval[0], text::produce_simple_string(ws, ws.world.reform_get_name(trigger::payload(tval[1]).ref_id)),
	                        text::produce_simple_string(ws, ws.world.reform_option_get_name(trigger::payload(tval[2]).ropt_id)), ws,
	                        layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_religion_nation_reb(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
		                        text::produce_simple_string(
		                            ws, ws.world.religion_get_name(ws.world.rebel_faction_get_religion(trigger::to_rebel(from_slot)))),
		                        ws, layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
...
void tf_religion_nation_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "national_religion"),
		    text::produce_simple_string(ws, ws.world.religion_get_name(ws.world.nation_get_religion(trigger::to_nation(from_slot)))), ws,
		    layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
...
void tf_religion_nation_this_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(this_slot != -1)
		display_with_comparison(
		    tval[0], text::produce_simple_string(ws, "national_religion"),
		    text::produce_simple_string(ws, ws.world.religion_get_name(ws.world.nation_get_religion(trigger::to_nation(this_slot)))), ws,
		    layout, box);
	else
		display_with_comparison(tval[0], text::produce_simple_string(ws, "national_religion"),
...
void trigger_description(sys::state& state, text::layout_base& layout, dcon::trigger_key k, int32_t primary_slot, int32_t this_slot,
                         int32_t from_slot) {
	...
	for(uint32_t i = 0; i < base.segments_count; ++i) {
		...
		if(seg.condition) {
			...
			trigger_tooltip::make_trigger_description(state, layout,
			                                          state.trigger_data.data() + state.trigger_data_indices[seg.condition.index()],
			                                          primary_slot, this_slot, from_slot, trigger_tooltip::indentation_amount * 2, true);
		}
	}
}
