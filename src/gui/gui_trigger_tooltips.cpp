...
void tf_can_build_factory_nation(TRIGGER_DISPLAY_PARAMS) {
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "capitalists_can_build" : "capitalists_cannot_build"));
	text::close_layout_box(layout, box);
}
...
void tf_owns(TRIGGER_DISPLAY_PARAMS) {
	...
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "owns" : "does_not_own"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, ws.world.province_get_name(p)));
	text::close_layout_box(layout, box);
}
void tf_controls(TRIGGER_DISPLAY_PARAMS) {
	...
	auto box = text::open_layout_box(layout, indentation);
	make_condition(tval, ws, layout, primary_slot, this_slot, from_slot, indentation, show_condition, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "controls" : "does_not_control"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, ws.world.province_get_name(p)));
	text::close_layout_box(layout, box);
}
...
void tf_can_build_in_province_railroad_no_limit_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "from_nation"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "can_build_railroad_here" : "cannot_build_railroad_here"));
	text::close_layout_box(layout, box);
}
...
void tf_can_build_in_province_fort_no_limit_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "from_nation"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "can_build_fort_here" : "cannot_build_fort_here"));
	text::close_layout_box(layout, box);
}
...
void tf_can_build_in_province_naval_base_no_limit_from_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	if(from_slot != -1)
		...
		text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, "from_nation"));
	text::add_space_to_layout_box(layout, ws, box);
	text::add_to_layout_box(layout, ws, box, text::produce_simple_string(ws, (tval[0] & trigger::association_mask) == trigger::association_eq ? "can_build_naval_base_here" : "cannot_build_naval_base_here"));
	text::close_layout_box(layout, box);
}
...
void tf_variable_pop_type_name_nation(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(layout, ws, box, ws.world.pop_type_get_name(trigger::payload(tval[1]).popt_id));
	text::add_space_to_layout_box(layout, ws, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "percentage"), text::fp_percentage{trigger::read_float_from_payload(tval + 2)}, ws, layout, box);
	text::close_layout_box(layout, box);
}
...
void tf_variable_pop_type_name_pop(TRIGGER_DISPLAY_PARAMS) {
	...
	text::add_to_layout_box(layout, ws, box, ws.world.pop_type_get_name(trigger::payload(tval[1]).popt_id));
	text::add_space_to_layout_box(layout, ws, box);
	display_with_comparison(tval[0], text::produce_simple_string(ws, "province_percentage"), text::fp_percentage{trigger::read_float_from_payload(tval + 2)}, ws, layout, box);
	text::close_layout_box(layout, box);
}
...
constexpr inline void (*trigger_functions[])(TRIGGER_DISPLAY_PARAMS) = {
	...
	tf_can_build_in_province_fort_no_limit_from_nation,		   // constexpr inline uint16_t can_build_in_province_fort_no_limit_from_nation =
															   // 0x022E;
	tf_can_build_in_province_fort_yes_limit_from_nation,	   // constexpr inline uint16_t can_build_in_province_fort_yes_limit_from_nation =
															   // 0x022F;
	tf_can_build_in_province_fort_no_limit_this_nation,		   // constexpr inline uint16_t can_build_in_province_fort_no_limit_this_nation =
															   // 0x0230;
	tf_can_build_in_province_fort_yes_limit_this_nation,	   // constexpr inline uint16_t can_build_in_province_fort_yes_limit_this_nation =
															   // 0x0231;
	...
	tf_can_build_fort_in_capital_yes_whole_state_no_limit,	   // constexpr inline uint16_t can_build_fort_in_capital_yes_whole_state_no_limit
															   // = 0x023B;
	tf_can_build_fort_in_capital_no_whole_state_yes_limit,	   // constexpr inline uint16_t can_build_fort_in_capital_no_whole_state_yes_limit
															   // = 0x023C;
	tf_can_build_fort_in_capital_no_whole_state_no_limit,	   // constexpr inline uint16_t can_build_fort_in_capital_no_whole_state_no_limit =
															   // 0x023D;
...
void multiplicative_value_modifier_description(sys::state& state, text::layout_base& layout, dcon::value_modifier_key modifier, int32_t primary_slot, int32_t this_slot, int32_t from_slot) {
	...
	for(uint32_t i = 0; i < base.segments_count; ++i) {
		...
		if(seg.condition) {
			...
			text::add_to_layout_box(layout, state, box, text::fp_two_places{seg.factor}, seg.factor >= 0.f ? text::text_color::green : text::text_color::red);
			text::close_layout_box(layout, box);
