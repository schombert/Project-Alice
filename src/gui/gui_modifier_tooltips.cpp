...
void active_single_modifier_description(sys::state& state, text::layout_base& layout, dcon::modifier_id mid, int32_t indentation, dcon::national_modifier_value nmid, float scaled = 1.f) {
	if(scaled == 0.f)
		return;
	auto fat_id = dcon::fatten(state.world, mid);
...
void active_single_modifier_description(sys::state& state, text::layout_base& layout, dcon::modifier_id mid, int32_t indentation, dcon::provincial_modifier_value pmid, float scaled = 1.f) {
	if(scaled == 0.f)
		return;
	auto fat_id = dcon::fatten(state.world, mid);
...
void acting_modifiers_description_province(sys::state& state, text::layout_base& layout, dcon::province_id p, int32_t identation, T nmid) {
	if(state.national_definitions.land_province)
		active_single_modifier_description(state, layout, state.national_definitions.land_province, identation, nmid);
	for(auto mpr : state.world.province_get_current_modifiers(p))
		active_single_modifier_description(state, layout, mpr.mod_id, identation, nmid);
	if(auto m = state.world.province_get_terrain(p); m)
		active_single_modifier_description(state, layout, m, identation, nmid);
	...
	if(state.economy_definitions.railroad_definition.province_modifier) {
		active_single_modifier_description(state, layout, state.economy_definitions.railroad_definition.province_modifier, identation, nmid, state.world.province_get_railroad_level(p));
	}
	if(state.national_definitions.infrastructure) {
		active_single_modifier_description(state, layout, state.national_definitions.infrastructure, identation, nmid, state.world.province_get_railroad_level(p) * state.economy_definitions.railroad_definition.infrastructure);
	}
	if(state.economy_definitions.fort_definition.province_modifier) {
		active_single_modifier_description(state, layout, state.economy_definitions.fort_definition.province_modifier, identation, nmid, state.world.province_get_fort_level(p));
	}
	if(state.economy_definitions.naval_base_definition.province_modifier) {
		active_single_modifier_description(state, layout, state.economy_definitions.naval_base_definition.province_modifier, identation, nmid, state.world.province_get_naval_base_level(p));
	}
	if(state.national_definitions.nationalism) {
		active_single_modifier_description(state, layout, state.national_definitions.nationalism, identation, nmid, (state.world.province_get_is_owner_core(p) ? 1.f : 0.f) * state.world.province_get_nationalism(p));
	}
	if(state.national_definitions.non_coastal) {
		active_single_modifier_description(state, layout, state.national_definitions.non_coastal, identation, nmid, !state.world.province_get_is_coast(p) ? 1.f : 0.f);
	}
	if(state.national_definitions.coastal) {
		active_single_modifier_description(state, layout, state.national_definitions.coastal, identation, nmid, state.world.province_get_is_coast(p) ? 1.f : 0.f);
	}
	if(state.national_definitions.overseas) {
		active_single_modifier_description(state, layout, state.national_definitions.overseas, identation, nmid, province::is_overseas(state, p) ? 1.f : 0.f);
	}
	if(state.national_definitions.core) {
		active_single_modifier_description(state, layout, state.national_definitions.core, identation, nmid, state.world.province_get_is_owner_core(p) ? 1.f : 0.f);
	}
	if(state.national_definitions.has_siege) {
		active_single_modifier_description(state, layout, state.national_definitions.has_siege, identation, nmid, military::province_is_under_siege(state, p) ? 1.f : 0.f);
	}
	if(state.national_definitions.blockaded) {
		active_single_modifier_description(state, layout, state.national_definitions.blockaded, identation, nmid, military::province_is_blockaded(state, p) ? 1.f : 0.f);
	}
}
...
void active_modifiers_description(sys::state& state, text::layout_base& layout, dcon::nation_id n, int32_t identation, dcon::national_modifier_value nmid) {
	if(auto ts = state.world.nation_get_tech_school(n); ts)
		active_single_modifier_description(state, layout, ts, identation, nmid);
	if(auto nv = state.world.nation_get_national_value(n); nv)
		active_single_modifier_description(state, layout, nv, identation, nmid);
	...
	state.world.for_each_technology([&](dcon::technology_id t) {
		...
		if(tmod && state.world.nation_get_active_technologies(n, t))
			active_single_modifier_description(state, layout, tmod, identation, nmid);
	});
	...
	state.world.for_each_issue([&](dcon::issue_id i) {
		...
		if(imod && (state.world.nation_get_is_civilized(n) || state.world.issue_get_issue_type(i) == uint8_t(culture::issue_type::party)))
			active_single_modifier_description(state, layout, imod, identation, nmid);
	});
	...
	if(in_wars.begin() != in_wars.end()) {
		if(state.national_definitions.war)
			active_single_modifier_description(state, layout, state.national_definitions.war, identation, nmid);
	} else {
		if(state.national_definitions.peace)
			active_single_modifier_description(state, layout, state.national_definitions.peace, identation, nmid);
	}

	if(state.national_definitions.badboy) {
		active_single_modifier_description(state, layout, state.national_definitions.badboy, identation, nmid, state.world.nation_get_infamy(n));
	}
	if(state.national_definitions.plurality) {
		active_single_modifier_description(state, layout, state.national_definitions.plurality, identation, nmid, state.world.nation_get_plurality(n));
	}
	if(state.national_definitions.war_exhaustion) {
		active_single_modifier_description(state, layout, state.national_definitions.war_exhaustion, identation, nmid, state.world.nation_get_war_exhaustion(n));
	}
	if(state.national_definitions.average_literacy) {
		auto total = state.world.nation_get_demographics(n, demographics::total);
		active_single_modifier_description(state, layout, state.national_definitions.average_literacy, identation, nmid, total > 0 ? state.world.nation_get_demographics(n, demographics::literacy) / total : 0.0f);
	}
	if(state.national_definitions.total_blockaded) {
		...
		active_single_modifier_description(state, layout, state.national_definitions.total_blockaded, identation, nmid, c > 0.0f ? bc / c : 0.0f);
	}
	if(state.national_definitions.total_occupation) {
		...
		active_single_modifier_description(state, layout, state.national_definitions.total_occupation, identation, nmid, total > 0.0f ? occupied / total : 0.0f);
	}

	if(state.world.nation_get_is_civilized(n) == false) {
		if(state.national_definitions.unciv_nation)
			active_single_modifier_description(state, layout, state.national_definitions.unciv_nation, identation, nmid);
	} else if(nations::is_great_power(state, n)) {
		if(state.national_definitions.great_power)
			active_single_modifier_description(state, layout, state.national_definitions.great_power, identation, nmid);
	} else if(state.world.nation_get_rank(n) <= uint16_t(state.defines.colonial_rank)) {
		if(state.national_definitions.second_power)
			active_single_modifier_description(state, layout, state.national_definitions.second_power, identation, nmid);
	} else {
		if(state.national_definitions.civ_nation)
			active_single_modifier_description(state, layout, state.national_definitions.civ_nation, identation, nmid);
	}
	...
	if(state.national_definitions.disarming) {
		if(bool(state.world.nation_get_disarmed_until(n)) && state.world.nation_get_disarmed_until(n) > state.current_date)
			active_single_modifier_description(state, layout, state.national_definitions.disarming, identation, nmid);
	}
	if(state.national_definitions.in_bankrupcy) {
		if(bool(state.world.nation_get_is_bankrupt(n)))
			active_single_modifier_description(state, layout, state.national_definitions.in_bankrupcy, identation, nmid);
	}
	// TODO: debt
	...
	for(auto tm : state.national_definitions.triggered_modifiers) {
		if(tm.trigger_condition && tm.linked_modifier) {
			...
			if(trigger_condition_satisfied)
				active_single_modifier_description(state, layout, tm.linked_modifier, identation, nmid);
		}
	}
...
void active_modifiers_description_with_name(sys::state& state, text::layout_base& layout, dcon::nation_id n, int32_t identation, dcon::national_modifier_value nmid) {
	...
	text::close_layout_box(layout, box);
	active_modifiers_description(state, layout, n, identation, nmid);
}
