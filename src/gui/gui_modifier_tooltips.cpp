...
void active_single_modifier_description(sys::state& state, text::layout_base& layout, dcon::modifier_id mid, int32_t indentation, bool& header, dcon::provincial_modifier_value pmid, float scaled = 1.f) {
	...
	for(uint32_t i = 0; i < def.modifier_definition_size; ++i) {
		...
		if(!header) {
			header = true;
			auto box = text::open_layout_box(layout, 0);
			text::add_to_layout_box(state, layout, box, text::produce_simple_string(state, national_modifier_names[pmid.index()].name), text::text_color::yellow);
			text::add_to_layout_box(state, layout, box, std::string_view(":"), text::text_color::yellow);
			text::close_layout_box(layout, box);
		}
...
void active_modifiers_description(sys::state& state, text::layout_base& layout, dcon::province_id p, int32_t identation, dcon::provincial_modifier_value nmid, bool have_header) { acting_modifiers_description_province(state, layout, p, identation, have_header, nmid); }
