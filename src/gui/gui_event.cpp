...
void event_desc_text::on_update(sys::state& state) noexcept {
	if(parent) {
		...
		auto contents = text::create_endless_layout(delegate->internal_layout, text::layout_parameters{0, 0, static_cast<int16_t>(base_data.size.x), static_cast<int16_t>(base_data.size.y), delegate->base_data.data.text.font_handle, 0, text::alignment::left, text::text_color::black, false});
...
void event_name_text::on_update(sys::state& state) noexcept {
	if(parent) {
		...
		auto contents = text::create_endless_layout(internal_layout, text::layout_parameters{0, 0, static_cast<int16_t>(base_data.size.x), static_cast<int16_t>(base_data.size.y), base_data.data.text.font_handle, -15, text::alignment::center, text::text_color::black, false});
