...
public:
	void on_create(sys::state& state) noexcept override {
		...
		// Brigades
		{
			auto ptr = make_element_by_type<nation_num_regiments>(state, state.ui_state.defs_by_name.find("ledger_default_textbox")->second.definition);
			apply_offset(ptr);
			add_child_to_front(std::move(ptr));
		}
		// Ships
		{
			auto ptr = make_element_by_type<nation_num_ships>(state, state.ui_state.defs_by_name.find("ledger_default_textbox")->second.definition);
			apply_offset(ptr);
			add_child_to_front(std::move(ptr));
		}
	}
...
public:
	void on_create(sys::state& state) noexcept override {
		...
		// Level
		{
			auto ptr = make_element_by_type<province_rgo_size_text>(state, state.ui_state.defs_by_name.find("ledger_default_textbox")->second.definition);
			apply_offset(ptr);
			add_child_to_front(std::move(ptr));
		}
	}
