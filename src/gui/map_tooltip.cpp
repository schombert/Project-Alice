...
void diplomatic_map_tt_box(sys::state& state, text::columnar_layout& contents, dcon::province_id prov) { // Done
	...
	if(prov.value < state.province_definitions.first_sea_province.value) {
		...
		dcon::nation_fat_id selectednation =
			(dcon::fatten(state.world, state.map_state.selected_province).get_nation_from_province_ownership().is_valid()) ? dcon::fatten(state.world, state.map_state.selected_province).get_nation_from_province_ownership() : dcon::fatten(state.world, state.local_player_nation);
...
void population_map_tt_box(sys::state& state, text::columnar_layout& contents, dcon::province_id prov) { // Done
	...
	if(prov.value < state.province_definitions.first_sea_province.value) {
		...
		text::localised_single_sub_box(state, contents, box, std::string_view("population_density_at_province"), text::variable_type::prov,
			(text::produce_simple_string(state, fat.get_name()) + ": " + text::format_float(fat.get_demographics(demographics::total), 0) + " (" + text::prettify(int64_t(fat.get_demographics(demographics::total))) + ")"));
		text::close_layout_box(contents, box);
	}
}
