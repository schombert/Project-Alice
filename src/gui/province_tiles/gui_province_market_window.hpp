...
inline static province_tile retrieve_nth_province_market_commodity(sys::state& state, dcon::province_id prov, int ind) {
	...
	if(cid.index() >= int(state.world.commodity_size())) {
		res.empty = true;
	} else {
		res.empty = false;
		res.commodity = cid;
		...
		res.market = sid.get_market_from_local_market();
	}
	return res;
}
...
public:
	...
	void on_create(sys::state& state) noexcept override {
		...
		for(int i = 0; i < 12; i++) {
			for(int j = 0; j < 8; j++) {
				if(i * 8 + j >= int(state.world.commodity_size())) {
					break;
				}

				auto ptr = make_element_by_type<province_market_commodity_button>(state, "province_market_tile");
				...
				auto ptr2 = make_element_by_type<image_element_base>(state, "province_market_tile_subicon_commodity");
