...
void player_kick_button::on_update(sys::state& state) noexcept {
	if(state.network_mode != sys::network_mode_type::host) {
		disabled = true;
		return;
	}
	auto player = retrieve<dcon::mp_player_id>(state, parent);
	disabled = !command::can_notify_player_kick(state, state.world.mp_player_get_nation_from_player_nation(player), player);
}

void player_kick_button::button_action(sys::state& state) noexcept {
	auto player = retrieve<dcon::mp_player_id>(state, parent);
	command::notify_player_kick(state, state.world.mp_player_get_nation_from_player_nation(player), false, player);
}
...
void player_kick_button::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
...
void player_ban_button::on_update(sys::state& state) noexcept {
	if(state.network_mode != sys::network_mode_type::host) {
		disabled = true;
		return;
	}
	auto player = retrieve<dcon::mp_player_id>(state, parent);
	disabled = !command::can_notify_player_ban(state, state.world.mp_player_get_nation_from_player_nation(player), player);
}

void player_ban_button::button_action(sys::state& state) noexcept {
	auto player =  retrieve<dcon::mp_player_id>(state, parent);
	command::notify_player_ban(state, state.world.mp_player_get_nation_from_player_nation(player), false, player);
}
...
void player_ban_button::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
	auto box = text::open_layout_box(contents, 0);
	text::localised_format_box(state, contents, box, std::string_view("tip_ban"));
	text::close_layout_box(contents, box);
}

void player_name_text::on_update(sys::state& state) noexcept {
...
void chat_edit_box::on_edit_command(sys::state& state, edit_command command, sys::key_modifiers mods) noexcept {
	if(command == ui::edit_command::tab) {
		state.current_scene.open_chat(state); //close/open like if tab was pressed!
		return;
