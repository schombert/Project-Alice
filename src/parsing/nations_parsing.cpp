...
namespace parsers {
void national_identity_file::any_value(std::string_view tag, association_type, std::string_view txt, error_handler& err, int32_t line,
	...
	if(scan_result.is_triggered_only) {
		if(auto it = context.map_of_provincial_events.find(scan_result.id); it != context.map_of_provincial_events.end()) {
			...
			context.map_of_provincial_events.insert_or_assign(
			    scan_result.id, pending_prov_event{dcon::provincial_event_id(), trigger::slot_contents::empty,
			                                       trigger::slot_contents::empty, trigger::slot_contents::empty, gen});
		}
		gen = scan_copy;
	} else {
		if(auto it = context.map_of_provincial_events.find(scan_result.id); it != context.map_of_provincial_events.end()) {
	...
	if(scan_result.is_triggered_only) {
		if(auto it = context.map_of_national_events.find(scan_result.id); it != context.map_of_national_events.end()) {
			...
			context.map_of_national_events.insert_or_assign(
			    scan_result.id, pending_nat_event{dcon::national_event_id(), trigger::slot_contents::empty, trigger::slot_contents::empty,
...
sys::event_option make_event_option(token_generator& gen, error_handler& err, event_building_context& context) {
	...
	if(e_context.compiled_effect.size() >= std::numeric_limits<uint16_t>::max()) {
		err.accumulated_errors += "effect " + text::produce_simple_string(context.outer_context.state, opt_result.name_) + " is " +
		                          std::to_string(e_context.compiled_effect.size()) + " cells big, which exceeds 64 KB bytecode limit (" +
		                          err.file_name + ")";
