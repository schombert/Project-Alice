...
struct trigger_building_context {
	...
	trigger_building_context(scenario_building_context& outer_context, trigger::slot_contents main_slot, trigger::slot_contents this_slot,
...
struct trigger_body {
	...
	void is_capital(association_type a, bool value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "is_capital trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void minorities(association_type a, bool value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "minorities trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void civilized(association_type a, bool value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "civilized trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void war_score(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "war_score trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void life_needs(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "life_needs trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void revanchism(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "revanchism trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void militancy(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "militancy trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void middle_tax(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "middle_tax trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.compiled_trigger.push_back(uint16_t(value * 100.0f));
	...
	void accepted_culture(association_type a, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_culture_names.find(std::string(value));
		   it != context.outer_context.map_of_culture_names.end()) {
			...
			err.accumulated_errors +=
			    "accepted_culture trigger supplied with an invalid culture (" + err.file_name + ", line " + std::to_string(line) + ")\n";
		}
	}
	...
	void has_pop_religion(association_type a, std::string_view value, error_handler& err, int32_t line,
		...
		if(is_this(value)) {
			...
			err.accumulated_errors +=
			    "has_pop_religion trigger supplied with an invalid religion (" + err.file_name + ", line " + std::to_string(line) + ")\n";
		}
	}
	...
	void continent(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "continent trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
			...
			if(is_this(value)) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
				   it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "military_access trigger supplied with an invalid tag (" + err.file_name + ", line " +
					                          std::to_string(line) + ")\n";
				}
			} else {
				err.accumulated_errors +=
	...
	void neighbour(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "neighbour trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void is_vassal(association_type a, bool value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "is_vassal trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void plurality(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "plurality trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value / 100.0f);
	...
	void corruption(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "corruption trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void is_coastal(association_type a, bool value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "is_coastal trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void in_sphere(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "in_sphere trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void total_pops(association_type a, float value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "total_pops trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		context.add_float_to_payload(value);
	...
	void truce_with(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "truce_with trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
			...
			if(is_this(value)) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
				   it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "constructing_cb trigger supplied with an invalid tag (" + err.file_name + ", line " +
					                          std::to_string(line) + ")\n";
				}
			} else {
				err.accumulated_errors +=
	...
	void vassal_of(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "vassal_of trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void in_default(association_type a, std::string_view value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "in_default trigger used in an incorrect scope type " + slot_contents_to_string(context.main_slot) +
			                          "(" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void diplomatic_influence(tr_diplomatic_influence const& value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			...
			err.accumulated_errors +=
			    "diplomatic_influence trigger supplied with an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void pop_unemployment(tr_pop_unemployment const& value, error_handler& err, int32_t line, trigger_building_context& context) {
		if(is_from(value.type)) {
			...
			err.accumulated_errors +=
			    "pop_unemployment trigger supplied with an invalid pop type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void can_build_fort_in_capital(tr_can_build_fort_in_capital const& value, error_handler& err, int32_t line,
		...
		if(context.main_slot != trigger::slot_contents::nation) {
			...
			if(value.limit_to_world_greatest_level)
				...
				context.compiled_trigger.push_back(uint16_t(trigger::can_build_fort_in_capital_yes_whole_state_no_limit |
				                                            trigger::association_eq | trigger::no_payload));
		} else {
			if(value.limit_to_world_greatest_level)
				context.compiled_trigger.push_back(uint16_t(trigger::can_build_fort_in_capital_no_whole_state_yes_limit |
				                                            trigger::association_eq | trigger::no_payload));
			else
				context.compiled_trigger.push_back(
	...
	void any_value(std::string_view label, association_type a, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_technologies.find(str_label); it != context.outer_context.map_of_technologies.end()) {
			if(context.main_slot != trigger::slot_contents::nation) {
				err.accumulated_errors +=
				    "named technology trigger used in an invalid context (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			} else {
			...
			if(context.main_slot != trigger::slot_contents::nation) {
				err.accumulated_errors +=
				    "named invention trigger used in an invalid context (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			} else {
				bool bvalue = parse_bool(value, line, err);
