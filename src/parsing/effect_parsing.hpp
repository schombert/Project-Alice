...
struct effect_building_context {
	...
	effect_building_context(scenario_building_context& outer_context, trigger::slot_contents main_slot, trigger::slot_contents this_slot,
...
struct effect_body {
	...
	void change_region_name(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::state) {
			...
			err.accumulated_errors +=
			    "change_region_name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	...
	void end_military_access(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors +=
			    "end_military_access effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void activate_technology(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_technologies.find(std::string(value));
		   it != context.outer_context.map_of_technologies.end()) {
			...
			err.accumulated_errors += "activate_technology effect supplied with invalid technology/invention name " + std::string(value) +
			                          " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void add_crisis_interest(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors +=
			    "add_crisis_interest effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void flashpoint_tension(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::state) {
			...
			err.accumulated_errors +=
			    "flashpoint_tension effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void casus_belli(ef_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			...
			if(auto it =
			       context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2]));
			   it != context.outer_context.map_of_ident_names.end()) {
	...
	void remove_casus_belli(ef_remove_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors +=
			    "remove_casus_belli effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
