...
struct ef_set_variable {
	...
	void which(association_type t, std::string_view v, error_handler& err, int32_t line, effect_building_context& context) { which_ = context.outer_context.get_national_variable(std::string(v)); }
	void finish(effect_building_context&) { }
};
...
struct ef_random_list {
	...
	void any_group(std::string_view label, int32_t value, error_handler& err, int32_t line, effect_building_context& context) { chances_sum += value; }
	void finish(effect_building_context&) { }
};
...
struct effect_body {
	...
	void capital(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "capital effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		if(0 <= value && size_t(value) < context.outer_context.original_id_to_prov_id_map.size()) {
			context.compiled_effect.push_back(trigger::payload(context.outer_context.original_id_to_prov_id_map[value]).value);
		} else {
			err.accumulated_errors += "capital effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
		...
		if(is_integer(value.data(), value.data() + value.length())) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
					...
					err.accumulated_errors += "add_core = int effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					context.compiled_effect.push_back(trigger::payload(dcon::province_id()).value);
				}
			} else {
				err.accumulated_errors += "add_core = int effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			if(context.main_slot == trigger::slot_contents::province) {
				if(is_reb(value)) {
					if(context.from_slot == trigger::slot_contents::rebel)
						...
						err.accumulated_errors += "add_core = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
					...
					if(context.from_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "add_core = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
					...
					if(context.this_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "add_core = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
		...
		if(is_integer(value.data(), value.data() + value.length())) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
					...
					err.accumulated_errors += "remove_core = int effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					context.compiled_effect.push_back(trigger::payload(dcon::province_id()).value);
				}
			} else {
				err.accumulated_errors += "remove_core = int effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			if(context.main_slot == trigger::slot_contents::province) {
				if(is_reb(value)) {
					if(context.from_slot == trigger::slot_contents::rebel)
						...
						err.accumulated_errors += "remove_core = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(is_from(value)) {
					if(context.from_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "remove_core = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(is_this(value)) {
					if(context.this_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "remove_core = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(value.length() == 3) {
					if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
				...
				err.accumulated_errors += "remove_core effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		}
	}
	void change_region_name(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::state) {
			...
			err.accumulated_errors += "change_region_name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	...
	void trade_goods(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_commodity_names.find(std::string(value)); it != context.outer_context.map_of_commodity_names.end()) {
			if(context.main_slot == trigger::slot_contents::province) {
				...
				err.accumulated_errors += "trade_goods effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void add_accepted_culture(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_accepted_culture effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "primary_culture = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "primary_culture = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				if(auto it = context.outer_context.map_of_culture_names.find(std::string(value)); it != context.outer_context.map_of_culture_names.end()) {
					...
					err.accumulated_errors += "primary_culture effect supplied with invalid culture name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				}
			}
		} else {
			err.accumulated_errors += "primary_culture effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_accepted_culture(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_culture_names.find(std::string(value)); it != context.outer_context.map_of_culture_names.end()) {
				...
				err.accumulated_errors += "remove_accepted_culture effect supplied with invalid culture name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_accepted_culture effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void religion(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_religion_names.find(std::string(value)); it != context.outer_context.map_of_religion_names.end()) {
				...
				err.accumulated_errors += "religion effect supplied with invalid religion name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "religion effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void is_slave(association_type t, bool value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::state) {
			...
			err.accumulated_errors += "is_slave effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void research_points(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "research_points effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void tech_school(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value)); it != context.outer_context.map_of_modifiers.end()) {
				...
				err.accumulated_errors += "tech_school effect supplied with invalid modifier name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "tech_school effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_reb(value)) {
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "government = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(auto it = context.outer_context.map_of_governments.find(std::string(value)); it != context.outer_context.map_of_governments.end()) {
				context.compiled_effect.push_back(uint16_t(effect::government));
				context.compiled_effect.push_back(trigger::payload(it->second).value);
			} else {
				err.accumulated_errors += "government effect supplied with invalid government name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "government effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void treasury(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "treasury effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void prestige(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "prestige effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_fixed_token_ci(value.data(), value.data() + value.length(), "culture")) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "change_tag_no_core_switch effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "change_tag_no_core_switch effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "change_tag_no_core_switch effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void set_country_flag(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "set_country_flag effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void clr_country_flag(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "clr_country_flag effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "military_access = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "military_access = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "military_access effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "military_access effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "military_access effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void badboy(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "badboy effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "secede_province = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "secede_province = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
				...
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "secede_province = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "inherit = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "inherit = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "inherit effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "inherit effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "inherit effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "annex_to = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "annex_to = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "annex_to effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "annex_to effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "annex_to effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "release effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "release effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "release effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "change_controller = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "change_controller = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "change_controller effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "change_controller effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "change_controller effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void infrastructure(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "infrastructure effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void fort(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "fort effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void naval_base(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "naval_base effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void money(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "money effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void leadership(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "leadership effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_vassal = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_vassal = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "create_vassal effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "create_vassal effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "create_vassal effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_military_access = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_military_access = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "end_military_access effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "end_military_access effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "end_military_access effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "leave_alliance = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "leave_alliance = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "leave_alliance effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "leave_alliance effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_war = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_war = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "end_war effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "end_war effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "end_war effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void enable_ideology(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_ideologies.find(std::string(value)); it != context.outer_context.map_of_ideologies.end()) {
			...
			err.accumulated_errors += "enable_ideology effect supplied with invalid ideology name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void ruling_party_ideology(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_ideologies.find(std::string(value)); it != context.outer_context.map_of_ideologies.end()) {
				...
				err.accumulated_errors += "ruling_party_ideology effect supplied with invalid ideology name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "ruling_party_ideology effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void remove_province_modifier(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value)); it != context.outer_context.map_of_modifiers.end()) {
				...
				err.accumulated_errors += "remove_province_modifier effect supplied with invalid modifier name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_province_modifier effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_country_modifier(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value)); it != context.outer_context.map_of_modifiers.end()) {
				...
				err.accumulated_errors += "remove_country_modifier effect supplied with invalid modifier name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_country_modifier effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_alliance = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_alliance = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "create_alliance effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "create_alliance effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "create_alliance effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release_vassal = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release_vassal = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_reb(value)) {
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "release_vassal = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
	...
	void change_province_name(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "change_province_name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
		auto name = text::find_or_add_key(context.outer_context.state, value);
		context.compiled_effect.push_back(trigger::payload(name).value);
	...
	void nationalvalue(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_modifiers.find(std::string(value)); it != context.outer_context.map_of_modifiers.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "nationalvalue effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "nationalvalue effect supplied with invalid modifier name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void civilized(association_type t, bool value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "civilized effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void election(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "election effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void social_reform(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_ioptions.find(std::string(value)); it != context.outer_context.map_of_ioptions.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "social_reform effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "social_reform effect supplied with invalid issue option name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void political_reform(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_ioptions.find(std::string(value)); it != context.outer_context.map_of_ioptions.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "political_reform effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "political_reform effect supplied with invalid issue option name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void add_tax_relative_income(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_tax_relative_income effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void neutrality(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "neutrality effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void reduce_pop(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "reduce_pop effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void move_pop(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			if(0 <= value && size_t(value) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "move_pop effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "move_pop effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void pop_type(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_poptypes.find(std::string(value)); it != context.outer_context.map_of_poptypes.end()) {
			if(context.main_slot == trigger::slot_contents::pop) {
				...
				err.accumulated_errors += "pop_type effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "pop_type effect supplied with invalid pop type " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void years_of_research(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "years_of_research effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void prestige_factor(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "prestige_factor effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void military_reform(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_roptions.find(std::string(value)); it != context.outer_context.map_of_roptions.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "military_reform effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void economic_reform(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_roptions.find(std::string(value)); it != context.outer_context.map_of_roptions.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "economic_reform effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "economic_reform effect supplied with invalid issue option name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_random_military_reforms(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "remove_random_military_reforms effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_random_economic_reforms(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "remove_random_economic_reforms effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void add_crime(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_crimes.find(std::string(value)); it != context.outer_context.map_of_crimes.end()) {
			...
			err.accumulated_errors += "add_crime effect supplied with invalid modifier name " + std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void nationalize(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "nationalize effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void build_factory_in_capital_state(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_factory_names.find(std::string(value)); it != context.outer_context.map_of_factory_names.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "build_factory_in_capital_state effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void activate_technology(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_technologies.find(std::string(value)); it != context.outer_context.map_of_technologies.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "activate_technology effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
	...
	void literacy(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "literacy effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void add_crisis_interest(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_crisis_interest effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void flashpoint_tension(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::state) {
			...
			err.accumulated_errors += "flashpoint_tension effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void consciousness(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "consciousness effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void militancy(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "militancy effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void rgo_size(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "rgo_size effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void add_country_modifier(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_country_modifier effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void trigger_revolt(ef_trigger_revolt const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(context.main_slot == trigger::slot_contents::nation)
			...
			err.accumulated_errors += "trigger_revolt effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	...
	void diplomatic_influence(ef_diplomatic_influence const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.who)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "diplomatic_influence = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "diplomatic_influence = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_from(value.who)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "relation = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
			} else if(is_this(value.who)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "relation = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
				...
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "relation = reb effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
			} else if(value.who.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.who[0], value.who[1], value.who[2])); it != context.outer_context.map_of_ident_names.end()) {
					...
					err.accumulated_errors += "relation effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
				err.accumulated_errors += "relation effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			...
			err.accumulated_errors += "relation effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void casus_belli(ef_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "casus_belli = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "casus_belli = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			context.compiled_effect.push_back(uint16_t(value.months));
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "casus_belli effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2])); it != context.outer_context.map_of_ident_names.end()) {
				context.compiled_effect.push_back(uint16_t(effect::casus_belli_tag));
				context.compiled_effect.push_back(trigger::payload(value.type_).value);
				context.compiled_effect.push_back(uint16_t(value.months));
				context.compiled_effect.push_back(trigger::payload(it->second).value);
			} else {
				err.accumulated_errors += "casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
	...
	void add_casus_belli(ef_add_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "add_casus_belli = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			context.compiled_effect.push_back(uint16_t(value.months));
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "add_casus_belli = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			context.compiled_effect.push_back(uint16_t(value.months));
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "add_casus_belli effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2])); it != context.outer_context.map_of_ident_names.end()) {
				...
				err.accumulated_errors += "add_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "add_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
	...
	void remove_casus_belli(ef_remove_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "remove_casus_belli = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
		} else if(is_this(value.target)) {
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "remove_casus_belli = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
		} else if(is_integer(value.target.data(), value.target.data() + value.target.length())) {
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "remove_casus_belli effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2])); it != context.outer_context.map_of_ident_names.end()) {
				...
				err.accumulated_errors += "remove_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
	...
	void this_remove_casus_belli(ef_this_remove_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "this_remove_casus_belli = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
		} else if(is_this(value.target)) {
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "this_remove_casus_belli = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
		} else if(is_integer(value.target.data(), value.target.data() + value.target.length())) {
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "this_remove_casus_belli effect given an invalid province id (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2])); it != context.outer_context.map_of_ident_names.end()) {
				...
				err.accumulated_errors += "this_remove_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "this_remove_casus_belli effect given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
	...
	void war(ef_war const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "war = from effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(is_this(value.target)) {
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "war = this effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value.target[0], value.target[1], value.target[2])); it != context.outer_context.map_of_ident_names.end()) {
	...
	void dominant_issue(ef_dominant_issue const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "dominant_issue effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void scaled_militancy(ef_scaled_militancy const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "scaled_militancy effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void scaled_consciousness(ef_scaled_consciousness const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "scaled_consciousness effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void move_issue_percentage(ef_move_issue_percentage const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation)
			...
			err.accumulated_errors += "move_issue_percentage effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	...
	void any_value(std::string_view label, association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(auto it = context.outer_context.map_of_commodity_names.find(str_label); it != context.outer_context.map_of_commodity_names.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "variable commodity name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "variable technology name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "variable invention name effect used in an incorrect scope type (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "unknown effect " + str_label + " encountered (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void limit(dcon::trigger_key lim, error_handler& err, int32_t line, effect_building_context& context) { context.compiled_effect[context.limit_position] = trigger::payload(lim).value; }
};
...
struct event_option : public effect_body {
	...
	void name(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) { name_ = text::find_or_add_key(context.outer_context.state, value); }
};
