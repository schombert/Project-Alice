...
struct ef_trigger_revolt {
	...
	void religion(association_type t, std::string_view value, error_handler& err, int32_t line,
	...
	void ideology(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
...
struct ef_wargoal {
	...
	void country(association_type t, std::string_view value, error_handler& err, int32_t line, effect_building_context& context) {
		if(value.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
				...
				err.accumulated_errors +=
					"wargoal given an invalid tag (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void state_province_id(association_type t, int32_t value, error_handler& err, int32_t line,
		effect_building_context& context) {
...
struct ef_dominant_issue {
	...
	void value(association_type t, std::string_view v, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_ioptions.find(std::string(v));
...
struct ef_add_war_goal {
	...
	void casus_belli(association_type t, std::string_view v, error_handler& err, int32_t line, effect_building_context& context) {
		if(auto it = context.outer_context.map_of_cb_types.find(std::string(v));
			it != context.outer_context.map_of_cb_types.end()) {
			casus_belli_ = it->second.id;
		} else {
			err.accumulated_errors +=
...
struct effect_body {
	...
	void add_core(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
					context.compiled_effect.push_back(
						trigger::payload(context.outer_context.original_id_to_prov_id_map[ivalue]).value);
				} else {
					err.accumulated_errors += "add_core = int effect given an invalid province id (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					context.compiled_effect.push_back(trigger::payload(dcon::province_id()).value);
				}
			} else {
				err.accumulated_errors += "add_core = int effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
			if(context.main_slot == trigger::slot_contents::province) {
				if(is_reb(value)) {
					if(context.from_slot == trigger::slot_contents::rebel)
						...
						err.accumulated_errors += "add_core = reb effect used in an incorrect scope type (" + err.file_name +
					...
					if(context.from_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "add_core = from effect used in an incorrect scope type (" + err.file_name +
												  ", line " + std::to_string(line) + ")\n";
						return;
					}
					...
					if(context.this_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "add_core = this effect used in an incorrect scope type (" + err.file_name +
												  ", line " + std::to_string(line) + ")\n";
						return;
					}
				...
				err.accumulated_errors += "add_core effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		}
		...
		if(is_integer(value.data(), value.data() + value.length())) {
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
					...
					err.accumulated_errors += "remove_core = int effect given an invalid province id (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					context.compiled_effect.push_back(trigger::payload(dcon::province_id()).value);
				}
			} else {
				err.accumulated_errors += "remove_core = int effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			if(context.main_slot == trigger::slot_contents::province) {
				if(is_reb(value)) {
					if(context.from_slot == trigger::slot_contents::rebel)
						...
						err.accumulated_errors += "remove_core = reb effect used in an incorrect scope type (" + err.file_name +
												  ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(is_from(value)) {
					if(context.from_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "remove_core = from effect used in an incorrect scope type (" + err.file_name +
												  ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(is_this(value)) {
					if(context.this_slot == trigger::slot_contents::nation)
						...
						err.accumulated_errors += "remove_core = this effect used in an incorrect scope type (" + err.file_name +
												  ", line " + std::to_string(line) + ")\n";
						return;
					}
				} else if(value.length() == 3) {
					if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
						...
						err.accumulated_errors += "remove_core effect given an invalid tag (" + err.file_name + ", line " +
												  std::to_string(line) + ")\n";
						return;
					}
				} else {
				...
				err.accumulated_errors += "remove_core effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		}
	...
	void trade_goods(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_commodity_names.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::province) {
				...
				err.accumulated_errors += "trade_goods effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void add_accepted_culture(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_accepted_culture effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void primary_culture(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "primary_culture effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_accepted_culture(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_culture_names.find(std::string(value));
				...
				err.accumulated_errors += "remove_accepted_culture effect supplied with invalid culture name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_accepted_culture effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void research_points(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "research_points effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_reb(value)) {
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "government = reb effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
	...
	void war_exhaustion(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "war_exhaustion effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_fixed_token_ci(value.data(), value.data() + value.length(), "culture")) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "change_tag_no_core_switch effect given an invalid tag (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else {
			...
			err.accumulated_errors += "change_tag_no_core_switch effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void set_country_flag(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			context.compiled_effect.push_back(
				trigger::payload(context.outer_context.get_national_flag(std::string(value))).value);
		} else if(context.main_slot == trigger::slot_contents::province) {
			context.compiled_effect.push_back(uint16_t(effect::set_country_flag_province));
			context.compiled_effect.push_back(
				trigger::payload(context.outer_context.get_national_flag(std::string(value))).value);
		} else {
			err.accumulated_errors += "set_country_flag effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void clr_country_flag(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "clr_country_flag effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "military_access effect given an invalid tag (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					return;
				}
			} else {
			...
			err.accumulated_errors += "military_access effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(is_this(value)) {
				...
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "secede_province = reb effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "secede_province effect given an invalid tag (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					return;
				}
			} else {
			...
			err.accumulated_errors += "secede_province effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "inherit = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "inherit = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
	...
	void annex_to(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "annex_to = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "annex_to = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "change_controller = this effect used in an incorrect scope type (" +
											  err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "change_controller = from effect used in an incorrect scope type (" +
											  err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "change_controller effect given an invalid tag (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					return;
				}
			} else {
			...
			err.accumulated_errors += "change_controller effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void infrastructure(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "infrastructure effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_vassal = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "create_vassal = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
			...
			err.accumulated_errors += "create_vassal effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_military_access = this effect used in an incorrect scope type (" +
											  err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_military_access = from effect used in an incorrect scope type (" +
											  err.file_name + ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "end_military_access effect given an invalid tag (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					return;
				}
			} else {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "leave_alliance = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "leave_alliance = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_war = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "end_war = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(value.length() == 3) {
	...
	void ruling_party_ideology(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_ideologies.find(std::string(value));
				...
				err.accumulated_errors += "ruling_party_ideology effect supplied with invalid ideology name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "ruling_party_ideology effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void remove_province_modifier(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value));
				...
				err.accumulated_errors += "remove_province_modifier effect supplied with invalid modifier name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_province_modifier effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void remove_country_modifier(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value));
				...
				err.accumulated_errors += "remove_country_modifier effect supplied with invalid modifier name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "remove_country_modifier effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				...
				if(auto it = context.outer_context.map_of_ident_names.find(nations::tag_to_int(value[0], value[1], value[2]));
					...
					err.accumulated_errors += "create_alliance effect given an invalid tag (" + err.file_name + ", line " +
											  std::to_string(line) + ")\n";
					return;
				}
			} else {
			...
			err.accumulated_errors += "create_alliance effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_this(value)) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release_vassal = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_from(value)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "release_vassal = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			} else if(is_reb(value)) {
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "release_vassal = reb effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			...
			err.accumulated_errors += "release_vassal effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void change_province_name(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::province) {
			...
			err.accumulated_errors += "change_province_name effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
		auto name = text::find_or_add_key(context.outer_context.state, value);
	...
	void nationalvalue(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_modifiers.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "nationalvalue effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void election(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
	...
	void social_reform(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_ioptions.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "social_reform effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void political_reform(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_ioptions.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "political_reform effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void add_tax_relative_income(association_type t, float value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "add_tax_relative_income effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void move_pop(association_type t, int32_t value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			if(0 <= value && size_t(value) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				context.compiled_effect.push_back(
					trigger::payload(context.outer_context.original_id_to_prov_id_map[value]).value);
			} else {
				err.accumulated_errors +=
	...
	void pop_type(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(auto it = context.outer_context.map_of_poptypes.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::pop) {
				...
				err.accumulated_errors += "pop_type effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "pop_type effect supplied with invalid pop type " + std::string(value) + " (" +
									  err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void years_of_research(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "years_of_research effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void prestige_factor(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::nation) {
			...
			err.accumulated_errors += "prestige_factor effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void military_reform(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_roptions.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "military_reform effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void economic_reform(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_roptions.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "economic_reform effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void add_crime(association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(auto it = context.outer_context.map_of_crimes.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::province) {
				...
				err.accumulated_errors += "add_crime effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
	...
	void build_factory_in_capital_state(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_factory_names.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "build_factory_in_capital_state effect used in an incorrect scope type (" +
										  err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "build_factory_in_capital_state effect supplied with invalid factory name " +
									  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void activate_technology(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(auto it = context.outer_context.map_of_technologies.find(std::string(value));
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "activate_technology effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
	...
	void consciousness(association_type t, float value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "consciousness effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void add_province_modifier(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value));
				...
				err.accumulated_errors += "add_province_modifier effect supplied with invalid modifier name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "add_province_modifier effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void add_country_modifier(association_type t, std::string_view value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(auto it = context.outer_context.map_of_modifiers.find(std::string(value));
				...
				err.accumulated_errors += "add_country_modifier effect supplied with invalid modifier name " +
										  std::string(value) + " (" + err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
			err.accumulated_errors += "add_country_modifier effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	void trigger_revolt(ef_trigger_revolt const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(context.main_slot == trigger::slot_contents::nation)
			...
			err.accumulated_errors += "trigger_revolt effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	...
	void diplomatic_influence(ef_diplomatic_influence const& value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "diplomatic_influence effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
		if(is_from(value.who)) {
			...
			if(auto it =
				...
				err.accumulated_errors += "diplomatic_influence effect given an invalid tag (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(is_from(value.who)) {
				if(context.from_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "relation = from effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
				...
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "relation = this effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
				...
				if(context.from_slot == trigger::slot_contents::rebel)
					...
					err.accumulated_errors += "relation = reb effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(int16_t(value.value)).value);
				...
				if(auto it = context.outer_context.map_of_ident_names.find(
					   nations::tag_to_int(value.who[0], value.who[1], value.who[2]));
					it != context.outer_context.map_of_ident_names.end()) {
	...
	void casus_belli(ef_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "casus_belli = from effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "casus_belli = this effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "casus_belli effect given an invalid province id (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
	...
	void add_casus_belli(ef_add_casus_belli const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "add_casus_belli effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "add_casus_belli = from effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "add_casus_belli = this effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "add_casus_belli effect given an invalid province id (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
	...
	void remove_casus_belli(ef_remove_casus_belli const& value, error_handler& err, int32_t line,
		...
		if(is_from(value.target)) {
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "remove_casus_belli effect given an invalid province id (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
	...
	void this_remove_casus_belli(ef_this_remove_casus_belli const& value, error_handler& err, int32_t line,
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "this_remove_casus_belli = from effect used in an incorrect scope type (" +
										  err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(context.this_slot == trigger::slot_contents::nation)
				...
				err.accumulated_errors += "this_remove_casus_belli = this effect used in an incorrect scope type (" +
										  err.file_name + ", line " + std::to_string(line) + ")\n";
				return;
			}
			context.compiled_effect.push_back(trigger::payload(value.type_).value);
			...
			if(0 <= ivalue && size_t(ivalue) < context.outer_context.original_id_to_prov_id_map.size()) {
				...
				err.accumulated_errors += "this_remove_casus_belli effect given an invalid province id (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
			if(auto it = context.outer_context.map_of_ident_names.find(
				...
				err.accumulated_errors += "this_remove_casus_belli effect given an invalid tag (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else {
	...
	void war(ef_war const& value, error_handler& err, int32_t line, effect_building_context& context) {
		...
		if(is_from(value.target)) {
			if(context.from_slot == trigger::slot_contents::nation)
				...
				context.compiled_effect.push_back(
					value.call_ally ? effect::war_from_province : effect::war_no_ally_from_province);
			else {
				err.accumulated_errors += "war = from effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else if(is_this(value.target)) {
			if(context.this_slot == trigger::slot_contents::nation)
				...
				context.compiled_effect.push_back(
					value.call_ally ? effect::war_this_province : effect::war_no_ally_this_province);
			else if(context.this_slot == trigger::slot_contents::state)
				...
				err.accumulated_errors += "war = this effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
		} else if(value.target.length() == 3) {
		...
		if(context.main_slot == trigger::slot_contents::nation) {
			if(value.days <= 0) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "country_event effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
			...
			err.accumulated_errors += "country_event effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
		...
		if(context.main_slot == trigger::slot_contents::province) {
			if(value.days <= 0) {
				if(context.this_slot == trigger::slot_contents::nation)
					...
					err.accumulated_errors += "province_event effect used in an incorrect scope type (" + err.file_name +
											  ", line " + std::to_string(line) + ")\n";
					return;
				}
				context.compiled_effect.push_back(trigger::payload(value.id_).value);
	...
	void sub_unit(ef_sub_unit const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(is_this(value.value)) {
			if(context.main_slot != trigger::slot_contents::nation || context.this_slot != trigger::slot_contents::province) {
				err.accumulated_errors += "sub_unit effect used in an incorrect scope type (" + err.file_name + ", line " +
										  std::to_string(line) + ")\n";
				return;
			}
	...
	void set_variable(ef_set_variable const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "set_variable effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	...
	void change_variable(ef_change_variable const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "change_variable effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	...
	void dominant_issue(ef_dominant_issue const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "dominant_issue effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void scaled_militancy(ef_scaled_militancy const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "scaled_militancy effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void scaled_consciousness(ef_scaled_consciousness const& value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::pop) {
			...
			err.accumulated_errors += "scaled_consciousness effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	}
	...
	void add_war_goal(ef_add_war_goal const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation || context.from_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "add_war_goal effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	...
	void move_issue_percentage(ef_move_issue_percentage const& value, error_handler& err, int32_t line,
		...
		if(context.main_slot == trigger::slot_contents::nation)
			...
			err.accumulated_errors += "move_issue_percentage effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
	...
	void party_loyalty(ef_party_loyalty const& value, error_handler& err, int32_t line, effect_building_context& context) {
		if(bool(value.province_id_)) {
			...
			err.accumulated_errors += "party_loyalty effect used in an incorrect scope type (" + err.file_name + ", line " +
									  std::to_string(line) + ")\n";
			return;
		}
	}
	void build_railway_in_capital(ef_build_railway_in_capital const& value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "build_railway_in_capital effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
		if(value.in_whole_capital_state && value.limit_to_world_greatest_level)
			...
			context.compiled_effect.push_back(
				uint16_t(effect::build_railway_in_capital_yes_whole_state_no_limit | effect::no_payload));
		else if(!value.in_whole_capital_state && value.limit_to_world_greatest_level)
			context.compiled_effect.push_back(
				uint16_t(effect::build_railway_in_capital_no_whole_state_yes_limit | effect::no_payload));
		else
			context.compiled_effect.push_back(
				uint16_t(effect::build_railway_in_capital_no_whole_state_no_limit | effect::no_payload));
	}
	void build_fort_in_capital(ef_build_fort_in_capital const& value, error_handler& err, int32_t line,
		effect_building_context& context) {
		if(context.main_slot != trigger::slot_contents::nation) {
			err.accumulated_errors += "build_fort_in_capital effect used in an incorrect scope type (" + err.file_name +
									  ", line " + std::to_string(line) + ")\n";
			return;
		}
		if(value.in_whole_capital_state && value.limit_to_world_greatest_level)
			context.compiled_effect.push_back(
				uint16_t(effect::build_fort_in_capital_yes_whole_state_yes_limit | effect::no_payload));
		else if(value.in_whole_capital_state && !value.limit_to_world_greatest_level)
			context.compiled_effect.push_back(
				uint16_t(effect::build_fort_in_capital_yes_whole_state_no_limit | effect::no_payload));
		else if(!value.in_whole_capital_state && value.limit_to_world_greatest_level)
			context.compiled_effect.push_back(
				uint16_t(effect::build_fort_in_capital_no_whole_state_yes_limit | effect::no_payload));
		else
			context.compiled_effect.push_back(
				uint16_t(effect::build_fort_in_capital_no_whole_state_no_limit | effect::no_payload));
	}
	void any_value(std::string_view label, association_type t, std::string_view value, error_handler& err, int32_t line,
		effect_building_context& context) {
		...
		if(auto it = context.outer_context.map_of_commodity_names.find(str_label);
			...
			if(context.main_slot == trigger::slot_contents::nation) {
				...
				err.accumulated_errors += "variable commodity name effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else if(auto itb = context.outer_context.map_of_technologies.find(str_label);
				  itb != context.outer_context.map_of_technologies.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
		...
		} else if(auto itc = context.outer_context.map_of_inventions.find(str_label);
				  itc != context.outer_context.map_of_inventions.end()) {
			if(context.main_slot == trigger::slot_contents::nation) {
				if(parse_bool(value, line, err))
				...
				err.accumulated_errors += "variable invention name effect used in an incorrect scope type (" + err.file_name +
										  ", line " + std::to_string(line) + ")\n";
				return;
			}
		} else {
