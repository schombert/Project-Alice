...
- `std::optional<file> open_file(directory const& dir, native_string_view file_name)` -- This function opens the file with the desired name and places its contents in memory, if possible. If the optional does not hold a `file`, the the file could not be opened. Do add any system-dependent path separators to the file name; even if you could make it work, it wouldn't be portable.
